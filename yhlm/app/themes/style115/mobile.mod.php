<?php
/**
 * [WEIDONGLI] Copyright (c) 2013 B2CTUI.COM
 */
...
function mobile_nav($position) {
	global $_W;
	$navs = pdo_fetchall("SELECT name, url, icon, css, position, module,displayorder FROM ".tablename('site_nav')." WHERE (position = '$position' OR position = '3') AND status = 1 AND weid = '{$_W['weid']}' ORDER BY displayorder DESC");
	if (!empty($navs)) {
