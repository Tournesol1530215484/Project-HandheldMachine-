...
<style type="text/css">
    ...
    .detail_topbar {height:44px; background:#5f6e8b; padding:6px;}
...
.list {min-height:75px; border-left:1px solid #eee; padding-left:20px; position:relative;}
.list .info {min-height:75px; border-top:1px solid #eee; padding:10px; font-size:14px; color:#666;}
.list .info .step { min-height:40px;}
.list .info .time { min-height:20px;}
...
<script id='tpl_detail' type='text/html'>
 <div class="page_topbar">
     <a href="javascript:history.go(-1);" class="back"><i class="fa fa-angle-left"></i></a>
...
<script type="text/javascript"> 
    ...
    require(['tpl', 'core'], function(tpl, core) {
        ...
        <?php  if(!empty($_GPC['merch'])) { ?>
        var data={
            id:"<?php  echo $_GPC['id'];?>",
            merch:<?php  echo $_GPC['merch'];?>
        };
        <?php  } else { ?>
        var data={
            id:"<?php  echo $_GPC['id'];?>"
        ...
        core.json('order/express',data,function(json){
                 ...
                 core.json('order/express',{op:'step',id:'<?php  echo $_GPC['id'];?>',express:order.express,expresssn:order.expresssn,'op':'step'},function(pjson){
                     console.log(pjson)
