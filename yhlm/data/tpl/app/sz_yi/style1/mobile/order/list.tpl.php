...
<style type="text/css">
    ...
    .order_topbar .on {color:#00c1ff; border-bottom:2px solid #00c1ff;}
    ...
    .order_main {height:auto; width:100%; background:#fff; padding:0px 3%; margin-top:16px; border-bottom:1px solid #e2e2e2; border-top:1px solid #e2e2e2;}
    .order_main .title {height:42px; width:100%; border-bottom:1px solid #e2e2e2; font-size:14px; line-height:42px; color:#666;}
    .order_main .title span {height:42px; width:auto; float:right; color:#00c1ff;}
    ...
    .order_main .good .info .inner { margin-left:60px;margin-right:80px; }
    ...
    .order_main .good .price { float:right;width:80px;;height:54px;margin-left:-60px;;}
    ...
    .order_main .sub1 {height:30px; width:auto; padding:0px 10px; border:1px solid #00c1ff; float:right; border-radius:5px; line-height:30px; font-size:14px; margin:10px 5px 10px 0px; color:#fff; background:#00c1ff;}
    .order_main .sub2 {height:30px; width:auto; padding:0px 10px; border:1px solid #ddd; float:right; border-radius:5px; line-height:30px; font-size:14px; margin:10px 5px 10px 0px; color:#999;}
    select { width:80px;height:30px;position:absolute;left:0; filter:alpha(Opacity=0); opacity: 0;-webkit-appearance: none;background:#fff; -webkit-tap-highlight-color: transparent;top: 0}
...
<script id='tpl_order_list' type='text/html'>
    ...
    <div class="order_topbar">
        ...
        <div class="nav <?php  if($_GPC['status']=='4') { ?>on<?php  } ?>"  data-status="4">退款订单</div>
        <?php  } ?>

        <!-- 到时候改为下面被注释的代码的话 对应的css样式也要更改 -->
        <!-- <?php  if($_GPC['status'] == 4 || ($_GPC['status'] == -1)) { ?>
        <div class="nav <?php  if($_GPC['status']=='') { ?>on<?php  } ?>" data-status="">其他订单</div>
        <div class="nav <?php  if($_GPC['status']=='4') { ?>on<?php  } ?>"  data-status="4">退款订单</div>
        <?php  } else { ?>
        <div class="nav <?php  if($_GPC['status']=='') { ?>on<?php  } ?>" data-status="">全部</div>
...
<script id='tpl_order' type='text/html'>
    ...
    <div class="order_main" data-orderid="<%order.id%>">
        ...
        <div class="sub">
			...
			<%/if%>
            <div class="sub2 order_cancel" style='position:relative;'>
                <span style='display:block;'>取消订单</span>
                <select>
...
<script id='tpl_empty' type='text/html'>
    <div class="order_no"><i class="fa fa-file-text-o" style="font-size:100px;"></i><br><br><span style="line-height:18px; font-size:16px;">您还没有相关订单</span><br>可以去看看哪些想买的</div>
...
<script language='javascript'>
  ...
  $('.order_express').unbind('click').click(function() {
             ...
             core.json('order/list', {page:page, status: '<?php  echo $_GPC['status'];?>'}, function(json) {
                          ...
                          if(loaded){
                            ...
                            if($(document).height() <= totalheight){ 
                                ...
                                if(stop==true){ 
                                    ...
                                    page++;
                                    core.json('order/list', {page:page, status: '<?php  echo $_GPC['status'];?>'}, function(morejson) {  
                                        stop = true;
