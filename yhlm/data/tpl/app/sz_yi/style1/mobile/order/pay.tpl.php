...
.order_main .line .label { float:left;width:80px;font-size: 14px;font-weight: normal;line-height: 35px;color: #999}
.order_main .line .info { float:right; width:100%; margin-left:-85px;text-align: right;overflow:hidden;height:44px;}
.order_main .line .info .inner { color:#666;margin-left:85px;text-align: left;}
...
.order_sub3 {/*height:44px;*/height: auto; margin:14px 5px;background:#e2cb04; border-radius:4px; text-align:center; font-size:16px; /*line-height:44px;*/ color:#fff;padding: 5px 0;line-height: 1.5;}
...
.order_main1 {height:50px;padding:10px;border-bottom:1px solid #f0f0f0; border-top:1px solid #f0f0f0; background:#fff;text-align:center;margin-top:10px; }
...
<script id='tpl_order_info' type='text/html'>
    ...
    <div class="order_main" >  
        ...
        <div class="line"><div class="label">支付金额</div>
            <div class="info">
                <div class="inner">
                    <div style='color:#ff6600'><?php  if($isexchange == 1) { ?><?php  } else { ?>￥<?php  } ?>
                        <span id="orderprice" price="<%order.price%>">
                            <?php  if($isexchange == 1) { ?>
                                <%order.goodsprice%>
                            <?php  } else { ?>
                                <%order.price%>
                        ...
                        <?php  if($isexchange == 1) { ?>换货码 (运费<%order.dispatchprice%>余额支付)<?php  } else { ?>元<?php  } ?>
                    </div>
                </div>
            </div>
        </div>
    ...
    <?php  if($isexchange == 1) { ?>
        <%if wechat.success%><div class="button order_sub1">微信支付邮费</div><%/if%>
    <?php  } else { ?>
    ...
    <?php  if($isexchange == 1) { ?>
        <div class="button order_sub3">换货码支付<br />(当前换货码:<%credit.current%>,当前余额:<%credit.credit2%>)</div>
    <?php  } else { ?>
        <div class="button order_sub3">余额支付<br />(当前余额:<%credit.current%>)</div>
    <?php  } ?>
...
<script language="javascript">
    ...
    require(['tpl', 'core'], function(tpl, core) {
        core.json('order/pay',{orderid:'<?php  echo $_GPC['orderid'];?>',openid:"<?php  echo $openid;?>"},function(json){
            ...
            console.log(result);
            credit3=parseFloat(result.credit.current);                               
            goodsprice=parseFloat(result.order.goodsprice);         
            dispatchprice=parseFloat(result.order.dispatchprice);
           ...
           if(result.wechat.success){
                $('.order_sub1').click(function(){   
                        ...
                        <?php  if($isexchange == 1) { ?>
                            if (goodsprice > credit3) {
                                core.tip.show('换货码不足，支付失败！');
                                return;                      
                            }

                            if (dispatchprice <= 0) {
                                core.tip.show('邮费为零,无需使用微信支付邮费，请直接点击下面的换货码支付');
                                return;                      
                            }                                       
                        <?php  } ?>
                         ...
                         core.json('order/pay', {op: 'pay',type: 'weixin', orderid:'<?php  echo $_GPC['orderid'];?>',deduct:deduct}, function (rjson) {
                            ...
                            var YDB = new YDBOBJ();
                            var data = rjson.result;
                          
                            YDB.SetWxpayInfo(data.productName ,data.desicript,data.price, data.outtradeno, data.attach);
                        	return ;
