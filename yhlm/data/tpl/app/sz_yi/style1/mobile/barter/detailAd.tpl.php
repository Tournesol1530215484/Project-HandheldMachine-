...
<div class="report-page">
    ...
    <div class="mui-content report-type-box">
		<ul class="mui-table-view mui-table-view-radio">
			<?php  if(is_array($reprot_type)) { foreach($reprot_type as $k => $v) { ?>
			<li class="mui-table-view-cell">
				<a class="mui-navigate-right report-type <?php  if($v['title'] == '其他原因') { ?>other-report-type<?php  } ?>" data-rtype="<?php  echo $v['id'];?>"><!-- data-rtype 举报类型-->
					<?php  echo $v['title'];?> 	 	
				</a>
			</li>
			<?php  } } ?>
    ...
    <input type="hidden" name="adid" id="adid_input" value="<?php  echo $_GPC['id'];?>"><!-- 广告id -->
    <input type="hidden" name="uid" id="uid_input" value="<?php  echo $tempad['recuid'];?>"> <!-- 店铺id -->
</div>
...
<div class="description">
    ...
    <div class="description-cont">
    	...
    	<div class="show-input-box flex-center"> <!-- core-single-on类添加 这里的个数等于数据库中设置的core长度 动态获取 个数不固定-->
    		<?php  if(is_array($corearr)) { foreach($corearr as $k => $v) { ?>
    	...
    	<div class="core-input-operate-box flex-center">
    		<div class="core-key-board"> <!-- 个数固定 -->
    			<?php  if(is_array($full)) { foreach($full as $k => $v) { ?>
    			<div class="core-single-board core-single"><?php  echo $v;?></div>
	    		<?php  } } ?>
    		</div>
...
<script id="tpl_main" type="text/html">
    ...
    <div class="external-video-box" >
    	<div class="video-content" ><!-- 下面的两个frame都可以看看样式 第一个是参考的视频到时候不能用-->
    		...
    		<iframe height="95%" width="99%" frameborder="0" allowfullscreen src="<?php  echo $tempad['video'];?>"></iframe>
    ...
    <div class="external-link-box">
    	<iframe class="external-link-frame" src="<?php  echo $tempad['outside'];?>"></iframe>
    </div>	 	 
    <%/if%>
    <!-- 轮播图 -->
	...
	<div class="recom-goods-box">
        <ul class="goods-ul">
        <%each ad.goods as v%>      
            <li class="goods-item clearfloat flex-center" onclick="location.href='<%v.url%>'">
	...
	<div class="shop-box">
		...
		<div class="shop-btn-box flex-center">  	 	
			...
			<div class="attent-shops <?php  if(!$isfavorite) { ?>attented-shops<?php  } else { ?><?php  } ?>"><i class="fa fa-user-plus shop-btn-icon" aria-hidden="true"></i><span class="attent-shops-tag"><?php  if($isfavorite) { ?>已关注<?php  } else { ?>关注店铺<?php  } ?></span></div>
		</div>
	</div>
	<!-- 底部咨询和拆红包按钮 根据不同情况选择显示 start -->
	<div class="consult-envelope-box flex-center">
		<div class="consult-btn" onclick="location.href='<?php  echo $consult;?>'">
			<span class="consult-icon"><i class="fa fa-comment-o" aria-hidden="true"></i></span>
			<span class="consult-tag">咨询</span>
		</div>
		<?php  if(empty($_GPC['isedit'])) { ?>
		<?php  if($sure) { ?>
...
<script type="text/javascript">
	require(['tpl', 'core'], function(tpl, core) {
		...
		core.json('barter/ad', {op:'detail',id:'<?php  echo $_GPC['id'];?>'}, function (json) {
            ...
            $('.ad-report').unbind('click').click(function(){
                ...
                $('.report-confirm-btn').unbind('click').click(function(){
                    ...
                    if($report_dom.length == 0){
                    	...
                    	core.json('barter/ad', {op:'report',id:'<?php  echo $_GPC["id"];?>',uid: uid, report_type: report_type, content:content}, function (json) {
			...
			$(".core-confirm-btn").click(function(){
				var cores = "";
				...
				require(['tpl', 'core'], function(tpl, core) {
					core.json('barter/bonus', {op:'draw',id:'<?php  echo $_GPC["id"];?>',core:cores}, function (json) {
			...
			$(".attent-shops").click(function(){
				...
				core.json('member/merch', {op:'set',sid:'<?php  echo $tempad['uid'];?>'}, function (json) {
