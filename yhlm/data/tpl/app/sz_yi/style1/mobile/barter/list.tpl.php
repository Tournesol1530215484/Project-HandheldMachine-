...
<style type="text/css">
   ...
   .main .category .all p.category_item { padding-top:10px;}
    ...
    .main .page .goods .good .price {height:25px; width:100%; font-size:15px; color:#cccccc;/*#e43a3d*/}
    .main .page .goods .good .price .price-val{
        font-size:15px; 
        color:#e43a3d;
        padding: 0;
    }
    .main .page .goods .good .price span {height:23px; width:auto; padding:2px 6px; color:#cccccc; font-size:12px; border-radius:5px; }
    .main .page .goods .good .price .original-price{
        text-decoration: line-through;
    }
    .main .page .goods .good .price .goods_list_pirce_unit {
        color: #9d9d9d;
        font-size: 15px;
        padding: 2px;
    }
    ...
    .main .page .sort_list {height:125px;/*height:100px;*/ width:90px; background:rgba(51,51,51,0.8); border-radius:5px; position:absolute; top:50px; right:5%; display:none;z-index:999}
...
.for-sort .for-sort-asc{
    position: absolute;
    top: 0;
    right: 4px;
}
.for-sort .for-sort-desc{
    position: absolute;
    bottom: 0;
    right: 4px;
}
...
.comprehensive-box .comprehensive-item{
    padding: 5px;
}
.comprehensive-box .comprehensive-on{
    color: #fd5454;
}
.no-wrap{
    white-space: nowrap;
...
<script id='tpl_main' type='text/html'>
    ...
    <div class="main">
        ...
        <div class='page'>
            <!--排序div-->
            <div class="sort_list">
                <div class="nav <?php  if($_GPC['order']=='sales' && $_GPC['by']=='asc') { ?>navon<?php  } ?>"  data-order='sales' data-by='asc'>销量从低到高</div>
                <div class="nav <?php  if($_GPC['order']=='') { ?>navon<?php  } ?> <?php  if($_GPC['order']=='sales' && $_GPC['by']=='desc') { ?>navon<?php  } ?>"  data-order='sales' data-by='desc'>销量从高到低</div>
           ...
           <!-- 排序分类 -->
            <div class="sortList">
                <ul class="sort-column">
                    <!-- navSort 根据状态来判断标志综合排序旁边的下拉菜单状态的小图标 -->
                    <li class="comprehensive-total <?php  if($_GPC['order']=='salesreal' || $_GPC['order']=='PostFlag' || $_GPC['order']=='LocalFlag') { ?>active<?php  } ?>" data-order="salesreal" data-by="desc" data-num='0'>综合
                        <?php  if(true) { ?>
                        <i class="comprehensive-icon fa fa-angle-down" aria-hidden="true"></i>
                        <?php  } else { ?>
                        <i class="comprehensive-icon fa fa-angle-up" aria-hidden="true"></i>
                        <?php  } ?>
                    </li>
                    <li class="sales-tag navSort for-sort <?php  if($_GPC['order']=='sales') { ?>active<?php  } ?>" data-order="sales" <?php  if($_GPC['order']=='sales' && $_GPC['by']=='asc') { ?>data-by="asc"<?php  } else if($_GPC['order']=='' || ($_GPC['order']=='sales' && $_GPC['by']=='desc')) { ?>data-by="desc"<?php  } else { ?>data-by="desc"<?php  } ?>>销量
                        <!-- 销量升序 -->
                        <i class="fa fa-sort-asc for-sort-asc <?php  if($_GPC['order']=='sales' && $_GPC['by']=='asc') { ?>sort-on<?php  } else { ?>no-sort-on<?php  } ?>"  aria-hidden="true"></i>
                        <!-- 销量降序 -->
                        <i class="fa fa-sort-desc for-sort-desc <?php  if($_GPC['order']=='sales' && $_GPC['by']=='desc') { ?>sort-on<?php  } else { ?>no-sort-on<?php  } ?>"  aria-hidden="true"></i>
                    </li>
                    <li class="price-tag navSort for-sort <?php  if($_GPC['order']=='marketprice') { ?>active<?php  } ?>" data-order="marketprice" <?php  if($_GPC['order']=='marketprice' && $_GPC['by']=='asc') { ?>data-by="asc"<?php  } else if($_GPC['marketprice']=='' || ($_GPC['marketprice']=='sales' && $_GPC['by']=='desc')) { ?>data-by="desc"<?php  } else { ?>data-by="desc"<?php  } ?>>价格
                        <!-- 价格升序 -->
                        <i class="fa fa-sort-asc for-sort-asc <?php  if($_GPC['order']=='marketprice' && $_GPC['by']=='asc') { ?>sort-on<?php  } else { ?>no-sort-on<?php  } ?>"  aria-hidden="true"></i>
                        <!-- 价格降序 -->
                        <i class="fa fa-sort-desc for-sort-desc <?php  if($_GPC['order']=='marketprice' && $_GPC['by']=='desc') { ?>sort-on<?php  } else { ?>no-sort-on<?php  } ?>"  aria-hidden="true"></i>
                    </li>
            ...
            <div class="comprehensive-box">
                <ul class="comprehensive-ul">
                    <li class="navSort comprehensive-item <?php  if($_GPC['order']=='salesreal') { ?>active<?php  } ?>" data-order="salesreal" data-by="desc">综合排序</li>
                    <li class="navSort comprehensive-item <?php  if($_GPC['order']=='PostFlag') { ?>active<?php  } ?>" data-order="PostFlag" data-by="desc">邮寄</li>
                    <li class="navSort comprehensive-item <?php  if($_GPC['order']=='LocalFlag') { ?>active<?php  } ?>" data-order="LocalFlag" data-by="desc">现场交易</li>
                </ul>
            </div>
            <!--商品列表-->
            <div class="goods">
...
<script id='tpl_goods_list' type='text/html'>
  <form action="" method="get" id="goodsform">
    ...
    <div class="good" data-goodsid='<?php  echo $g['id'];?>' style="position:relative;">
        ...
        <div class="name">
        <span class="pl">
            <?php  if($g['PostFlag'] == 1) { ?>邮寄<?php  } ?> 
            <?php  if($g['PostFlag'] == 1 && $g['LocalFlag'] == 1 ) { ?> & <?php  } ?>
            <?php  if($g['LocalFlag'] == 1) { ?>现场<?php  } ?>   
        </span>
        <?php  echo $g['title'];?> </div>
        <div class="price no-wrap">
            <span class="price-val">
            <?php  if($g['minprice'] != $g['maxprice']) { ?><?php  echo $g['minprice'];?> - <?php  echo $g['maxprice'];?><?php  } else { ?><?php  echo $g['minprice'];?><?php  } ?></span><span class="goods_list_pirce_unit">换货码</span>
            <?php  if($g['productprice']>0 && $g['marketprice']!=$g['productprice']) { ?>
            <span class="original-price">￥<?php  echo $g['productprice'];?></span>
            <?php  } ?>
        </div>
    ...
    <div class="category">
        ...
        <div class="all">
            <%each category as parent%>
                <%if parent.id!='824'%>
                    <a href="<?php  echo $this->createMobileUrl('barter/list')?>&pcate=<%parent.id%>">
                        <p class='category_item' style=" color:#ffff00;"><i class="fa fa-angle-double-right"></i> <%parent.name%></p>
                    </a>
                    <%each parent.children as child%>
                    <a href="<?php  echo $this->createMobileUrl('barter/list')?>&pcate=<%parent.id%>&ccate=<%child.id%>">
...
<script id='tpl_empty' type='text/html'>
    ...
    <div class="list_no_menu">
        <div class="list_no_nav" onclick="location.href='<?php  echo $this->createMobileUrl('barter/list')?>'">看看其他的</div>
    </div>
</script>
<script language='javascript'>
    ...
    require(['tpl', 'core'], function (tpl, core) {
        ...
        function getGoods() {
            ...
            core.json('barter/list', args, function (json) {
        ...
        function bindCategorySortEvents(){              
            //绑定分类事件
        ...
        $('.navSort').click(function () {               
            //右上排序事件 排除销量和价格的特殊toggle选项
            if ($(this).data('order') ==args.order && $(this).data('by') == args.by && !$(this).hasClass("for-sort")) {
                return;
            }
            ...
            $("#order").val($(this).data('order'));
            $("#by").val($(this).data('by'));
            if($(this).hasClass("price-tag") && $(this).hasClass("active")){
                if($(this).data('by') == 'desc'){
                    $("#by").val("asc");
                }else if($(this).data('by') == 'asc'){
                    $("#by").val("desc");
                }
            }
            if($(this).hasClass("sales-tag") && $(this).hasClass("active")){
                if($(this).data('by') == 'desc'){
                    $("#by").val("asc");
                }else if($(this).data('by') == 'asc'){
                    $("#by").val("desc");
...
//                getGoods();
        ...
        $(".sort-column > li").click(function() {
            ...
            //处理综合排序项目
            if(!$(this).hasClass("comprehensive-total")){
                $(".comprehensive-total").data('num',0);
                //指示图标箭头向下
                $(".comprehensive-icon").removeClass("fa-angle-up").addClass("fa-angle-down");
                $(".comprehensive-box").hide();
            }
        })
        /*取消筛选 确认*/
        ...
        getGoods();
        //处理综合排序项目
        $(".comprehensive-total").click(function(){
            $(".comprehensive-box").toggle();
            //处理综合排序项目 默认指示图标箭头向下 默认data-num=0
            if($(this).data('num') == 0){
                console.log("num0");
                $(this).data('num',1);
                //指示图标箭头向上
                $(".comprehensive-icon").removeClass("fa-angle-down").addClass("fa-angle-up");
            }else{
                console.log("num1");
                $(this).data('num',0);
                //指示图标箭头向下
