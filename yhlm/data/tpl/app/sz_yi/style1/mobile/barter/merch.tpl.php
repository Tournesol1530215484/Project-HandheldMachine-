...
<title>同城换货</title>
<link href="../addons/sz_yi/template/mobile/style1/barter/css/city1.css" rel="stylesheet" type="text/css">
<link rel="stylesheet" href="../addons/sz_yi/template/mobile/default/member/merch/css/dropload.css">
<link rel="stylesheet" type="text/css" href="../addons/sz_yi/template/mobile/style1/barter/css/swiper.min.css" />
<script src="../addons/sz_yi/template/mobile/default/member/merch/js/dropload.min.js"></script>
<!-- <script src="../addons/sz_yi/template/mobile/style1/barter/js/swiper.min.js"></script> -->
...
<style type="text/css">
	...
	.img-album-container .img-slide{
		width: 105px;
		height: 105px;
		margin-right: 5px;
	}
	.img-album-container .img-slide:last-child{
		margin-right: 0px;
	}
	.goods-link{
		display: inline-block;
	...
	.barter-unit .unit-info-box{
		width: 100%;
		padding-bottom: 10px;
		padding-top: 15px;
		background-color: #fff;
	}
	.barter-unit .unit-info-box .unit-link:before,
	.barter-unit .unit-info-box .unit-link:after{
		content: "";
		display: block;
		clear: both;
	}
	...
	.barter-unit .unit-location{
		width: calc(100% - 80px);
		position: relative;
		margin-bottom: 15px;
	}
	.barter-unit .unit-name{
		width: 72%;
		font-size: 1.4rem;
		color: #000;
		white-space: nowrap;
	...
	.barter-unit .distance,
	.barter-unit .address-box{
		font-size: 1.2rem;
		color: #888585;
		padding-top: 5px;
	    padding-bottom: 5px;
...
<div id="diqu" style="display: none;">
	...
	<div class="city-container">
		...
		<div class="city"><!-- data-id在这里好像并没有用到 -->
			<!-- <div class="city-list search-btn-box">
				<div class="search-cot-box flex-center">
	                <input class="city-cot" type="text" name="keywords" placeholder="输入城市名">
	                <div class="city-post">查找</div>
			...
			<div class="city-list customize-list customize-list2 clearfloat"><span class="city-letter" id="hot_city">热门区域</span>
				...
				<?php  } } ?>	 	 	
				<?php  if(false) { ?>	 		 	 		 	 	 	 
				<p class="special-city" data-id="3">济宁市</p>
				<p class="special-city" data-id="4">衡水市</p>
				<p class="special-city" data-id="5">临汾市</p>
				<p class="special-city" data-id="6">石家庄市</p>
				<p class="special-city" data-id="7">伊犁哈萨克自治州</p>
				<?php  } ?>	 		 		 	 	 
			</div>
...
<script id="tpl_list" type="text/html">
    <%each list as g%>
	<div class='barter-unit'>
		<div class='unit-info-box'>
			<a class='unit-link' href='<%g.shopurl%>'>
				<div class='unit-logo fleft' style='background: url(<%g.img%>) no-repeat center; background-size: cover;'></div>
				<div class='unit-location fright'>
					<div class='unit-name'><%g.merchname%></div>
					<div class='distance'><%g.distance%><%if g.lat != "" && g.lng != ""%>Km<%/if%></div>
					<div class='address-box'><span class='icon'><i class='fa fa-map-marker location-icon'></i></span><%g.address%>
					</div>
				</div>
			</a>
			<div class='swiper-container img-album-container swcon<%g.id%>'>
				<div class='swiper-wrapper'>
					<%each g.album as b%>
					<div class='swiper-slide img-slide' style='background: url(<%b.img%>) no-repeat center;background-size: cover;'>
						<a class='goods-link' href='<%b.url%>'></a>
					</div>
					<%/each%>
				</div>
			</div>
...
<script>
	...
	var loaded = false;
    var stop = true;
	require(['tpl', 'core', 'swiper'], function(tpl, core, jQuery) {
		/*如果cookie中的lng or lat or city 任意一个为空则授权定位并吧通过经纬度获取的城市名称填入html中*/
	    //获取当前城市
...
//                        console.log(lng);
		...
		function getList(page){
            core.json('barter/merch', {op:'ajaxList',page: page, lng: lng, lat: lat, city: city}, function(json) {
                if (json.result.list.length <= 0) {
                    $(".min-a").html(tpl('tpl_null'));
                    return;
                }
                $(".min-a").html(tpl('tpl_list',json.result));
                removeLoading();
				bindEvent(json.result.list);
        ...
        function getmore(page,loaded,stop){
            $(".getmore").unbind("click").click(function(){ 
                ...
                if(stop==true){ 
                    ...
                    page++;
                    console.log("a: "+page);
                    core.json('barter/merch', {op:'ajaxList',page: page, lng: lng, lat: lat, city: city}, function(morejson) {  
                        stop = true;        
                        $('#list_loading').remove();
                        $(".min-a").append(tpl('tpl_list', morejson.result));
                        bindEvent(morejson.result.list);
                        //用下面这种数据结构
