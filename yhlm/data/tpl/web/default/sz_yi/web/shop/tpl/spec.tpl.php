<?php defined('IN_IA') or exit('Access Denied');?> <style type='text/css'>
	...
	<div class="col-sm-9 col-xs-12">
			...
			<?php  if(is_array($spec['items'])) { foreach($spec['items'] as $specitem) { ?>
			     ...
			     <?php (!empty($this) && $this instanceof WeModuleSite || 1) ? (include $this->template('web/shop/tpl/spec_item', TEMPLATE_INCLUDEPATH)) : (include template('web/shop/tpl/spec_item', TEMPLATE_INCLUDEPATH));?>
                          
			<?php  } } ?>
			</div>
		</div>
	</div>   <?php if( ce('shop.goods' ,$item) ) { ?>
          <div class="form-group">
