		...
		<div class="">
			...
			<div class="">
				...
				<div class="tab-content" style="overflow:inherit">
					<div class="tab-pane  active" id="tab_basic" style="margin-top: -35px;"><?php (!empty($this) && $this instanceof WeModuleSite || 1) ? (include $this->template('web/shop/goods/basic', TEMPLATE_INCLUDEPATH)) : (include template('web/shop/goods/basic', TEMPLATE_INCLUDEPATH));?></div>
					...
					<div class="col-lg-2 col-md-6 col-sm-6 col-xs-12">
						<div class='input-group'>
							...
							<select name="status" class='form-control'>
								...
								<option value="0" <?php  if($_GPC['status'] == '0') { ?> selected<?php  } ?>>下架</option></select>
						</div>
					</div>
				...
				<table class="table table-hover">
					...
					<tr>
						...
						<td class='tdedit'>
							...
							<?php  } ?><?php  } ?>
						<td>
							<a style="color:#1e95c9" href="<?php  echo $this->createPluginWebUrl('yipinyima/temp', array('goodsid' => $item['id']))?>">管理</a>
						</td>
						</td>
						<td class='tdedit'>
							<?php  if($item['hasoption']==1) { ?>
						...
						<td>
							...
							<label data='<?php  echo $item['isrecommand'];?>' class='label label-default text-default <?php  if($item['isrecommand']==1) { ?>label-info text-pinfo<?php  } ?>' <?php if(cv('shop.goods.edit')) { ?>onclick="setProperty(this,<?php  echo $item['id'];?>,'recommand')"<?php  } ?>>推荐</label>-<br>
						...
						<td style="position:relative;">
							<a href="javascript:;" data-url="<?php  echo $this->createMobileUrl('shop/detail', array('id' => $item['id']))?>"  title="复制连接" class="btn btn-default btn-sm js-clip" style="background-color: #ffb034;"><i class="fa fa-link" style="color: #fff;"></i></a>
							<?php if(cv('shop.goods.edit|shop.goods.view')) { ?>
							<a href="<?php  echo $this->createWebUrl('shop/goods', array('id' => $item['id'], 'op' => 'post'))?>"class="btn btn-sm btn-default" title="<?php if(cv('shop.goods.edit')) { ?>编辑<?php  } else { ?>查看<?php  } ?>" style="background-color: #ff5858;"><i class="fa fa-pencil" style="color: #fff;"></i></a><?php  } ?>
							<?php if(cv('shop.goods.delete')) { ?>
							<a href="<?php  echo $this->createWebUrl('shop/goods', array('id' => $item['id'], 'op' => 'delete'))?>" onclick="return confirm('确认删除此商品？');return false;" class="btn btn-default  btn-sm" title="删除" style="background-color: #2d2d2d;"><i class="fa fa-times" style="color: #fff;"></i></a><?php  } ?>
						</td>
					</tr>
					<?php  } } ?>
					<tr>
