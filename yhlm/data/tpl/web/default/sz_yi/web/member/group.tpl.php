...
<div class="main"> 
    ...
    <form <?php if( ce('member.group' ,$group) ) { ?>action="" method="post"<?php  } ?> class="form-horizontal form" enctype="multipart/form-data">
        ...
        <div class='panel panel-default'>
            <div class='panel-heading'>会员分组设置</div>
            <div class='panel-body'>
                <div class="form-group">
                	<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
	                    <label class="col-xs-12 col-sm-3 col-md-3 control-label"><span style='color:red'>*</span> 分组名称</label>
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <a class='btn btn-default' href="<?php  echo $this->createWebUrl('member', array('groupid' => $row['id']))?>" style="background-color: #ffb034; color: #fff;"><i class='fa fa-users'></i></a>
                            <?php  if(!empty($row['id'])) { ?>
                            <?php if(cv('member.group.add|member.group.view')) { ?>
                                <a class='btn btn-default' href="<?php  echo $this->createWebUrl('member/group', array('op' => 'post', 'id' => $row['id']))?>" style="background-color: #ff5858; color: #fff;"><i class='fa fa-edit'></i></a>
                            <?php  } ?>
                            <?php if(cv('member.group.delete')) { ?>
                               <a class='btn btn-default'  href="<?php  echo $this->createWebUrl('member/group', array('op' => 'delete', 'id' => $row['id']))?>" onclick="return confirm('确认删除此会员分组吗？');return false;" style="background-color: #000; color: #fff;"><i class='fa fa-remove'></i></a></td>
                            <?php  } ?>
                            <?php  } ?>
