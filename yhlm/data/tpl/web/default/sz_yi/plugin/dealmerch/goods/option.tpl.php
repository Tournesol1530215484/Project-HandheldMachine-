<?php defined('IN_IA') or exit('Access Denied');?><style type="text/css">
    ...
    .table .primary,.table .danger{display:none;}
</style>

<div class="form-group">
	<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
       <label class="col-xs-12 col-sm-3 col-md-3 control-label">是否启用商品规格</label>
        <div class="col-sm-6 col-xs-12">
    	<?php if( ce('dealmerch.goods.edit' ,$item) ) { ?>
		<label class="checkbox-inline"><input type="checkbox" id="hasoption" value="1" name="hasoption" <?php  if($item['hasoption']==1) { ?>checked<?php  } ?> />启用商品规格</label>
    	<?php  } else { ?>
...
<script language="javascript">
	...
	$("#add-spec").html("正在处理...").attr("disabled", "true").toggleClass("btn-primary");
		var url = "<?php  echo $this->createWebUrl('shop/tpl',array('tpl'=>'spec'))?>";
		$.ajax({
			"url": url,
	...
	$("#add-specitem-" + specid).html("正在处理...").attr("disabled", "true");
		var url = "<?php  echo$this->createWebUrl('shop/tpl',array('tpl'=>'specitem'))?>" + "&specid=" + specid;
		$.ajax({
			"url": url,
			success:function(data){
	...
	for(var i=0;i<newlen;i++){
		...
		hh += '<td class="primary"><input name="option_weight_' + ids +'[]" type="text" class="form-control option_weight option_weight_' + ids +'" " value="' +(val.weight=='undefined'?'':val.weight )+'"/></td>';
		hh += "</tr>";
	}
	html+=hh;
	html+="</table>";
