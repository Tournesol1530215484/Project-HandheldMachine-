                     ...
                     <div class="form-group">
                        <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                            ...
                            <div class="col-sm-9 col-xs-12">
                                ...
                                <div class='input-group'>
                                    <input type="text" name="store" maxlength="30" value="<?php  echo $store['title'];?>" id="store" class="form-control" readonly />
                                    <div class='input-group-btn'>
                                ...
                                <?php  } else { ?>
                                   <div class='form-control-static'><?php  if(empty($store['storename'])) { ?>全局兑换点<?php  } else { ?><?php  echo $store['storename'];?><?php  } ?></div>
                                <?php  } ?>
                            </div>
                        </div>
                        <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
...
<script language='javascript'>
    $('form').submit(function () {
        if ($(':input[name=saler]').isEmpty()) {
            Tip.focus($(':input[name=saler]'), '请选择管理员!');
            return false;
        }
        if ($(':input[name=salername]').isEmpty()) {
            Tip.focus($(':input[name=salername]'), '请输入管理员姓名!');
            return false;
        }
        if ($(':input[name=mobile]').isEmpty()) { 
            Tip.focus($(':input[name=mobile]'), '请输入管理员电话!');
            return false;
        }
        return true;
    })
...
<form action="" method="post" onsubmit="return formcheck(this)">
    <div class='panel panel-default'>
        ...
        <div class=''>
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr><td><img src='<?php  echo $row['avatar'];?>' style='width:30px;height:30px;padding1px;border:1px solid #ccc' /> <?php  echo $row['nickname'];?></td>
                        <td><?php  echo $row['salername'];?></td> 
                        <td><?php  if(empty($row['storename'])) { ?>全局兑换点<?php  } else { ?><?php  echo $row['storename'];?><?php  } ?></td>
                        ...
                        <td><?php if(cv('verify.saler.view|verify.saler.edit')) { ?><a class='btn btn-default' href="<?php  echo $this->createPluginWebUrl('dealmerch/dealmerch_adminset', array('op' => 'post', 'id' => $row['id']))?>"><i class='fa fa-edit'></i></a><?php  } ?>
                             <?php if(cv('verify.saler.delete')) { ?><a class='btn btn-default'  href="<?php  echo $this->createPluginWebUrl('dealmerch/dealmerch_adminset', array('op' => 'delete', 'id' => $row['id']))?>" onclick="return confirm('确认删除此管理员吗？');
        ...
        <div class='panel-footer'>
            <a class='btn btn-primary' href="<?php  echo $this->createPluginWebUrl('dealmerch/dealmerch_adminset', array('op' => 'post'))?>"><i class="fa fa-plus"></i> 添加新管理员</a>
        </div>
        <?php  } ?>
    </div>
</form>
...
<script language='javascript'>
    ...
    function search_stores() {
        ...
        $.get('<?php  echo $this->createPluginWebUrl('dealmerch/store',array('op'=>'query'));?>', {
            keyword: $.trim($('#search-kwd1').val())  
        }, function(dat){
            $('#module-menus1').html(dat); 
        });
    }
    function select_saler(o) {
        $("#storeid").val(o.id);
        $("#store").val( o.title );
        $(".close").click();
    }
