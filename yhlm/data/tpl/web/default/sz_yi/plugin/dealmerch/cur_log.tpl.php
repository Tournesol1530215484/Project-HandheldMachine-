...
<div class="panel panel-info">
    ...
    <div class="panel-body">
        <form action="./index.php" method="get" class="form-horizontal" role="form" id="form1">
            ...
            <input type="hidden" name="method" value="cur_log" />
            ...
            <div class="form-group">
             	<div class="col-lg-4 col-md-6 col-sm-6 col-xs-12">
	                <label class="col-xs-12 col-sm-2 col-md-2 col-lg-3 control-label">会员分组</label>
	                <div class="col-sm-8 col-lg-9 col-xs-12">
    ...
    <div class="">
        <table class="table table-hover">
            <thead class="navbar-inner">
                <tr>
                    <th style='width:10%;'><?php  if($_GPC['type']==0) { ?>充值单号<?php  } else { ?>提现单号<?php  } ?></th>
                    <th style='width:10%;'>粉丝</th>
                    <th style='width:20%;'>会员信息</th>
                    <th style='width:10%;'  class='hidden-xs'>微信号</th>
                    <th style='width:10%;' class='hidden-xs'>会员等级</th>
                    <th style='width:10%;'  class='hidden-xs'>会员分组</th>
                    <th style='width:10%;'><?php  if($_GPC['type']==1) { ?>申请提现金额<?php  } else { ?>充值易货额度<?php  } ?></th>
                    <th style='width:12%;'><?php  if($_GPC['type']==1) { ?>申请提现时间<?php  } else { ?>充值时间<?php  } ?></th>
                    <?php  if($_GPC['type']==0) { ?><th style='width:10%;'>充值方式</th><?php  } ?>
            ...
            <tbody>
                ...
                <tr>
                     <td><?php  if(!empty($row['logno'])) { ?>
                         ...
                         recharge<?php  echo $row['id'];?>
                         <?php  } ?></td>
                    <td><a href="<?php  echo $this->createWebUrl('member',array('op'=>'detail','id' => $row['mid']));?>" style="color: #003CB3;"><img src='<?php  echo $row['avatar'];?>' style='width:30px;height:30px;padding1px;border:1px solid #ccc' /> <?php  echo $row['nickname'];?></a></td>
                    <td><?php  echo $row['realname'];?><br/><?php  echo $row['mobile'];?> <br>开户行：<?php  echo $row['bankname'];?><br>银行卡号：<?php  echo $row['bank'];?>  </td>
				    ...
				    <?php  if($_GPC['type']==0) { ?>
				    <td><?php  if($row['rechargetype']=='alipay') { ?>
                    ...
                    <td><?php if(cv('member.member.view')) { ?>
                        <a class='btn btn-default' href="<?php  echo $this->createWebUrl('member',array('op'=>'detail','id' => $row['mid']));?>" style="background-color: #7DBAEF; color: #fff;">用户信息</a>
                        <?php  } ?>
                        <?php  if($row['type']==0 && $row['status']==1) { ?>
                          <?php  if($row['rechargetype']=='alipay' || $row['rechargetype']=='wechat') { ?>
                             <?php if(cv('finance.recharge.refund')) { ?>
                                      <a class='btn btn-danger' onclick="return confirm('确认退款到微信钱包?')" href="<?php  echo $this->createPluginWebUrl('dealmerchlog',array('op'=>'pay','paytype'=>'refund','id' => $row['id']));?>">退款</a>		         
                             <?php  } ?>
                          <?php  } ?>
                        <?php  } ?>
                        <?php  if($row['type']==1 && $row['status']==0) { ?>
                        <?php if(cv('finance.withdraw.withdraw')) { ?>
	                        <a class='btn btn-default' onclick="return confirm('确认微信钱包提现?')" href="<?php  echo $this->createPluginWebUrl('dealmerchlog',array('op'=>'pay','paytype'=>'wechat','id' => $row['id']));?>" style="background-color: #ffb034; color:#fff;">微信提现</a><br />	
	                        <a class='btn btn-default' onclick="return confirm('确认手动提现完成?')" href="<?php  echo $this->createPluginWebUrl('dealmerchlog',array('op'=>'pay','paytype'=>'manual','id' => $row['id']));?>" style=" margin-top: 6px; background-color: #ff5858; color:#fff;">手动提现</a><br />
	                        <a class='btn btn-default' onclick="return confirm('确认拒绝提现申请?')" href="<?php  echo $this->createPluginWebUrl('dealmerchlog',array('op'=>'pay','paytype'=>'refuse','id' => $row['id']));?>" style=" margin-top: 6px; background-color: #000; color:#fff;">拒绝</a><br />
                        <?php  } ?>
                        <?php  } ?>
                    </td>
                </tr>
