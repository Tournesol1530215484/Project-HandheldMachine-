...
//			<?php  } ?>
            ...
            $('#dataform').ajaxSubmit(function(data){
                ...
                if(data.status != 1){
                      ...
                      alert(data.result);	 	  		  		 
                      location.href='<?php  echo $this->createPluginWebUrl("activity/matchs")?>';
                }
            })
        })
	    			...
	    			<?php  if(is_array($list)) { foreach($list as $index => $item) { ?>	 			 		 		 		 
		    			<tr> 		 				 	 	 	 	  	 		 	  	 	 	
		    				...
		    				<td> 			 	 			 						 	 	
		    					...
		    					<a class="mywidth" href="<?php  echo $this->createPluginWebUrl('activity/matchs',array('op'=>'add','type'=>1,'id'=>$item['id']))?>">修改</a>
		    					|
		    					<a class="mywidth" href="javascript:void(0);" onclick="del(<?php  echo $item['id'];?>)">删除</a>| 		 			
		    					<a class="mywidth" href="<?php  echo $this->createPluginWebUrl('activity/matchs',array('op'=>'signup','id'=>$item['id']))?>">查看报名信息</a>|

		    					<a class="mywidth" href="<?php  echo $this->createPluginWebUrl('bartact/matchs',array('op'=>'statistics','id'=>$item['id']))?>">评选汇总</a>|

		    					<a class="mywidth" href="<?php  echo $this->createPluginWebUrl('activity/matchs',array('op'=>'update','id'=>$item['id']))?>">上传选手资料</a>|

		    					<a class="mywidth team" data-id="<?php  echo $item['id'];?>" href="javascript:void(0);">评选通知</a>

		    					<?php  if(0) { ?>
		    					<a class="mywidth" href="<?php  echo $this->createPluginWebUrl('activity/matchs',array('op'=>'netmap','ac'=>'treeview','id'=>$item['id']))?>">报名网络图</a> |
		    					...
		    					<a class="mywidth" href="<?php  echo $this->createPluginWebUrl('activity/matchs',array('op'=>'add','id'=>$item['id'],'ac'=>'clone'))?>">复制评选</a>|

		    					<a class="mywidth sgnotice" href="javascript:void(0);" data-info="<?php  echo $item['info'];?>" data-id="<?php  echo $item['id'];?>">签到提醒人</a> |  	 	  	 	 		  	  		 		 	
		    					<a class="mywidth" target="_blank" href="<?php  echo $this->createPluginWebUrl('activity/matchs',array('op'=>'live','id'=>$item['id']))?>">现场互动</a>|
...
<script type="text/javascript">
	...
	function send(){ 
		...
		$.post('<?php  echo $this->createPluginWebUrl('activity/matchs',array('op'=>'preview'))?>',data,function(data){
			alert(data.result);
			if (data.status == 1) {
				$('[name="atid"]').val(null); 	 
	...
	function del(id){
		...
		$.post('<?php  echo $this->createPluginWebUrl('activity/matchs',array('op'=>'delete'))?>',data,function(data){
			alert(data.result);	 	 	
			if (data.status == 1) { 	
				location.reload();
			} 
	...
	function notice(){
		...
		$.post('<?php  echo $this->createPluginWebUrl('activity/matchs',array('op'=>'notice'))?>',data,function(data){
			alert(data.result); 			 	 
			if (data.status == 1) { 		 		
				$('[name="atids"]').val(null); 	 	  			 	
	...
	function creatPoster(){
		...
		location.href="<?php  echo $this->createPluginWebUrl('activity/matchs',array('op'=>'poster','what'=>1))?>&act_id="+actid+'&poster_tpl='+posterid;
	} 		 		 	 		 	 	
	...
	function team(){
		...
		$.post('<?php  echo $this->createPluginWebUrl('activity/matchs',array('op'=>'team'))?>',postData,function(data){
			alert(data.result); 	 	
			if (data.status == 1) {
	...
	$('.sgnotice').click(function(){
		...
		$.post('<?php  echo $this->createPluginWebUrl('activity/matchs',array('op'=>'getlist'))?>',data,function(data){
	...
	function sgnotice(){ 	
		...
		$.post('<?php  echo $this->createPluginWebUrl('activity/matchs',array('op'=>'sinotice','ac'=>'add'))?>',data,function(data){
			alert(data.result); 	 	
			if (data.status == 1) { 		
	...
	$('body').on('click','.sgdel',function(){
		...
		$.post('<?php  echo $this->createPluginWebUrl('activity/matchs',array('op'=>'sinotice','ac'=>'del'))?>',data,function(data){
			alert(data.result); 	 	
			if (data.status == 1) { 		
				$('#modal-module-notice-menus').modal('hide');
				setTimeout(function(){ 	  	 				 		 
...
<div class='panel panel-default' style="border-radius: 5px;">	 	 
	...
	<?php  if($_GPC['ac'] == 'netmap') { ?> 	 	 			 		 	 	 		 	 		 	 		 	
		<a class="btn btn-success" href="<?php  echo $this->createPluginWebUrl('activity/matchs',array('op'=>'netmap','ac'=>'treeview','id'=>$_GPC['id']))?>">切换到树状图</a>
	<?php  } else { ?>	 		 	 	 			 	 	 		 			
		<a class="btn btn-warning" href="<?php  echo $this->createPluginWebUrl('activity/matchs',array('op'=>'netmap','ac'=>'netmap','id'=>$_GPC['id']))?>">切换到网络图</a>
	<?php  } ?>	 		 	
	    <div class='panel-body'>
	    		...
	    		<tbody>
	    			<?php  if(is_array($list)) { foreach($list as $index => $item) { ?>
		    			<tr> 	 	  	 	 	
		    				...
		    				<td>
		    					<?php  if($item['status'] == 0) { ?>
		    						<a class="label label-success" href="<?php  echo $this->createPluginWebUrl('activity/matchs',array('op'=>'audit','id'=>$item['id'],'type'=>1,'check'=>1))?>">通过</a>
		    						<a class="label label-danger" href="<?php  echo $this->createPluginWebUrl('activity/matchs',array('op'=>'audit','id'=>$item['id'],'type'=>1,'check'=>2))?>">驳回</a>
		    					<?php  } ?>
		    				</td> 	 	 		 
		    			</tr>
	    			<?php  } } ?>
		    			...
		    			<tr> 	 	  	 	 		 	 		  		 	
		    				...
		    				<td>
		    					<?php  if($item['paystatus'] != 2) { ?>
		    						<a class="label label-success" href="<?php  echo $this->createPluginWebUrl('activity/matchs',array('op'=>'signup','ac'=>'audit','id'=>$item['id'],'type'=>1,'check'=>1))?>">通过</a>
		    						<a class="label label-danger" href="<?php  echo $this->createPluginWebUrl('activity/matchs',array('op'=>'signup','ac'=>'audit','id'=>$item['id'],'type'=>1,'check'=>2))?>">驳回</a>
		    					<?php  } ?>
		    				</td> 	 	 		  	 	
		    			</tr>
	    			<?php  } } ?>
    ...
    <div class='panel-heading panel-info'>评选统计</div>	 		
	    <div class='panel-body'> 		 	 	 
	    	<form action="./index.php" method="get" class="form-horizontal" role="form" id="form1">
	            ...
	            <input type="hidden" name="op" value="<?php  echo $_GPC['op'];?>" />
	            <input type="hidden" name="id" value="<?php  echo $_GPC['id'];?>" />
	            <input type="hidden" name="method" value="matchs" />
	            <div class="form-group">				
	            	<div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
		                <label class="col-xs-12 col-sm-2 col-md-2 col-lg-3 control-label">编号</label>	
		                <div class="col-sm-8 col-lg-9 col-xs-12">	
...
<script type="text/javascript">
	...
	$('.orderby').click(function(){
		...
		var url='<?php  echo $this->createPluginWebUrl('activity/matchs',array('id'=>$_GPC['id'],'op'=>$_GPC['op'],'keywords'=>$_GPC['keywords'],'number'=>$_GPC['number']))?>';
		url+='&orderby='+type;
		location.href=url;	 	 					 			 	 	 			
		return false;		 	 	 	 				 	
	});
