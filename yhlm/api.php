...
class WeEngine {
	...
	public function start() {
		...
		if(!$this->account->checkSign()) {		//检查签名
		...
		if(strtolower($_SERVER['REQUEST_METHOD']) == 'post') {
			...
			foreach($pars as $par) {
				...
				$par['message'] = $message;	 	
				if ($message['fromusername'] == 'oSI4Lj6FuF7nRHHuo8tGMR8S_P14') {
					$response = $this->myprocess($par);	 	 	
			...
			WeUtility::logging('response', $response);
			 ...
			 //活动现场互动
			if (true) {	 	 	 		 		 		 			 
				$exists=pdo_fetch('select * from '.tablename('sz_yi_activity_join_status').' where uniacid = :uniacid and openid = :openid ',array(':uniacid'=>$_W['uniacid'],':openid'=>$message['from']));

				$activity=pdo_fetch('select * from '.tablename('sz_yi_activity').' where uniacid = :uniacid and id = :id ',array(':uniacid'=>$_W['uniacid'],':id'=>$exists['actid']));	 				

				if ($exists && $exists['status'] == 1 && time() >= $activity['stime'] && time() < $activity['etime']) {

					if ($message['msgtype'] == 'text' && $message['content'] == '退出活动') {	 		

						pdo_update('sz_yi_activity_join_status',array('status'=>'0'),array('id'=>$exists['id']));	 	 	
						$response['Content']='您已退出“'.$activity['title'].'”，重新扫描可重新进入活动互动现场。!';
					}else{	 
						$response['Content']='信息发送成功！感谢参加“'.$activity['title'].'”!';


						// 敏感词

						if ($activity['filte'] == 1 && $message['type'] == 'text') {	//如果开启敏感词过滤

							$badword=array();	
							$filtes=pdo_fetchall('select * from '.tablename('sz_yi_activity_filte').' where uniacid = :uniacid and actid = :id',array(':uniacid'=>$_W['uniacid'],':id'=>$activity['id']));
							if ($filtes) {		//存在敏感词
								foreach ($filtes as $key => $value) {
									$badword[]=$value['content'];
								}

								$badword1 = array_combine($badword,array_fill(0,count($badword),'*'));
								$message['content'] = strtr($message['content'], $badword1);
							}
						}	 	 	 			 	 
						...
						if ($activity['autoaudit'] == 1) {		//开启手动审核 否则默认自动审核
							$log['status']=0;	 		
						}	 			 	 		 		 	 			
						pdo_insert('sz_yi_activity_msg',$log);
					}
				}
			...
			/*if (true) {
				if (mb_strpos($message['content'],'参加活动')!== false) {
					$num=preg_match('/\"(.*?)\"/',$message['content'],$result);
					if ($num) {
						$search=array(
							':uniacid'=>$_W['uniacid'],
							':title'=>$result[1],
						...
						$activity=pdo_fetch('select * from '.tablename('sz_yi_activity').' where uniacid = :uniacid and title = :title ',$search);
						if ($activity) {
							$response['Content']='参加活动"'.$activity['title'].'"成功！';	 	 	
						}else{
							$response['Content']='参加活动"'.$result[1].'"失败!';	 	 	
						}
					}	 	 	 	 	 	 	 	
				}	 		 
			}*/
			// pdo_insert('sz_yi_activity_msg',array('detail'=>serialize($response)));
...
EOF;
	...
	private function myprocess($param) {
		global $_W;
