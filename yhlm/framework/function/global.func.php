...
function pagination1($total, $pageIndex, $pageSize = 15, $url = '', $context = array('before' => 5, 'after' => 4, 'ajaxcallback' => '')) {
	global $_W;
	$pdata = array(
		'tcount' => 0,
		'tpage' => 0,
...
function tomedia($src, $local_path = false){
	...
	if ($local_path || empty($_W['setting']['remote']['type']) || file_exists(IA_ROOT . '/' . $_W['config']['upload']['attachdir'] . '/' . $src)) {
		...
		if (strexists($src,'//attachment')) {			//去掉重复//attchment
			$src=str_replace('//attachment','',$src);
		}
	} else {
