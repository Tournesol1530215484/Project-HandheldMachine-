...
class Captcha {
	...
	public function build2($width, $height,$str) {
		$image = @imagecreatetruecolor($width, $height);
		if(empty($image)) {
			return error('1', 'Not supplied GD');
		}
		$bg = imagecolorallocate($image, 255,250,240);
		imagefill($image, 0, 0, $bg);
		...
		$image = $this->distort2($image, $width, $height, $bg);
		$this->image = $image;
		return $this;
	}	 
	...
	protected function drawLine($image, $width, $height, $tcol = null) {
		if ($tcol === null) {	  
			$tcol = imagecolorallocate($image,240,140,50);
	...
	protected function writePhrase($image, $phrase, $font, $width, $height) {
		$size = $width / mb_strlen($phrase,'utf8') - $this->rand(0, 3) - 15;
		$box = imagettfbbox($size, 0, $font, $phrase);
		$textWidth = $box[2] - $box[0];
		$textHeight = $box[1] - $box[7];
		$x = ($width - $textWidth) / 2;
		$y = ($height - $textHeight) / 2 + $size;
		// var_dump(mb_strlen($phrase,'utf8'));
		$textColor = array($this->rand(0, 150), $this->rand(0, 150), $this->rand(0, 150));
		$col = imagecolorallocate($image,245,145,50);
		// $col = imagecolorallocate($image, $textColor[0], $textColor[1], $textColor[2]);
		$length = mb_strlen($phrase,'utf8');
		for ($i=0; $i<$length; $i++) { 		 		  	
			// $box = imagettfbbox($size, 0, $font, mb_substr($phrase,$i,1,'utf8'));
			// $box = imagettfbbox($size, 0, $font, $phrase); old
			$w = $box[2] / $length; 
			// $w = $box[2] - $box[0]; old
			// var_dump($size);
			// var_dump(mb_substr($phrase,$i,1,'utf8'));
			$angle = $this->rand(-$this->maxAngle, $this->maxAngle) - 10;
			$offset = $this->rand(-$this->maxOffset, $this->maxOffset);
			imagettftext($image, $size, $angle, $x, $y + $offset, $col, $font, mb_substr($phrase,$i,1,'utf8'));
	...
	protected function writePhrase2($image, $phrase, $font, $width, $height) {
		$size = $width / mb_strlen($phrase,'utf8') - $this->rand(0, 0) - 50;
		$box = imagettfbbox($size, 0, $font, $phrase);	 	 
		$textWidth = $box[2] - $box[0];
		$textHeight = $box[1] - $box[7];
		$x = ($width - $textWidth) / 2;
	...
	public function distort2($image, $width, $height, $bg) {
		$contents = imagecreatetruecolor($width, $height);
		$X = $this->rand(0, 0);
		$Y = $this->rand(0, 0);	 			 	 	 	 
		$phase = $this->rand(0, 0);
	...
	protected function font() {
		return IA_ROOT . '/web/resource/fonts/msyh.ttf';
	}
}
?>