...
if ($operation == "display") {
	...
	if (!empty($_GPC["agentlevel"])) {
		$condition .= " and dm.bonuspluslevel=" . intval($_GPC["agentlevel"]);
	}
	if ($_GPC["status"] != '') {
	...
	$sql = 'select dm.*,dm.nickname,dm.avatar,l.levelname,p.nickname as parentname,p.avatar as parentavatar from ' . tablename("sz_yi_member") . " dm " . " left join " . tablename("sz_yi_member") . " p on p.id = dm.agentid " . " left join " . tablename("sz_yi_bonusplus_level") . " l on l.id = dm.bonuspluslevel" . " left join " . tablename("mc_mapping_fans") . "f on f.openid=dm.openid and f.uniacid={$_W["uniacid"]}" . " where dm.uniacid = " . $_W["uniacid"] . " and dm.isagent =1  {$condition} ORDER BY dm.agenttime desc";
	...
	foreach ($list as &$row) {
		...
		$row["commission_ok"] = $info["commission_ok"];
	...
	if ($operation == "detail") {
		...
		if (checksubmit("submit")) {
			ca("bonusplus.agent.edit|bonusplus.agent.check|bonusplus.agent.agentblack");
			...
			if (empty($_GPC["oldstatus"]) && $data["status"] == 1) {
				...
				plog("bonusplus.agent.check", "审核分销商 <br/>分销商信息:  ID: {$member["id"]} /  {$member["openid"]}/{$member["nickname"]}/{$member["realname"]}/{$member["mobile"]}");
			}
			...
			plog("bonusplus.agent.edit", "修改分销商 <br/>分销商信息:  ID: {$member["id"]} /  {$member["openid"]}/{$member["nickname"]}/{$member["realname"]}/{$member["mobile"]}");
			if (!empty($data["bonuspluslevel"])) {
				$data["bonusplus_status"] = 1;
			}
			...
			message("保存成功!", $this->createPluginWebUrl("bonusplus/agent"), "success");
		}
		$diyform_flag = 0;
