...
$sql       = 'select distinct cg.mid from ' . tablename('sz_yi_bonusplus_goods') . ' cg left join  ' . tablename('sz_yi_order') . '  o on o.id=cg.orderid and cg.status=0 left join ' . tablename('sz_yi_order_refund') . " r on r.orderid=o.id and ifnull(r.status,-1)<>-1 where 1 and o.status>=3 and o.uniacid={$_W['uniacid']} and ({$time} - o.finishtime > {$day_times})  ORDER BY o.finishtime DESC,o.status DESC";
...
foreach ($list as $key => &$row) {
	...
	$member = $this->model->getInfo($row['mid'], array(
		'ok',
		'pay',
		'myorder'
	));
	...
	if ($member['myordermoney'] < $set['consume_withdraw']) {
		...
		if ($member['commission_ok'] <= 0) {
			...
			if (!empty($member['bonuspluslevel'])) {
				$row['realname'] = pdo_fetchcolumn('select levelname from ' . tablename('sz_yi_bonusplus_level') . ' where id=' . $member['bonuspluslevel']);
			} else {
...
if (!empty($_POST)) {
	...
	foreach ($count as $key => $value) {
		$member     = $this->model->getInfo($value['mid'], array(
			'ok',
			'pay'
		));
		...
		pdo_update('sz_yi_bonusplus_goods', $bonusplus_goods, array(
			'mid' => $member['id'],
			'uniacid' => $_W['uniacid']
		));
	}
	...
	message('代理商分红发放成功', $this->createPluginWebUrl('bonusplus/detail', array(
