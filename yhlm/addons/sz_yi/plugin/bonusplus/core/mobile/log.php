...
if ($_W['isajax']) {
	if ($operation == 'display') {
		...
		$commissioncount = 0;
		$list            = pdo_fetchall('select * from ' . tablename('sz_yi_bonusplus_log') . " where 1 {$condition} order by id asc LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
		$total           = pdo_fetchcolumn('select count(*) from ' . tablename('sz_yi_bonusplus_log') . " where 1 {$condition}", $params);
		...
		foreach ($list as &$row) {
			...
			if(empty($row['sendpaytime']))
			{
				$curhour = intval(date('H', $row['paytime']));
				if($curhour >= intval($zhengdian))
				{
					$plansendpaytime = strtotime(date("Y-m-d ".$zhengdian.":00:00",$row['paytime'])) + 3600 * 24;
					$row['plansendpaytime']	= date('Y-m-d H:i', $plansendpaytime);			
				}
				else
				{
					$plansendpaytime = strtotime(date("Y-m-d ".$zhengdian.":00:00",$row['paytime']));
					$row['plansendpaytime']	= date('Y-m-d H:i', $plansendpaytime);					
				}
			}
		}
		...
		$id    = intval($_GPC['id']);
		$apply = pdo_fetch('select * from ' . tablename('sz_yi_bonusplus_log') . ' where id=:id and `mid`=:mid and uniacid=:uniacid limit 1', array(
			':id' => $id,
			':mid' => $mid,
			':uniacid' => $uniacid
		));
		...
		$pindex = max(1, intval($_GPC['page']));
		$psize  = 5;
		$apply  = pdo_fetch('select orderids from ' . tablename('sz_yi_commission_apply') . ' where id=:id and `mid`=:mid and uniacid=:uniacid limit 1', array(
			':id' => $id,
			':mid' => $mid,
			':uniacid' => $uniacid
		));
