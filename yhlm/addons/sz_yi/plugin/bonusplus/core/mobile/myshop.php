...
if ($op == 'display') {
	if ($_W['isajax']) {
		...
		$advs        = pdo_fetchall('select id,advname,link,thumb from ' . tablename('sz_yi_adv') . ' where uniacid=:uniacid and enabled=1 order by displayorder desc', array(
			':uniacid' => $uniacid
		));
	...
	if ($_W['isajax']) {
		if ($_W['ispost']) {
			...
			if (empty($shop['id'])) {
				...
				pdo_update('sz_yi_commission_shop', $shopdata, array(
					'id' => $shop['id']
				));
			}
			show_json(1);
	...
	if ($_W['isajax']) {
		...
		if (!empty($shop['selectgoods'])) {
			$goodsids = explode(',', $shop['goodsids']);
			if (!empty($goodsids)) {
				$goods = pdo_fetchall('select id,title,marketprice,thumb from ' . tablename('sz_yi_goods') . ' where uniacid=:uniacid and id in ( ' . trim($shop['goodsids']) . ')', array(
					':uniacid' => $_W['uniacid']
				));
