...
define('TM_match_PAY', 'match_pay');
if (!class_exists('matchModel')){
    class match123Model extends PluginModel{
        public $parentAgents = "";
       ...
       function perms(){
            return array(                       
				'match' => array(
						...
						'isplugin' => true,  
						'child' => array(
							'match' => array('text' => '比赛管理'),
                            'match.add' => array('text' => '我发布的比赛'),
                            'picture' => array('text' => '图片管理'),
                            'picture.add' => array('text' => '我发布的图片'),
                        ), 
				) 
			); 
        } 
        ...
        public function sendmatchInform($dephp_64 = '', $dephp_69 = ''){
        ...
        public function order_split($dephp_74){
            ...
            $dephp_75 = pdo_fetchall('select distinct match_uid from ' . tablename('sz_yi_order_goods') . ' where orderid=:orderid and uniacid=:uniacid', array(':orderid' => $dephp_74, ':uniacid' => $_W['uniacid']));
            if(count($dephp_75) == 1){
                pdo_update('sz_yi_order', array('match_uid' => $dephp_75[0]['match_uid']), array('id' => $dephp_74, 'uniacid' => $_W['uniacid']));
                return;
            }
            $dephp_76 = pdo_fetchall('select match_uid, id from ' . tablename('sz_yi_order_goods') . ' where orderid=:orderid and uniacid=:uniacid ', array(':orderid' => $dephp_74, ':uniacid' => $_W['uniacid']));
            ...
            foreach ($dephp_76 as $dephp_80 => $dephp_81){
                $dephp_79[$dephp_81['match_uid']][]['id'] = $dephp_81['id'];
            }
            $dephp_82 = false;
            unset($dephp_77['id']);
            ...
            if(!empty($dephp_79)){
                foreach ($dephp_79 as $dephp_80 => $dephp_81){
                    ...
                    foreach($dephp_81 as $dephp_96){
                        $dephp_70 = pdo_fetch('select price,realprice,oldprice,match_uid from ' . tablename('sz_yi_order_goods') . ' where id=:id and uniacid=:uniacid ', array(':id' => $dephp_96['id'], ':uniacid' => $_W['uniacid']));
