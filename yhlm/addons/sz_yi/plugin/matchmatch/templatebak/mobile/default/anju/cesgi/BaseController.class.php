...
//		}
	...
	protected  function dandain(){
		$mobile=$this->_post['mobile'];
		$this->log('mobile----dandian'.$mobile);
		if(empty($mobile)){
			return false;
		}
		//$mobile=$this->_post['mobile'];
		//存入数据库
		$session_id=M('ThinkSession')->where("session_expire='$mobile'")->field('session_id')->find();
		if(!empty($session_id)){
			$sessionid=$session_id('appUser');
			if($sessionid==$session_id){
				$this->log('判断相等1111'.$sessionid);
				return true;
			}else{
				$this->log('判断相等'.$sessionid);
				return false;
			}

		}else{
			session('appUser',$mobile);
			$ss=session_id('appUser');
			$this->log('session_id----login2'.$ss);
			$data['session_id']=$ss;
			$data['session_expire']=$mobile;
			$userList = M('ThinkSession')->add($data);
			if(!$userList){
				$this->log('数据库写入'.$data['session_expire']);
				return false;
			}else{
				return true;
			}
		...
		if(empty($regist_attr['attr12'])){
			...
			if(strtotime($fk_time)<=strtotime($date)){
				if($trade_info['ins_type']=="一年"){
					$sve['attr12']=date("Y-m-d", strtotime("+12 month", strtotime($date)));//到期时间
					$sve['attr11']=12;
				}else if($trade_info['ins_type']=="二年"){
					$sve['attr12']=date("Y-m-d", strtotime("+24 month", strtotime($date)));//到期时间
					$sve['attr11']=24;
				}else if($trade_info['ins_type']=="三年"){
					$sve['attr12']=date("Y-m-d", strtotime("+36 month", strtotime($date)));//到期时间
					$sve['attr11']=36;
				}
			}
				...
				if($trade_info['ins_type']=="一年"){
					...
					$b="+" . $attr11;
					$sve['attr12']=date("Y-m-d", strtotime("$b month", strtotime($create_date)));//到期时间
		...
		if($flag){
			...
			if($trade){
				...
				if(empty($regist_attr['attr12'])){
						  ...
						  // $create_date = $regist_attr['fk_date'];//创建时间
					//日志
					$create_date =date("Y-m-d H:i:s",strtotime('+7 day',$regist_attr['fk_date']));//创建时间===在这里加7天
					$this->log("创建时间了：".$create_date);
						   $fk_time = date("Y-m-d", strtotime("$a month", strtotime($create_date)));//到期时间
	...
	public  function  notify2(){
		...
		if($flag){
			...
			if($trade){
				...
				if(empty($regist_attr['attr12'])){
					$trade_a=date("Y-m-d H:i:s",strtotime('+7 day',$trade_info['gmt_payment']));
					$term_b=date('Y-m-d',$trade_a);
					$term= $term_b."--".$sve['attr12'];
				}else{
					$term=$regist_attr['attr12']."--". $sve['attr12'];
...
protected function addPush_tag($tag,$Android_info,$Android_title,$Android_value,$ios_alert,$ios_value,$ios_sound){
     ...
     */
    public function return_msg($code,$msg='',$date=[]){
        header("Access-Control-Allow-Origin:*");
        header('Access-Control-Allow-Methods:POST');
        header('Access-Control-Allow-Headers:x-requested-with, content-type');
        $return_msg['code']=$code;
        $return_msg['msg']=$msg;
        $return_msg['date']=$date;
