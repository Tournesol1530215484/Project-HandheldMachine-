...
if (!$muser) {
	// m('tools')->tips('你还不是图片商家，请先成为图片商家',$this->createPluginMobileUrl('supplier/af_supplier',array('merch'=>6)));	 		
}
...
if ($op == 'display') {
	...
	$reward=pdo_fetchall('select m.avatar,m.mobile,m.nickname,ar.money,ar.remark from '.tablename('sz_yi_match_reward').' ar left join '.tablename('sz_yi_member').' m on m.openid = ar.openid where ar.uniacid = :uniacid and ar.atid = :id and ar.type = 4 order by ar.atid desc limit 0,3 ',array(':uniacid'=>$_W['uniacid'],':id'=>$id)); 	
	// var_dump($reware);
	...
	$tmu=m('match')->getMuser($picture['openid']);
	if ($tmu) {
