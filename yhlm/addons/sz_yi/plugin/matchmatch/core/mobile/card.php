...
if($op == 'display'){	 	 	
	...
	// }
		...
		if (!$tuser) {	 	 	 	 	 		 	 	 	 	 
			$tmuser=m('match')->getMuser($tmember['openid']); 			 	
	...
	$msg=pdo_fetchall('select m.id tid,l.ctime,l.id,m.avatar,l.content,l.ctime,mu.realname from '.tablename('sz_yi_match_leave_msg').' l left join '.tablename('sz_yi_member').' m on m.openid = l.openid left join '.tablename('sz_yi_visiting_info').' mu on mu.openid = m.openid where l.uniacid = :uniacid and l.openid = :openid order by l.id desc limit 0,5',array(':uniacid'=>$_W['uniacid'],':openid'=>$tuser['openid']));
	foreach ($msg as $key => $value) {	  	 	 
		$msg[$key]['link']=$this->createPluginMobileUrl('match/card',array('tid'=>$value['tid']));
	...
	/*$tmember=m('member')->getMember($tid);
	$tinfo=m('match')->getVisitingInfo($tmember['openid']);
	...
	pdo_insert('sz_yi_match_leave_msg',$data);
	...
	$re=pdo_delete('sz_yi_match_leave_msg',array('id'=>$msgid));
	...
	/*$tid=$_GPC['tid'];
	$tmember=m('member')->getMember($tid);
	$tuser=m('match')->getVisitingInfo($tmember['openid']);
	if (!$tuser) {
	...
	if ($ac == 'get') {
		...
		foreach ($list as $key => $value) {	 	
			...
			$list[$key]['url']=$this->createPluginMobileUrl('match/card',array('op'=>'poster','posterid'=>$value['id'],'tid'=>$tid)); 	 
		}	 	 	 	 	 	  
    ...
    $pt=m('tools')->createCardPoster($minfo['openid'],$posterid,$this->createPluginMobileUrl('match/card',array('tid'=>$tid)));  
