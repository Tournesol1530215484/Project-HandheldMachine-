...
if ($op == 'display') {
	...
	if ($ac == 'sub') {	  	 
        ...
        if ($_GPC['status'] == 1) {				//修改基本信息
	 		$data=array(
            	...
            	'ContactOrg'=>$muser['realname'],      
            	'mobileOrg'=>$muser['mobile'],  
            	'descOrg'=>$muser['orgDesc'],		 	
		...
		if (empty($id)) {
        	...
        	if ($data['cost'] == 1) {
        		$payitem['atid']=$id;
        		pdo_insert('sz_yi_match_payitem',$payitem);
        	}
        	if ($id) {	 	 		 	 	 	
        		show_json(1);	 	 	 	 
        	}
        	...
        	if ($data['cost'] == 1) {
        		$py=pdo_fetch('select * from '.tablename('sz_yi_match_payitem').' where uniacid = :uniacid and atid = :id and ismobile = 1 and enabled = 1',array('uniacid'=>$_W['uniacid'],':id'=>$id));
        		if ($py) {	 	 	 	 	 	 	 
	        		pdo_update('sz_yi_match_payitem',$payitem,array('id'=>$py['id'],'uniacid'=>$_W['uniacid']));
	        	}else{	 		 	 	 	 	 		 	 		 	 
	        		$payitem['atid']=$id;
	        		pdo_insert('sz_yi_match_payitem',$payitem);
	        	}
	...
	include $this->template('matNotice');	//发活动前的提示页面
	exit;
}else if($op == 'mymatch'){
	...
	/*if ($_W['isajax']) {
		if ($ac == 'get') {
			...
			if ($type == '1') {
				$sql="select a.* from ".tablename('sz_yi_match').' a left join '.tablename('sz_yi_match_signup').' s on a.id=s.actid where s.uniacid = :uniacid and s.openid = :openid and s.deleted = 0';
			...
			if ($list) {	 	
				foreach ($list as $key => $value) {
					...
					$list[$key]['url']=$this->createPluginMobileUrl('match/match',array('op'=>'adminAct','id'=>$value['id'],'type'=>$type));	 	 	 	 	 	 
				}
	...
	if ($_W['isajax']) {
		if ($ac == 'sub') {
			...
			if (!empty($mid)) {
				...
				$pid=pdo_fetchcolumn('select id from '.tablename('sz_yi_match_signup').' where uniacid = :uniacid and openid = :openid and actid = :id',array(':uniacid'=>$_W['uniacid'],':openid'=>$mmember['openid'],':id'=>$_GPC['id']));	 	
			 		$data['fmid']=$mid;	 	
			...
			if ($data['liek'] == 1) {		//关注
				$tmuser=m('match')->getMuser($tact['openid']);
				$tre=pdo_fetch('select * from '.tablename('sz_yi_match_favorite').' where uniacid = :uniacid and openid = :openid and merchid = :uid',array(':uniacid'=>$_W['uniacid'],':openid'=>$openid,':uid'=>$tmuser['uid']));
				if ($tre) {
					if (!empty($tre['deleted'])) {
						pdo_update('sz_yi_match_favorite',array('deleted'=>0),array('id'=>$tre['id'],'uniacid'=>$_W['uniacid']));
					}
				}else{
					$tfadata=array(
						'uniacid'=>$_W['uniacid'],
					...
					pdo_insert('sz_yi_match_favorite',$tfadata);
				}
			}

			if ($_GPC['fee']) {
				$pt=pdo_fetch('select * from '.tablename('sz_yi_match_payitem').' where uniacid = :uniacid and id = :id and enabled = 1',array(':uniacid'=>$_W['uniacid'],':id'=>$_GPC['fee']));		
				...
				if ($act['teamModel'] == 1) {		
					...
					if ($member['agentid']) {
						...
						if ($agent) {
							...
							$fw1=pdo_fetch('select * from '.tablename('sz_yi_match_share').' where uniacid = :uniacid and openid = :openid and type = 1 and actid = :id',$params);
							$tb=pdo_fetch('select * from '.tablename('sz_yi_match_team_bonus').' where uniacid = :uniacid and mid = :mid and openid = :openid and level = 1 ',array(':uniacid'=>$_W['uniacid'],':mid'=>$agent['id']));
							if (empty($tb)) {	 	
								$bonus1=array(
									'uniacid'=>$_W['uniacid'],	 	
									'mid'=>$_W['uniacid'],	 	
								...
								pdo_insert('sz_yi_match_team_bonus',$bonus1);
						...
						if ($agent['agentid']) {					// 2级
							...
							if ($agent) {	 		 	
								...
								$fw2=pdo_fetch('select * from '.tablename('sz_yi_match_share').' where uniacid = :uniacid and openid = :openid and type = 1 and actid = :id and level = 2',$params);
								...
								if (empty($tb)) {	 	
									...
									pdo_insert('sz_yi_match_team_bonus',$bonus2);	 
			...
			if ($pt['limit']!= -1) {	 	 //报名成功后人数-1	 	 	 	 		 
				pdo_update('sz_yi_match_payitem',array('limit'=>$pt['limit']-1),array('id'=>$pt['id']));
			}	 				 	  		 	 		 	 
			if ($id) {
				//发送
...
/*	$id=$_GPC['actid']; 	 	 	 	 
	...
	$sgdata=pdo_fetchcolumn('select data from '.tablename('sz_yi_match_signup').' where uniacid = :uniacid and openid = :openid ',array(':uniacid'=>$_W['uniacid'],':openid'=>$openid));
