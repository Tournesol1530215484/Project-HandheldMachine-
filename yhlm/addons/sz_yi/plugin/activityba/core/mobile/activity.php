...
if ($op == 'display') {
	...
	$tmu=m('activity')->getMuser($tac['openid']);
	if ($tmu) {	 			 	 	 	 	 
	...
	$like=pdo_fetchall('select m.avatar from '.tablename('sz_yi_activity_like').' al left join '.tablename('sz_yi_member').' m on m.openid = al.openid where al.uniacid = :uniacid and al.atid = :id and al.type = 1 order by al.id desc limit 0,32 ',array(':uniacid'=>$_W['uniacid'],':id'=>$id)); 
	...
	if ($ac == 'getlist') {
		...
		$sql='select s.* from '.tablename('sz_yi_activity_signup').' s left join '.tablename('sz_yi_member').' m on m.openid = s.openid where s.uniacid = :uniacid and s.actid = :id and s.deleted = 0';
		$sql.=' limit '.($pindex -1) * $psize.','.$psize;
	...
	if ($_W['isajax']) {
		if ($ac == 'get') {
			...
			if ($type == '1') {
				$sql="select a.* from ".tablename('sz_yi_activity_signup').' s left join '.tablename('sz_yi_activity').' a on a.id=s.actid where s.uniacid = :uniacid and s.openid = :openid and s.deleted = 0';
				$sql.=' order by s.id desc ';
			}else if($type == '2'){
				$sql='select a.id,a.title,s.ctime from '.tablename('sz_yi_activity_share').' s left join '.tablename('sz_yi_activity').' a on a.id = s.actid where s.uniacid = :uniacid and s.openid = :openid and s.type = 1 group by s.actid ';
				$sql.=' order by s.id desc ';
			}else if($type == '3'){ 		
	...
	$activity=m('activity')->getact($id,$type);
	$fd=array(
		'title'=> $activity['title'],
	...
	if ($_W['isajax']) {
		if ($ac == 'sub') {
			...
			if ($activity['cost'] == 1 && !empty($payitem) && empty($_GPC['fee'])) {
				show_json(0,'请选择项目!');
			}
					...
					if ($member['agentid']) {
						...
						if ($agent) {
							...
							if (empty($tb)) {	 	
								$bonus1=array(
									...
									'mid'=>$_W['uniacid'],	 	
									'openid'=>$openid,	 	 
									'money'=>floatval($data['money']) * floatval($tact['agent1'] / 100),	 	 
									'level'=>1,	 	 
									'actid'=>$_GPC['id'],	 	 
									'ctime'=>time()
								);
						...
						if ($agent['agentid']) {					// 2级
							...
							if ($agent) {	 		 	
								...
								$fw2=pdo_fetch('select * from '.tablename('sz_yi_activity_share').' where uniacid = :uniacid and openid = :openid and type = 1 and actid = :id and level = 2',$params);
								...
								if (empty($tb)) {	 	
									$bonus2=array(
										...
										'mid'=>$_W['uniacid'],	 	
										'openid'=>$openid,	 	 
										'money'=>floatval($data['money']) * floatval($tact['agent2'] / 100),	 	 
										'level'=>2,	 	 	 	 	 	 
										'actid'=>$_GPC['id'],	 	 	 	
										'ctime'=>time()	 
									);	 	 	 	
	...
	foreach ($sglist as $key => $value) {
		if ($value['pay']>0) {
			m('member')->setCredit($value['oenid'],'credit2',$value['pay']);
		}
