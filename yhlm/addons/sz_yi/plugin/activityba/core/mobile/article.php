...
if ($op == 'display') {
	...
	$activity=pdo_fetch('select * from '.tablename('sz_yi_activity_article').' where uniacid  = :uniacid and id = :id',array(':uniacid'=>$_W['uniacid'],':id'=>$id));
	...
	$like=pdo_fetchall('select m.avatar from '.tablename('sz_yi_activity_like').' al left join '.tablename('sz_yi_member').' m on m.openid = al.openid where al.uniacid = :uniacid and al.atid = :id and al.type = 2 order by al.id desc limit 0,32 ',array(':uniacid'=>$_W['uniacid'],':id'=>$id)); 
