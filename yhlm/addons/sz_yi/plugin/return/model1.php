...
if (!class_exists('ReturnModel')) {
	...
	class ReturnModel extends PluginModel
	{
		...
		public function setGoodsQueue($orderid) {
			...
			$goods_queue = pdo_fetch("select * from " . tablename('sz_yi_order_goods_queue'). " where uniacid=:uniacid and status1=1 and id=:id order by id desc", array(':uniacid' =>$_W['uniacid'],':id'=>$queueid)); 		
		
			$return = pdo_fetch("select * from " . tablename('sz_yi_order_goods_queue'). " where uniacid=:uniacid and goodsid=:goodsid and status1=1  order by id desc", array(':uniacid' =>$_W['uniacid'],':goodsid'=>$goods_queue['goodsid'])); 		
						 
						 $num = pdo_fetchcolumn("select count(goodsid) from ".tablename('sz_yi_order_goods_queue')." where uniacid='".$_W['uniacid']."'  and status1=1   and goodsid='".$goods_queue['goodsid']."' order by id asc ");
						 $member = m('member')->getMember($good['openid']);
						 
						  $status = pdo_fetch("select queue from ".tablename('sz_yi_order_goods_queue')." where uniacid='".$_W['uniacid']."'  and status1=1    and goodsid='". $goods_queue['goodsid'] . "' and type=0 order by id asc ");
							
						 $a =$_var_0['out'];$b=1; 
						  file_put_contents(dirname(__FILE__).'/dasdsadsa',json_encode( $a)); 
								 for($a ; $a<=$num ; ){
								 
									if($a >= $status['queue'] * $_var_0['out'] ){
										$list = pdo_fetch("select * from ".tablename('sz_yi_order_goods_queue')." where uniacid='".$_W['uniacid']."'  and status1=1   and goodsid='".$goods_queue['goodsid']."'    and type='0' order by id asc limit 1");					$member2 = m('member')->getMember($list['openid']);
