...
if ($operation == 'af_supplier') {
	...
	if (empty($openid)) {
		...
		if ($status == 0) {
			$res = pdo_delete('sz_yi_af_supplier', array('openid' => $openid));
			if (!empty($res)) {
				message('驳回申请成功', $this->createPluginWebUrl('supplier/supplier_for'), 'success');
			}
			exit;
		}
...
$i = 'select idimg1,idimg2 from'.tablename('sz_yi_supplier_idimages').'where uniacid=:uniacid and openid=:openid';
