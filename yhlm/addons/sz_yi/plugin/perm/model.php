...
if (!class_exists('PermModel')) {
	class PermModel extends PluginModel
	{
		...
		{			//dealmerch.dealmerch
			...
			//修复设置权限有些功能访问无权限 新增 AND u.uniacid=:uniacid /data 2016.09.09start
			$user = pdo_fetch('SELECT u.id,u.uid,u.uniacid, u.status as userstatus,r.status as rolestatus,u.perms as userperms,r.perms as roleperms '.
							  ' FROM ' . tablename('sz_yi_perm_user') . ' u ' . ' LEFT JOIN ' . tablename('sz_yi_perm_role') . ' r on u.roleid = r.id ' . 
							  ' WHERE uid=:uid AND u.uniacid=:uniacid  limit 1 ', array(':uid' => $uid, ':uniacid' => $_W['uniacid']));	
		...
		function check_plugin($pluginname = '')			//activity
		{
			global $_W, $_GPC;
