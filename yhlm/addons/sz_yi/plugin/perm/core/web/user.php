...
if ($operation == 'display') {
	...
	if ($_W['isajax'] && $_W['ispost']) {
		...
		//新增修改操作员密码 DATE:2016.09.01 start
		if (!empty($item['username'])) {	 	 
			$where = ' WHERE `username`=:username';
			$params[':username'] = $item['username'];	 	 	 	
		}	 	 	 	 		 	 
		$user_info = pdo_fetch('SELECT salt FROM ' . tablename('users') . " $where LIMIT 1 ", $params);

		if (!empty($item['id'])) {	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 
			//user_update(array('uid' => $item['uid'],  'password' => $_GPC['password']));
			user_update(array('uid' => $item['uid'], 'salt' => $user_info['salt'],  'password' => $_GPC['password']));
		//end DATE:2016.09.01
