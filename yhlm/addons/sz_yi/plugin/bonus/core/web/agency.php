...
if ($operation == 'display'){
    ...
    if($_GPC['bonus_area'] != ''){
        ...
        }else if($_GPC['bonus_area'] == 4){
            $condition .= ' and dm.bonus_area=4';
        }
    ...
    $sql = 'select dm.*,sf.worknumber,sf.name workname,sf.status workstatus,dm.nickname,dm.avatar,l.levelname,p.nickname as parentname,p.avatar as parentavatar from ' . tablename('sz_yi_staff') . ' sf ' . ' left join ' . tablename('sz_yi_perm_user') . ' pu on sf.merchid = pu.uid left join '.tablename('sz_yi_member').' dm on pu.openid = dm.openid left join ' . tablename('sz_yi_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('sz_yi_bonus_level') . ' l on l.id = dm.bonuslevel' . ' left join ' . tablename('mc_mapping_fans') . "f on f.openid=dm.openid and f.uniacid={$_W['uniacid']}" . ' where dm.bonus_area >0 and pu.dealmerchid > 0 and dm.uniacid = ' . $_W['uniacid'] . " and dm.isagent =1  {$condition} ORDER BY dm.agenttime desc";
    if (empty($_GPC['export'])){
        $sql .= ' limit ' . ($pindex - 1) * $psize . ',' . $psize;
    }
    $list = pdo_fetchall($sql, $params);
    $total = pdo_fetchcolumn('select count(sf.id) from' . tablename('sz_yi_staff') . ' sf  ' . ' left join ' . tablename('sz_yi_perm_user') . ' pu on pu.uid = sf.mid ' . ' left join ' . tablename('sz_yi_member') . 'dm on pu.openid=dm.openid' . ' where pu.dealmerchid > 0 and dm.uniacid =' . $_W['uniacid'] . " and dm.isagent =1 {$condition}", $params);  
    ...
    if (checksubmit('submit')){
        ...
        if(!empty($data['bonus_area'])){
            ...
            empty($_GPC['agpwd']) && message('请输入代理商密码', '', 'error');
