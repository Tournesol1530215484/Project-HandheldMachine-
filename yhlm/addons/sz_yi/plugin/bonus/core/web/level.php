...
if ($operation == 'display'){
    ...
    $commission_level = pdo_fetchall('select * from '.tablename('sz_yi_bonus_level')." where uniacid = '{$_W['uniacid']}' ");  

    $update = json_decode($level['update_con'],true);
    if (checksubmit('submit')){
        if (empty($_GPC['levelname'])){
            message('抱歉，请输入名称！');
        }
        //$data = array('conditions' => $_GPC['conditions'],'uniacid' => $_W['uniacid'], 'level' => $_GPC['level'], 'levelname' => $_GPC['levelname'], 'agent_money' => floatval($_GPC['agent_money']), 'commissionmoney' => $_GPC['commissionmoney'], 'ordermoney' => $_GPC['ordermoney'], 'ordercount' => intval($_GPC['ordercount']), 'downcount' => intval($_GPC['downcount']), 'downcountlevel1' => intval($_GPC['downcountlevel1']), 'content' => intval($_GPC['content']), 'premier' => intval($_GPC['premier']), 'pcommission' => floatval($_GPC['pcommission']), 'msgtitle' => trim($_GPC['msgtitle']), 'msgcontent' => trim($_GPC['msgcontent']));
        ...
        $data = array(
           ...
           // 'downcountlevel1' => intval($_GPC['downcountlevel1']), 
            'update_con' => json_encode($_GPC['update']),
