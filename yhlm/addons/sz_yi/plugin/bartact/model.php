...
define('TM_bartact_PAY', 'bartact_pay');
if (!class_exists('bartactModel')){
    class bartactModel extends PluginModel{
        public $parentAgents = "";
       ...
       function perms(){
            return array(
				'bartact' => array(
						...
						'child' => array(
                            'set' => array('text' => '设置'),      
                            'slide' => array('text' => '幻灯片'), 
                            'activity' => array('text' => '活动'),
                            'poster' => array('text' => '推广海报'), 
                            'activity.show' => array('text' => '查看活动'),
                            'article' => array('text' => '文章'),
                            'activity.edit' => array('text' => '修改活动'),
                            'activity.delete' => array('text' => '删除活动'),
                            'activity.audit' => array('text' => '审核活动'),
                            'article.show' => array('text' => '查看文章'),
                            'article.edit' => array('text' => '修改文章'),
                            'article.delete' => array('text' => '删除文章'),
                            'article.audit' => array('text' => '审核文章'),
                            'merch' => array('text' => '活动商'),
                            'merch.add' => array('text' => '添加活动商'),
                            'bloc' => array('text' => '集团'),              
                    ), 
				) 
			); 
        ...
        public function sendbartactInform($dephp_64 = '', $dephp_69 = ''){
        ...
        public function order_split($dephp_74){
            ...
            $dephp_75 = pdo_fetchall('select distinct bartact_uid from ' . tablename('sz_yi_order_goods') . ' where orderid=:orderid and uniacid=:uniacid', array(':orderid' => $dephp_74, ':uniacid' => $_W['uniacid']));
            if(count($dephp_75) == 1){
                pdo_update('sz_yi_order', array('bartact_uid' => $dephp_75[0]['bartact_uid']), array('id' => $dephp_74, 'uniacid' => $_W['uniacid']));
                return;
            }
            $dephp_76 = pdo_fetchall('select bartact_uid, id from ' . tablename('sz_yi_order_goods') . ' where orderid=:orderid and uniacid=:uniacid ', array(':orderid' => $dephp_74, ':uniacid' => $_W['uniacid']));
            ...
            foreach ($dephp_76 as $dephp_80 => $dephp_81){
                $dephp_79[$dephp_81['bartact_uid']][]['id'] = $dephp_81['id'];
            }
            $dephp_82 = false;
            unset($dephp_77['id']);
            ...
            if(!empty($dephp_79)){
                foreach ($dephp_79 as $dephp_80 => $dephp_81){
                    ...
                    foreach($dephp_81 as $dephp_96){
                        $dephp_70 = pdo_fetch('select price,realprice,oldprice,bartact_uid from ' . tablename('sz_yi_order_goods') . ' where id=:id and uniacid=:uniacid ', array(':id' => $dephp_96['id'], ':uniacid' => $_W['uniacid']));
