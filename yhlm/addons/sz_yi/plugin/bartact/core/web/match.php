...
//                $table='sz_yi_match';
    ...
    $sql='select aa.* from '.tablename('sz_yi_match').' aa left join '.tablename('sz_yi_member_user').' mu on mu.openid = aa.openid where 1 '.$condition;

    $sql.=' order by aa.id desc ';

    $totals = pdo_fetchcolumn("select count(*) from " . tablename('sz_yi_match'). " aa left join ".tablename('sz_yi_member_user')." mu on mu.openid = aa.openid   where 1 {$condition} ", $params);
