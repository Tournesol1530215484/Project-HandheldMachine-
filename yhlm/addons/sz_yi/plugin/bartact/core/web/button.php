...
if ($operation == 'display') {
    ...
    if (checksubmit('submit')) {
        ...
        message('更新幻灯片成功！', $this->createPluginWebUrl('bartact/button'), 'success');
    }
...
//    print_r($pic);exit;
    ...
    if (empty($item)) {
        message('抱歉，底部航栏不存在或是已经被删除！', $this->createPluginWebUrl('bartact/button', array(
            'op' => 'display'
        )), 'error');
    }
    ...
    message('底部导航栏删除成功！', $this->createPluginWebUrl('bartact/button', array(
        'op' => 'display'
    )), 'success');
}
load()->func('tpl');
