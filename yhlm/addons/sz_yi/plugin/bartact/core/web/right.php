...
if ($operation == 'display') {
    ...
    if (checksubmit('submit')) {
        ...
        message('更新成功！', $this->createPluginWebUrl('bartact/right'), 'success');
    }
    ...
    if (empty($item)) {
        message('抱歉，航栏不存在或是已经被删除！', $this->createPluginWebUrl('bartact/right', array(
            'op' => 'display'
        )), 'error');
    }
    ...
    message('导航栏删除成功！', $this->createPluginWebUrl('bartact/right', array(
        'op' => 'display'
    )), 'success');
}
load()->func('tpl');
include $this->template('right');
