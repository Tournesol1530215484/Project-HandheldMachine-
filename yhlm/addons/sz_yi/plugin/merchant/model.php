...
define('TM_Merchant_PAY', 'merchant_pay');
if (!class_exists('MerchantModel')){
    class MerchantModel extends PluginModel{
       ...
       function perms(){
            return array( 
				'merchant' => array( 
						...
						'child' => array( 
                            ...
                            'merchant' => array('text' => '商家管理'),
                            'merchant.add' => array('text' => '添加商家'),
                            'merchant.edit' => array('text' => '修改商家'),
                            'merchant.view' => array('text' => '查看商家'),
							'merchant.delete' => array('text' => '删除商家'),
							'merchant_list' => array('text' => '商家订单'), 
							'merchant_for' => array('text' => '会员申请商家'), 
							'merchant_for_resu' => array('text' => '申请结果'), 
							'merchant_apply' => array('text' => '商家提现申请'), 
							'merchant_finish' => array('text' => '商家提现完成')
								)
							)
						);
        }
        ...
        public function sendMerchantInform($dephp_64 = '', $dephp_69 = ''){
        ...
        public function order_split($dephp_74){
            ...
            $dephp_75 = pdo_fetchall('select distinct Merchant_uid from ' . tablename('sz_yi_order_goods') . ' where orderid=:orderid and uniacid=:uniacid', array(':orderid' => $dephp_74, ':uniacid' => $_W['uniacid']));
            if(count($dephp_75) == 1){
                pdo_update('sz_yi_order', array('Merchant_uid' => $dephp_75[0]['Merchant_uid']), array('id' => $dephp_74, 'uniacid' => $_W['uniacid']));
                return;
            }
            $dephp_76 = pdo_fetchall('select Merchant_uid, id from ' . tablename('sz_yi_order_goods') . ' where orderid=:orderid and uniacid=:uniacid ', array(':orderid' => $dephp_74, ':uniacid' => $_W['uniacid']));
            ...
            foreach ($dephp_76 as $dephp_80 => $dephp_81){
                $dephp_79[$dephp_81['Merchant_uid']][]['id'] = $dephp_81['id'];
            }
            $dephp_82 = false;
            unset($dephp_77['id']);
            ...
            if(!empty($dephp_79)){
                foreach ($dephp_79 as $dephp_80 => $dephp_81){
                    ...
                    foreach($dephp_81 as $dephp_96){
                        $dephp_70 = pdo_fetch('select price,realprice,oldprice,Merchant_uid from ' . tablename('sz_yi_order_goods') . ' where id=:id and uniacid=:uniacid ', array(':id' => $dephp_96['id'], ':uniacid' => $_W['uniacid']));
				...
				if($_var_1['dispatchtype']==1){
                    ...
                    $dephp_86['Merchant_uid'] = $dephp_97;
