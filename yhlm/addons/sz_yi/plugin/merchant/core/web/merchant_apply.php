...
if ($operation == 'display'){
    ...
    $list = pdo_fetchall('select a.*,p.accountname, mobile as telephone, accountbank, banknumber   from ' . tablename('sz_yi_merchant_apply') . ' a left join ' . tablename('sz_yi_perm_user') . ' p on p.uid=a.uid where a.status=0 and p.uniacid=' . $_W['uniacid'] . $where);
    $total = count($list);
    ...
    if(!empty($id)){
        ...
        pdo_update('sz_yi_merchant_apply', $data, array('id' => $id));
        $msg = $apply['type'] == 1 ? '手动打款成功' : '提现到微信钱包成功!';
        p('merchant') -> sendMessage($openid, array('money' => $apply['apply_money'], 'type' => $msg), TM_merchant_PAY);
