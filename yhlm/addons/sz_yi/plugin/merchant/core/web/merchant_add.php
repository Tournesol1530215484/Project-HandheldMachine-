...
if($operation == 'post'){           //如果method是post类型
    ...
    $id = $_GPC['id'];              //如果有传id
    if (empty($id)){
        ca('merchant.merchant.add');
    }else{
        ca('merchant.merchant.view|merchant.merchant.edit');
    }
    ...
    pdo_delete('sz_yi_merch_user',array('uid'=>$item['uid']));
