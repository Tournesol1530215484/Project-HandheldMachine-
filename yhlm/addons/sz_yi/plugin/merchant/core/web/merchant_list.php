...
global $_W, $_GPC;
ca('merchant.merchant_list');
$merchants = pdo_fetchall('select * from ' . tablename('sz_yi_perm_user') . " where uniacid={$_W['uniacid']} and merchid>0 and roleid = (select id from " . tablename('sz_yi_perm_role') . ' where status1=1 and uniacid = '.$_W['uniacid'].' LIMIT 1)');
...
load() -> func('tpl');
include $this -> template('merchant_list');
exit;
