...
foreach ($list as $key => $value) {
    if ($value['recommendopenid']) {
       $list[$key]['agent']=m('member')->getMember($value['recommendopenid']);
    }else{
        $temp=m('member')->getMember($value['openid']);
        $list[$key]['agent']=m('member')->getMember($temp['agentid']);
    }
}
if ($_GPC['export1'] == '1'){
...
load() -> func('tpl');
include $this -> template('merchant_for_resu');  
