...
$where='';
if($_GPC['merch'] == 2){
    $where.=' and dealmerchid = 0 and merchid = 0 ';             //普通商家的uid(全国)
}else if ($_GPC['merch'] == 3){
    $where.=' and merchid > 0 ';             //普通商家的uid(包括本地)
}else if ($_GPC['merch'] == 5){
    $where.=' and dealmerchid > 0 ';           //易货商家的uid
}
...
//    if ($_GPC['merch'] == 3 or $_GPC['merch'] == 2){
//        $conditions.=' and o.isexchange =0 ';             //普通的订单
//    }else if ($_GPC['merch'] == 5){
//        $conditions.=' and o.isexchange = 1 ';           //易货的订单
//    }

    $order = pdo_fetchall('select o.addressid,o.isexchange,o.id,o.ordersn,o.paytime,o.status,o.price, o.uniacid,o.paytype,o.goodsprice from '.tablename('sz_yi_order')." as o join ".tablename('sz_yi_order_goods')." as og on o.id = og.orderid and og.supplier_uid = $uid  where o.uniacid = '{$_W['uniacid']}'  {$conditions} group by o.id  order by o.createtime desc limit   ".($page*$psize)." , {$psize}" );

    foreach ($order as $key => &$value) {
        $value['uid']= $uid;
        $value['goods'] = pdo_fetchall('select og.orderid as id , og.orderid , g.uniacid as uniacid , og.price, og.total ,og.realprice ,g.title, CONCAT('."'{$_W['attachurl']}'".',g.thumb) as thumb from '.tablename('sz_yi_order_goods').' as og left join '.tablename('sz_yi_goods')." as g on og.goodsid = g.id  where og.orderid = '{$value['id']}' and g.uniacid = '{$_W['uniacid']}' and og.supplier_uid = $uid  ");
        ...
        $value['url']= $this->createPluginMobileUrl('suppliermenu/order',array('op'=>'action','id'=>$value['id'],'merch'=>$_GPC['merch']));
    }
...
if ($op == 'withdraw') { 
    ...
    // 计算可提现金额
    $costmoney = 0; 
    $sp_goods = pdo_fetchall('select og.*,o.dispatchprice from ' . tablename('sz_yi_order_goods') . ' og left join ' . tablename('sz_yi_order') . " o on (o.id=og.orderid) where og.uniacid={$_W['uniacid']} and og.supplier_uid={$uid} and o.status=3 and og.supplier_apply_status=0");
    ...
    if (!empty($applytype)) {
        ...
        $mygoodsid = pdo_fetchall('select og.id from ' . tablename('sz_yi_order_goods') . ' og left join '.tablename('sz_yi_order').' o on o.id  = og.orderid where o.status = 3 and og.supplier_uid=:supplier_uid and og.supplier_apply_status = 0 ', array(':supplier_uid' => $uid));
        ...
        $data = array(
            ...
            if ($_GPC['merch'] == 5) {          //邮费提现
                $deduct=doubleval($costmoney * ($sets['bart']['withdraw'] / 100));
                $data['apply_money'] = ( doubleval($costmoney) - $deduct );
                $data['apply_deduct'] = $deduct; 
            }
...
if ($op == 'withdrawlist') {
    ...
    //merch openid
    $orderInfo=pdo_fetch('select o.id,o.createtime,o.ordersn,o.price,ma.realname,ma.mobile,ma.province,ma.city,ma.area,ma.address from '.tablename('sz_yi_order').' as o left join '.tablename("sz_yi_member_address").' as ma on o.addressid=ma.id where o.id=:oid and o.uniacid=:uniacid '
        ,$param);
        $tempTotal=pdo_fetchall('select total from '.tablename('sz_yi_order_goods').' where orderid=:orderid and uniacid=:uniacid',array(':orderid'=>$orderid,':uniacid'=>$_W['uniacid']));

    foreach ($tempTotal as $k => $v){
        $orderInfo['total']+=intval($v['total']);
    }
