...
$res = pdo_fetch($sql, array(':uniacid' => $_W['uniacid'], ':uid' => $_W['uid']));
if (false === $res) {
    message('您没有权限', '', 'error');
    exit;
}
...
if ($op == 'display') {
    ...
    $pages    = pdo_fetchall("SELECT * FROM " . tablename('sz_yi_designer') . " WHERE uniacid= :uniacid and supplier = '{$_W['uid']}' and pagename LIKE :name " . "ORDER BY savetime DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, array(
        ':uniacid' => $_W['uniacid'],
        ':name' => "%{$kw}%"
    ));
    $pagesnum = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('sz_yi_designer') . " WHERE uniacid= :uniacid   and supplier = '{$_W['uid']}' " . "ORDER BY savetime DESC ", array(
        ':uniacid' => $_W['uniacid']
    ));
    $total    = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('sz_yi_designer') . " WHERE uniacid= :uniacid  and supplier = '{$_W['uid']}' and pagename LIKE :name " . "ORDER BY savetime DESC ", array(
        ':uniacid' => $_W['uniacid'],
        ':name' => "%{$kw}%"
    ));
    ...
    $menus     = pdo_fetchall("SELECT id,menuname,isdefault FROM " . tablename('sz_yi_designer_menu') . " WHERE uniacid= :uniacid and supplier = '{$_W['uid']}'  ", array(
        ':uniacid' => $_W['uniacid']
    ));
    $pages     = pdo_fetchall("SELECT id,pagename,pagetype,setdefault FROM " . tablename('sz_yi_designer') . " WHERE uniacid= :uniacid  and supplier = '{$_W['uid']}'  ", array(
        ':uniacid' => $_W['uniacid']
    ));
    ...
    if (!empty($pageid)) {
     ...
     //   ca('designer.page.edit');
       // $defaultmenuid = $this->model->getDefaultMenuID();
        ...
        $defaultmenuid = p('designer')->getDefaultMenuID();
    ...
    if ($_W['ispost']) {
        if ($apido == 'savepage') {
            ...
            $kw    = $_GPC['kw'];
            $goods = pdo_fetchall("SELECT id,title,productprice,marketprice,thumb,sales,unit FROM " . tablename('sz_yi_goods') . " WHERE uniacid= :uniacid and status=:status and deleted=0 AND supplier_uid = :supplier_uid AND title LIKE :title ", array(
                ':title' => "%{$kw}%",
                ':uniacid' => $_W['uniacid'],
                ':status' => '1',
                ':supplier_uid' => $_W['uid'], // 供应商只能选择自己的商品
            ));
            ...
            $type = $_GPC['type'];
            if ($do == 'on') {
                $pages = pdo_fetch("SELECT * FROM " . tablename('sz_yi_designer') . " WHERE pagetype=:pagetype and setdefault=:setdefault and supplier = '{$_W['uid']}' and uniacid=:uniacid ", array(
                    ':pagetype' => $type,
                    ':setdefault' => '1',
                    ':uniacid' => $_W['uniacid']
                ));
                ...
                $pages = pdo_fetch("SELECT * FROM " . tablename('sz_yi_designer') . " WHERE  id=:id and uniacid=:uniacid and supplier = '{$_W['uid']}' ", array(
                    ':id' => $id,
                    ':uniacid' => $_W['uniacid']
                ));
