<?php
	...
	if ($op == 'display') {
		...
		$condition  = " where sl.uniacid = :uniacid and g.supplier_uid=:supplier_uid ";
		$params = array(
			':uniacid'=> $_W['uniacid'],
			':supplier_uid'=> $_W['uid']
		);
        if ($_GPC['searchtime'] == 1) {
            if (!empty($_GPC['datetime'])){
                ...
                $condition .= " and sl.dealtime > :starttime and sl.dealtime < :endtime ";
		...
		if (!empty($_GPC['goodsname'])) {
			$condition .= " and sl.goodsname like :goodsname";
			$params[':goodsname'] = "%{$_GPC['goodsname']}%";
		} 
		//操作类型
		if (!empty($_GPC['type'])) {
			$condition .= " and sl.type = :type";
			$params[':type'] = intval($_GPC['type']);
		}
		//商品编号
		if (!empty($_GPC['goodssn'])) {
			$condition .= " and sl.goodssn like :goodssn";
			$params[':goodssn'] = "%{$_GPC['goodssn']}%";
		}
		//规格型号
		if (!empty($_GPC['optionid'])) {
			$condition .= " and sl.optionid like :optionid";
		...
		$total = pdo_fetchcolumn("select count(*) from ".tablename('sz_yi_stock_log') .' sl left join '.tablename('sz_yi_goods').' g on g.id=sl.goodsid '. $condition ." order by sl.id asc ", $params);
		$pager = pagination($total, $pindex, $psize); 
			 		 
		$info = pdo_fetchall("select sl.*  from ".tablename('sz_yi_stock_log').' sl left join '.tablename('sz_yi_goods').' g on g.id=sl.goodsid '.$condition." order by sl.id asc limit ".($pindex - 1 )*$psize . ',' . $psize , $params);
