            ...
            return array(
				'supplier' => array(
						...
						'child' => array(
							...
							'supplier' => array('text' => '商家管理'),
                            'supplier.add' => array('text' => '添加商家'),
                            'supplier.edit' => array('text' => '修改商家'),
                            'supplier.view' => array('text' => '查看商家'),
                            'supplier.delete' => array('text' => '删除商家'),
							'supplier_list' => array('text' => '商家订单'),
							'supplier_for' => array('text' => '会员申请商家'),
							'supplier_for_resu' => array('text' => '申请结果'),
							'supplier_apply' => array('text' => '商家提现申请'),
							'supplier_finish' => array('text' => '商家提现完成')
								)
							)
						);
        }
...
//            print_r($dephp_2);exit;
        ...
        public function sendSupplierInform($dephp_64 = '', $dephp_69 = '')
        {
            ...
            if ($dephp_69 == 2) {
                $sql = 'select username from '.tablename('sz_yi_af_supplier').'where openid = :openid';
                $username = pdo_fetchcolumn($sql, array(':openid' => $dephp_64));
                $dephp_68 = array(
                    'keyword1' => array(
                        'value' => $dephp_73,
                        'color' => '#73a68d'
                ...
                $sql = 'select account from '.tablename('sz_yi_af_supplier').'where openid = :openid';
                $account = pdo_fetchcolumn($sql, array(':openid' => $dephp_64));
                $account = !empty($account) ? $account : '未说明';
                $dephp_68 = array(
                    'keyword1' => array(
                        'value' => $dephp_73,
                        'color' => '#73a68d'
        ...
        public function order_split($dephp_74){
            ...
            $dephp_75 = pdo_fetchall('select distinct supplier_uid from ' . tablename('sz_yi_order_goods') . ' where orderid=:orderid and uniacid=:uniacid', array(':orderid' => $dephp_74, ':uniacid' => $_W['uniacid']));
            if(count($dephp_75) == 1){
                pdo_update('sz_yi_order', array('supplier_uid' => $dephp_75[0]['supplier_uid']), array('id' => $dephp_74, 'uniacid' => $_W['uniacid']));
                return;
            }
            $dephp_76 = pdo_fetchall('select supplier_uid, id from ' . tablename('sz_yi_order_goods') . ' where orderid=:orderid and uniacid=:uniacid ', array(':orderid' => $dephp_74, ':uniacid' => $_W['uniacid']));
            ...
            foreach ($dephp_76 as $dephp_80 => $dephp_81){
                $dephp_79[$dephp_81['supplier_uid']][]['id'] = $dephp_81['id'];
            }
            $dephp_82 = false;
            unset($dephp_77['id']);
            ...
            if(!empty($dephp_79)){
                foreach ($dephp_79 as $dephp_80 => $dephp_81){
                    ...
                    foreach($dephp_81 as $dephp_96){
                        $dephp_70 = pdo_fetch('select price,realprice,oldprice,supplier_uid from ' . tablename('sz_yi_order_goods') . ' where id=:id and uniacid=:uniacid ', array(':id' => $dephp_96['id'], ':uniacid' => $_W['uniacid']));
				...
				if($_var_1['dispatchtype']==1){
                    ...
                    $dephp_86['supplier_uid'] = $dephp_97;
