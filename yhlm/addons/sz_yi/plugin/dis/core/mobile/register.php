...
if ($_W['isajax']){
    ...
    if (empty($set['become'])){
        ...
        $goods = pdo_fetch('select id,title from' . tablename('sz_yi_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $set['become_goodsid'], ':uniacid' => $_W['uniacid']));
        $goodscount = pdo_fetchcolumn('select count(*) from ' . tablename('sz_yi_order_goods') . ' og ' . '  left join ' . tablename('sz_yi_order') . ' o on o.id = og.orderid' . ' where og.goodsid=:goodsid and o.openid=:openid and o.status>=1  limit 1', array(':goodsid' => $set['become_goodsid'], ':openid' => $openid));
        if ($goodscount <= 0){
            $ret['status'] = 3;
            ...
            $ret['status'] = 4;
            $data = array('isagent' => 1, 'status' => 1, 'agenttime' => time());
            $member['status'] = 1;
            $ret['member'] = $member;
            pdo_update('sz_yi_member', $data, array('id' => $member['id']));
            $this -> model -> sendMessage($member['openid'], array('agenttime' => $data['agenttime']), TM_COMMISSION_BECOME);
    ...
    if ($_W['ispost']){
        ...
        if ($ret['status'] == 1 || $ret['status'] == 2){
            ...
            if ($template_flag == 1){
                ...
                pdo_update('sz_yi_member', $m_data, array('id' => $member['id']));
                if ($become_check == 1){
                    $this -> model -> sendMessage($member['openid'], array('agenttime' => $m_data['agenttime']), TM_COMMISSION_BECOME);
                }
                if (!empty($member['uid'])){
                    load() -> model('mc');
                    if (!empty($mc_data)){
