...
$bonus_level = pdo_fetchall("SELECT * FROM " . tablename('sz_yi_bonus_level') . " WHERE uniacid = '$uniacid'"); 
$bd_level = pdo_fetchall("SELECT * FROM " . tablename('bd_level') . " WHERE uniacid = '$uniacid'");
...
if ($operation1 == 'insert') {
		...
		if($dis_level1){
			...
			foreach($agentlevel as $ro){
				...
				if($ro['agentlevel'] ==$_POST['level'] ||  $ro['bonuslevel'] == $_POST['level_name']){
					$a=2;
				}
			}
					if($a==2){
						...
						load()->model('account');	
								...
								$member = pdo_fetchall("SELECT id,realname,mobile,weixin,avatar,agentlevel FROM " . tablename('sz_yi_member') . " WHERE  uniacid=:uniacid and agentlevel=:agentlevel  and id=:id", array( ':uniacid' => $_W['uniacid'],':agentlevel' => $_POST['level'],':id' => $result_id));
								/* print_r('<pre>');
								   print_r($member); */
									if(!empty($member)){
							  ...
							  message('新添加用户证件成功！', $this->createPluginWebUrl('dis/notice', array('op' => 'post' )), 'success'); 
						}	
			...
			elseif($_POST['level_name'] !=0){
					...
					if($a==3){
						if (checksubmit('submit')) {
							load()->model('account');	
							$member = pdo_fetchall("SELECT id,realname,mobile,weixin,avatar,bonuslevel FROM " . tablename('sz_yi_member') . " WHERE  uniacid=:uniacid and bonuslevel=:bonuslevel", array( ':uniacid' => $_W['uniacid'],':bonuslevel' => $_POST['level_name']));
			...
			elseif($_POST['db_level'] !=0){
					...
					if($a==3){
						...
						if (checksubmit('submit')) {
							...
							$bd_member = pdo_fetchall("select m.id,m.realname,m.mobile,m.weixin,m.avatar,bm.level as level1 from " . tablename('sz_yi_member') . " m" . " left join " . tablename('bd_member') . " bm on bm.openid = m.openid"  . " WHERE  m.uniacid=:uniacid and bm.uniacid=:uniacid and bm.level=:level1", array( ':uniacid' => $_W['uniacid'],':level1' => $_POST['db_level']));
