...
if (checksubmit('submit')) {
	...
	$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'sz_yi经销商中心入口设置', 'module' => 'cover', 'displayorder' => 0, 'status' => intval($data['status']));
	...
	$cover_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => $this->modulename, 'title' => trim($data['title']), 'description' => trim($data['desc']), 'thumb' => $data['thumb'], 'url' => $this->createPluginMobileUrl('dis'));
	pdo_insert('cover_reply', $cover_data);
	plog('dis.cover', '修改经销商查询入口设置');
	message('经销商查询入口设置成功!', referer(), 'success');
}
load()->func('tpl');
