...
/* 	 $bd_level = pdo_fetchall("select dm.id,dm.realname,dm.bd_level,bm.levelname from " . tablename('sz_yi_dis_level') . " dm " . " left join " . tablename('bd_level') . " bm on bm.id = dm.bd_level"  . " where   dm.uniacid = " . $_W['uniacid'] . "  and bm.uniacid = " . $_W['uniacid'] . "   ORDER BY dm.id desc"); 
    ...
    $psize2     = 20;
    $params2    = array();
	$sql2 = "select dm.id,dm.realname,dm.bd_level,bm.levelname from " . tablename('sz_yi_dis_level') . " dm " . " left join " . tablename('bd_level') . " bm on bm.id = dm.bd_level"  . " where   dm.uniacid = " . $_W['uniacid'] . "  and bm.uniacid = " . $_W['uniacid'] . "   ORDER BY dm.id desc";
	 $sql2 .= " limit " . ($pindex2 - 1) * $psize2 . ',' . $psize2;
	 $bd_level  = pdo_fetchall($sql2, $params2);
	 $total2 = pdo_fetchcolumn("select count(dm.id) from" . tablename('sz_yi_dis_blevel') . " dm  " . " left join " . tablename('sz_yi_bonus_level') . "f on f.id=dm.bonus_level" . " where dm.uniacid =" . $_W['uniacid'] . " ", $params1);
	 $pager12 = pagination($total2, $pindex2, $psize2); */
