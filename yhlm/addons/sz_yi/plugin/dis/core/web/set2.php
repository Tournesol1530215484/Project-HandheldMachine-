						...
						if($a==2){
							...
							if (checksubmit('submit')) {
								load()->model('account');	
									...
									foreach($result as $result_id){
									$member = pdo_fetchall("SELECT id,realname,mobile,weixin,avatar,bonuslevel FROM " . tablename('sz_yi_member') . " WHERE  uniacid=:uniacid and bonuslevel=:bonuslevel  and id=:id", array( ':uniacid' => $_W['uniacid'],':bonuslevel' => $_POST['level_name'],':id' => $result_id));
										if(!empty($member)){
												foreach($member as $row){
													 $data = array(
														'uniacid' => $_W['uniacid'],
