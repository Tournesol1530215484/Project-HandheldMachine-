...
if ($operation == 'display') { 
	...
	if($_GPC['day1']){ 
	  ...
	  // and bonus_area ='0'  这个字段判断区域是否，0不是，1是
	  $sql = "select orderid,mid,levelid,mid,money,levelid from" . tablename('sz_yi_bonus_goods') .  " where uniacid = " . $_W['uniacid'] . "   {$condition}   and bonus_area ='0'    order by id asc ";
    ...
    if ($irme) {
	 ...
	 $total = pdo_fetchcolumn("select count(id) from " . tablename('sz_yi_bonus_goods') . " where uniacid = " . $_W['uniacid'] . "  {$condition} and bonus_area ='0' ", $params);
