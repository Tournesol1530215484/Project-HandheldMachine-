...
if ($operation == 'display') { 
	...
	if($_GPC['day1']){ 
	  ...
	  $sql = "select orderid,mid,levelid,mid,money from" . tablename('sz_yi_bonus_goods') .  " where uniacid = " . $_W['uniacid'] . "  and bonus_area ='1'   {$condition}    order by id asc ";
    ...
    if ($irme) {
		...
		foreach ($irme as $k=> $oe){
			...
			//买家资材信息
			$member =  pdo_fetch("select id,realname,bonuslevel,agentid,bonus_province,bonus_city,bonus_district from ".tablename('sz_yi_member'). " where  openid = ". "'$openid'" ."  and   uniacid=" . $_W['uniacid'] ."  order by id asc");
			...
			$realname_1 =  pdo_fetch("select realname as realname_1,bonus_area from ".tablename('sz_yi_member'). " where  id = ". $oe['mid'] ."  and   uniacid=" . $_W['uniacid'] ." {$condition2} order by id asc");
