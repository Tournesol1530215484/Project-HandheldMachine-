...
if ($op == 'display') {
  ...
  $color     = !empty($_GPC['color']) ? $_GPC['color'] : '';
    $sql="select u.* from ".tablename('sz_yi_perm_user')." u left join " .tablename('sz_yi_perm_role'). " r on r.id = u.roleid where r.status1=1 and r.uniacid = {$_W['uniacid']} and u.uniacid = :uniacid";
