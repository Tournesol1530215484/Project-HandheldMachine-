...
if($op == 'get' && $_W['isajax']){
    ...
    if($_W['ispost'] ){
        ...
        if( $ac == 'get' ){ //获取修改数据
            ...
            $_GPC['PostFlag']= $_GPC['PostFlag']=='PostFlag'?1:0;

            $_GPC['LocalFlag']= $_GPC['LocalFlag']=='LocalFlag'?1:0;
            ...
            $data = array(
                ...
                'province'        => trim($_GPC['province']),   //修改

                'city'        => trim($_GPC['city']), 

                'district'        => trim($_GPC['district']), //数据库没有
                ...
                'PostFlag'     => $_GPC['PostFlag'],

                'LocalFlag'    =>$_GPC['LocalFlag'],

                'isCheck'      => 0,

                'shelves'      =>1,

                'noticeopenid'=>$openid,        //通知人的信息
    ...
    if($_GPC['debug']==1){
        include $this->template('dealgoods_upload_debug');
    }else{
