...
//	$storeData = pdo_fetch($sdsql, array(':uid' => $uid, ':uniacid' => $_W['uniacid']));
	...
	if ($_W['isajax']) {
        ...
        if($_GPC['role']==2){
            $sql = 'select uid,dealmerchid,merchid from'.tablename('sz_yi_perm_user').' where openid = :openid and dealmerchid=0 and merchid=0';
            $merch = pdo_fetch($sql, array(':openid' => $openid));
        ...
        if($_GPC['role']==3){
            $sql = 'select uid,dealmerchid,merchid from'.tablename('sz_yi_perm_user').' where openid = :openid and merchid>0';
            $merch = pdo_fetch($sql, array(':openid' => $openid));

            $data = array('logo' => $logo, 'img' => $signboard, 'details' => $description, 'merchname' => $storename);
            $res = pdo_update('sz_yi_merch_user', $data, array('uid' => $merch['uid']));
//            print_r($res);exit;
        }

        if($_GPC['role']==5){
            $sql = 'select uid,dealmerchid,merchid from'.tablename('sz_yi_perm_user').' where openid = :openid and dealmerchid>0';
            $merch = pdo_fetch($sql, array(':openid' => $openid));

            $data = array('logo' => $logo, 'img' => $signboard, 'details' => $description, 'merchname' => $storename);
            $res = pdo_update('sz_yi_dealmerch_user', $data, array('uid' => $merch['uid']));
        }
