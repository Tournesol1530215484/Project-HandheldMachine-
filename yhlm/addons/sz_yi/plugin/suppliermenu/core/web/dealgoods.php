...
//		ca('dealmerch.dealgoods.edit|dealmerch.dealgoods.view');

//	} else {

//		ca('dealmerch.dealgoods.add');
...
//	}
	...
	if (checksubmit('submit')) {
		...
		// if (count($_GPC['reside']['province'])==0 
		// 	|| count($_GPC['reside']['city'])==0 
		// 	|| count($_GPC['reside']['district'])==0) {
		// 	message('请完善商品发货地！');
		// }

		if (empty($_GPC['reside']['province']) || $_GPC['reside']['province']=="请选择省份") {

			message('请完善商品发货地！');

		}
		if (empty($_GPC['reside']['city']) || $_GPC['reside']['city']=="请选择城市") {

			message('请完善商品发货地！');

		}
		if (empty($_GPC['reside']['district']) || $_GPC['reside']['district']=="请选择区域") {
...
//            'isverify' => intval($_GPC['isverify']),
            ...
            'province'=>trim($_GPC['reside']['province']),		//省
            'city'=>trim($_GPC['reside']['city']),				//市
            'district'=>trim($_GPC['reside']['district'])		//区县
		...
		if (empty($id)) {  
			...
			plog('dealmerch.dealgoods.add', "添加易货商品 ID: {$id}");  
		...
		message('商品更新成功！', $this->createPluginWebUrl('suppliermenu/dealgoods'), 'success');
...
//	ca('dealmerch.dealgoods.view');

	if (!empty($_GPC['displayorder'])) {

//		ca('dealmerch.dealgoods.edit');
