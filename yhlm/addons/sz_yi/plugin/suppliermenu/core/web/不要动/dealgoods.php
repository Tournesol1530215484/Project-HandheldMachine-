...
//	}
	...
	if (checksubmit('submit')) {
		...
		// if (empty(trim($_GPC['reside']['province']))) {

		// 	message('请完善商品发货地！');

		// }

		if (empty(trim($_GPC['reside']['city'])) || empty(trim($_GPC['reside']['district'])) || empty(trim($_GPC['reside']['province']))) {
		...
		// if (empty(trim($_GPC['reside']['city']))) {

		// 	message('请完善商品发货地！');

		// }
		// if (empty(trim($_GPC['reside']['district']))) {
...
if($_GPC['debug']){
	...
	include $this->template('dealgoods_debug');
}else{
