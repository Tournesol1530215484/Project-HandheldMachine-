...
if ($operation == 'change') {
	...
	$id = intval($_GPC['id']);
 	...
 	$pu=p('bonus')->getMerch($_W['uid']);
	$saler=m('member')->getMember($pu['openid']);
	...
	// 权限 type 1 供应商 2 商家
	$sql = 'select type from'.tablename('sz_yi_perm_user').'where uniacid = :uniacid and uid = :uid';
	$stype = pdo_fetch($sql, array(':uniacid' => $_W['uniacid'], ':uid' => $_W['uid']));
	$set = p('supplier')->getSet();
	$stype['type'] == 1 && $authority = $set['power'];
	$stype['type'] == 2 && $authority = $set['storepower'];
	// End 权限
	...
	if (checksubmit('submit')) {
		...
		if (p('supplier')) {
			// 供应商上架商品权限改动
			// if ($perm_role == 1) { // 供应商上架商品权限改动
			if (in_array('suppliermenu.shelves', $authority)) {

				$data['supplier_uid'] = $_W['uid'];
				// $data['status'] = 0; // 供应商上架商品权限改动
		...
		// if (empty($data['noticeopenid'] ) && !empty($data['supplier_uid'])) {
		...
		message('商品更新成功！', $this->createPluginWebUrl('suppliermenu/goods' ), 'success');
	}
	if (p('commission')) {
	...
	if (!empty($_GPC['displayorder'])) {
		...
		message('商品排序更新成功！', $this->createPluginWebUrl('suppliermenu/goods', array('op' => 'display')), 'success');
	}
	...
	if (p('supplier')) {
		$suproleid = pdo_fetchcolumn('select id from' . tablename('sz_yi_perm_role') . ' where uniacid = '.$_W['uniacid'].' and status1 = 1');
	...
	$id = intval($_GPC['id']);
	$row = pdo_fetch('SELECT id, title, thumb FROM ' . tablename('sz_yi_goods') . ' WHERE id = :id', array(':id' => $id,'supplier_uid'=>$_W['uid']));
	if (empty($row)) {
