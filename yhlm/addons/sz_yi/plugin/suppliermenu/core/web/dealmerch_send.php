...
if ($operation == 'display') {
	...
	if (p('supplier')) {
		...
		if ($perm_role == 1) {
			...
			$supplierapply = pdo_fetchall('select u.uid,p.realname,p.mobile,p.banknumber,p.accountname,p.accountbank,a.applysn,a.apply_money,a.apply_time,a.type,a.finish_time,a.status,a.apply_deduct from ' . tablename('sz_yi_supplier_apply') . ' a ' . ' left join' . tablename('sz_yi_perm_user') . ' p on p.uid=a.uid ' . 'left join' . tablename('users') . ' u on a.uid=u.uid where u.uid=' . $_W['uid']);
	...
	if ($_GPC['export'] == 1) {
		...
		foreach ($supplierapply as $key => $value) {
			if($supplierapply[$key]['type']==1){
				$supplierapply[$key]['type']="手动提现";
			}else if($supplierapply[$key]['type']==2){
				$supplierapply[$key]['type']="微信提现";
			}else if($supplierapply[$key]['type']=3){
				$supplierapply[$key]['type']="余额提现";
			}
			if($supplierapply[$key]['status']==0){
				$supplierapply[$key]['status']='申请中';
				$supplierapply[$key]['time']=date('Y-m-d H:i:s',$supplierapply[$key]['apply_time']);
			}else{
				$supplierapply[$key]['status']="已完成";
				$supplierapply[$key]['time']=date('Y-m-d H:i:s',$supplierapply[$key]['finish_time']);
			}
        ...
        //ca('order.op.export');
		//导出数据
		 plog('member.member.export', '导出提现单号');
        m('excel') -> export($supplierapply, array('title' => '导出提现单号' . date('Y-m-d-H-i', time()), 'columns' => array(array('title' => '提现单号', 'field' => 'applysn', 'width' => 12),array('title' => '开户号', 'field' => 'accountname', 'width' => 12), array('title' => '电话', 'field' => 'telephone', 'width' => 12), array('title' => '开户银行', 'field' => 'accountbank', 'width' => 12), array('title' => '银行账户', 'field' => 'banknumber', 'width' => 12), array('title' => '提现金额', 'field' => 'apply_money', 'width' => 12), array('title' => '提现手续费', 'field' => 'apply_deduct', 'width' => 12), array('title' => '提现方式', 'field' => 'type', 'width' => 12), array('title' => '申请/完成时间', 'field' => 'time', 'width' => 12), array('title' => '状态', 'field' => 'status', 'width' => 12))));
	...
	//var_dump($totalmoney);
	//$totalpostermoney = pdo_fetchcolumn('SELECT ifnull(sum(o.price),0) FROM ' . tablename('sz_yi_order') . ' o ' . ' left join ( select rr.id,rr.orderid,rr.status from ' . tablename('sz_yi_order_refund') . ' rr left join ' . tablename('sz_yi_order') . ' ro on rr.orderid =ro.id order by rr.id desc limit 1) r on r.orderid= o.id' . ' left join ' . tablename('sz_yi_member') . ' m on m.openid=o.openid  and m.uniacid =  o.uniacid' . ' left join ' . tablename('sz_yi_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('sz_yi_member') . ' sm on sm.openid = o.verifyopenid and sm.uniacid=o.uniacid' . ' left join ' . tablename('sz_yi_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . " WHERE $condition $statuscondition $cond ", $paras);
	
	//$costmoney  所有商品和邮费的总价格

	 //$uid = pdo_fetchcolumn(' select uid from  '.tablename('sz_yi_perm_user')." where openid = '{$openid}' {$where} limit 1 ");

	//计算可提现邮费金额
	 $sp_goods = pdo_fetchall('select og.*,o.dispatchprice from ' . tablename('sz_yi_order_goods') . ' og left join ' . tablename('sz_yi_order') . " o on (o.id=og.orderid) where og.uniacid={$_W['uniacid']} and og.supplier_uid={$_W['uid']} and o.status=3 and og.supplier_apply_status=0");
	 foreach ($sp_goods as $key => $value) {   

            if ($value['dispatchprice'] > 0) {
                $costmoneys+=$value['dispatchprice'];

            }

    } 

	include $this->template('posterorder/list'); 
	...
	$_W['uid'] && $cond = ' and o.supplier_uid = '.$_W['uid'] .' and o.storeid = 0 and o.addressid > 0';
	...
	include $this->template('posterorder/detail');
	...
	if ($to == 'confirmpay') {
		...
		include $this->template('posterorder/changeprice');
		...
		include $this->template('posterorder/express');
...
elseif ($operation == 'canti') {
	...
	$sp_goods = pdo_fetchall('select o.dispatchprice,og.orderid,g.title,o.goodsprice,o.createtime from ' . tablename('sz_yi_order_goods') . ' og left join ' . tablename('sz_yi_order') . " o on (o.id=og.orderid) left join " . tablename('sz_yi_goods') . " g on (og.goodsid=g.id) where og.uniacid={$_W['uniacid']} and og.supplier_uid={$_W['uid']} and o.status=3 and og.supplier_apply_status=0");
		...
		include $this->template('posterorder/canti');
 ...
 elseif ($operation == 'apply') {
	...
	include $this->template('posterorder/apply');
