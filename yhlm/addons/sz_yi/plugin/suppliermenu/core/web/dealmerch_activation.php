...
$set=unserialize($set);
$ratio=floatval($set['bart']['ratio']);

$tmerch=p('bonus')->getMerch($_W['uid']);
$tmember=m('member')->getMember($tmerch['openid']);
...
if ($op == 'direct'){
    ...
    if ($profile){ 
        $credit3=m('member')->getCredit($profile['openid'],'freeze_credit3'); 
        if (doubleval($credit3)<doubleval($_GPC['freeze_currency'])){ 
            show_json(0,'冻结易货码不足');             
        }

        $deduct=doubleval($_GPC['freeze_currency']) * $ratio;  
        ...
        $credit2=m('member')->getCredit($profile['openid'],'credit2'); 
        if (doubleval($credit2)<doubleval($deduct)){ 
            show_json(0,'余额不足，你当前余额剩余'.$deduct);                        
        }

        p('bonus')->calculateactive($profile['openid'],$deduct,$ordersn);                              
        m('member')->setCredit($profile['openid'],'credit2',-$deduct, array($profile['uid'], '商家后台会员激活'.doubleval($_GPC['freeze_currency']).'易货码'.'激活扣除余额手续费'.$deduct)); //激活易货码手续费               
        ...
        m('member')->setCredit($profile['openid'],'credit3',floatval($_GPC['freeze_currency']), array($profile['uid'], '商家后台会员解冻获得'.doubleval($_GPC['freeze_currency']).'易货码')); //获得易货码
        m('log')->putBarterCodeLog($profile['openid'],$profile['uid'],12,1,2,floatval($_GPC['freeze_currency']),$ordersn,'激活所得易货码');
        
        pdo_insert('sz_yi_currency_activation',$data);      
        
        //激活分成
        p('bonus')->calculateactive($profile['openid'],$deduct,pdo_insertid(),$ordersn);
