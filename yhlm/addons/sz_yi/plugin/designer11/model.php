...
if (!class_exists('DesignerModel')) {
    ...
    {
        public function getPage($type = 1,$supplier = 0)
        {
            global $_W, $_GPC;

            $page = pdo_fetch("SELECT * FROM " . tablename('sz_yi_designer') . " WHERE supplier = '{$supplier}'  and uniacid= :uniacid and pagetype=:type and setdefault=:default", array(
                ':uniacid' => $_W['uniacid'],
                ':type' => $type,
                ':default' => '1'
            ));
            ...
            return $this->getData($page,$supplier );
        }
        ...
        public function getData($page,$supplier = 0)
        {
            global $_W;
            ...
            if ($p[0]['params']['footer'] == 2) {
                ...
                $menu   = pdo_fetch('select * from ' . tablename('sz_yi_designer_menu') . ' where id=:id and uniacid=:uniacid and supplier = :supplier limit 1', array(
                    ':id' => $menuid,
                    ':uniacid' => $_W['uniacid'],
                    ':supplier' => $supplier
                ));
        ...
        {
            global $_W,$_GPC;
            return pdo_fetchcolumn('select id from ' . tablename('sz_yi_designer_menu') . ' where isdefault=1 and uniacid=:uniacid and supplier = :supplier limit 1', array(
                ':uniacid' => $_W['uniacid'],
                ':supplier' => empty($_GPC['supplier'])?0:$_GPC['supplier']
            ));
        }
        public function getDefaultMenu()
        {
            global $_W,$_GPC;
            return pdo_fetch('select * from ' . tablename('sz_yi_designer_menu') . ' where isdefault=1 and uniacid=:uniacid and supplier = :supplier limit 1', array(
                ':uniacid' => $_W['uniacid'],
                ':supplier' => empty($_GPC['supplier'])?0:$_GPC['supplier']
            ));
        }
