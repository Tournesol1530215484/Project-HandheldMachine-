...
if ($op == 'display') {
    ...
    $menus    = pdo_fetchall("SELECT * FROM " . tablename('sz_yi_designer_menu') . " WHERE uniacid= :uniacid and supplier = 0 and menuname LIKE :name " . "ORDER BY createtime DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, array(
        ':uniacid' => $_W['uniacid'],
        ':name' => "%{$kw}%"
    ));
    $menusnum = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('sz_yi_designer_menu') . " WHERE uniacid= :uniacid and supplier = 0  " . "ORDER BY createtime DESC ", array(
        ':uniacid' => $_W['uniacid']
    ));
    $total    = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('sz_yi_designer_menu') . " WHERE uniacid= :uniacid and supplier = 0  and menuname LIKE :name " . "ORDER BY createtime DESC ", array(
        ':uniacid' => $_W['uniacid'],
        ':name' => "%{$kw}%"
    ));
    ...
    if(!empty($_GPC['menuid'])){
           
        $menu   = pdo_fetch("SELECT * FROM " . tablename('sz_yi_designer_menu') . " WHERE uniacid= :uniacid and supplier = 0    and id = '{$_GPC['menuid']}' limit 1  ", array(
            ':uniacid' => $_W['uniacid'],
        ));
    }
    ...
    unset($m);
    $pages     = pdo_fetchall("SELECT id,pagename,pagetype,setdefault FROM " . tablename('sz_yi_designer') . " WHERE uniacid= :uniacid and supplier = 0   ", array(
        ':uniacid' => $_W['uniacid']
    ));
    ...
    $menu = pdo_fetch("SELECT * FROM " . tablename('sz_yi_designer_menu') . " WHERE uniacid= :uniacid and id=:id and supplier = 0  ", array(
        ':uniacid' => $_W['uniacid'],
        ':id' => $menuid
    ));
    if (empty($menu)) {
