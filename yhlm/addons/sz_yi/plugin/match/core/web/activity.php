...
if ($op == 'display') {
    ...
    $match['payitem']=pdo_fetchall('select * from '.tablename('sz_yi_match_payitem').' where enabled = 1 and  uniacid = :uniacid and atid = :id',array(':uniacid'=>$_W['uniacid'],':id'=>$id));
    ...
    if ($_W['isajax']) {                 
        ...
        if (empty($id)) {
            ...
            if ($id) {
        	    ...
        	    foreach ($payitem as $key => $value) {
                    pdo_insert('sz_yi_match_payitem',$value);
                }        
            	show_json(1,'添加成功');
        	}                      
        	show_json(0,'添加失败');
            ...
            if ($_GPC['ac'] == 'clone') {
                ...
                $pys=pdo_fetchall('select id from '.tablename('sz_yi_match_payitem').' where uniacid = :uniacid and atid = :id and enabled = 1',array(':uniacid'=>$_W['uniacid'],':id'=>$id));
                ...
                foreach ($payitem as $key => $value) {
                    ...
                    if ($value['id']) {   			 		 	 	 	 	 	 	 		 	  		 		 	 	 	 	 	  			 		 	 		 
                        ...
                        pdo_update('sz_yi_match_payitem',$tdata,array('id'=>$value['id']));
                    }else{ 		 
                    	$tdata=array(
                    		'uniacid'=>$_W['uniacid'],
                    		'title'=>$value['title'],
                    	...
                    	pdo_insert('sz_yi_match_payitem',$tdata);
                    } 	 	 	
                } 	 	 		 	 

              	foreach ($pys as $key => $value) {
              		pdo_update('sz_yi_match_payitem',array('enabled'=>0),array('id'=>$value,'uniacid'=>$_W['uniacid']));
              	} 	  		 	 	 	 	 	 	 	 	 	  	 		 		 		 	 	  		 		 	 	 	 
    ...
    $id=intval($_GPC['id']);         
    if ($id) {
        isetcookie('matchId',$id);
    }             
    header('Location:'.$this->createPluginWebUrl('matchba/match',array('op'=>'basic')));                    	 	 	   
    // $act=m('match')->getact($id);
}else if($op == 'team'){
    $id=intval($_GPC['id']); 
    $act=m('match')->getact($id);
