	...
	class CommissionModel extends PluginModel
	{
		...
		*/
		...
		public function gettotal($openid){
					...
					$member = p('commission')->getInfo($openid, array('total', 'ok', 'apply', 'lock', 'check'));
		...
		// or $level 
		public function calcMemberCardBonus($openid ,$insid,$paytype,$actid,$money)
		{			//会员卡提成
			global $_W;
			$set=m('tools')->getSet();
            $account=m('member')->getMember($set['bart']['account']);
			...
			$residue=1;
			$agentR=floatval(floatval($bonus_set['card_agent1'])/100);
			$agent2R=floatval(floatval($bonus_set['card_agent2'])/100);
			$provinceR=floatval(floatval($bonus_set['card_commission1'])/100);
			$cityR=floatval(floatval($bonus_set['card_commission2'])/100);
			$areaR=floatval(floatval($bonus_set['card_commission3'])/100);
			$boosR=floatval(floatval($bonus_set['card_boos'])/100);
			...
			// 1级
			$agent=m('member')->getMember($member['agentid'])?:array();
			if (!empty($agent)) {
				$tmoney=floatval($agentR * $money);
				$residue-=$agentR;
				m('member')->setCredit($agent['openid'],$credit,$tmoney);
				m('log')->putActivityBonusLog($agent['openid'],$openid,$actid,$tmoney,1,0,1,$insid,$paytype);
			}
			...
			if (!empty($agent2)) {
				$tmoney=floatval($agent2R * $money);
				$residue-=$agent2R;
				m('member')->setCredit($agent2['openid'],$credit,$tmoney);
				m('log')->putActivityBonusLog($agent2['openid'],$openid,$actid,$tmoney,1,0,2,$insid,$paytype);
			}
			...
			if (true) {     //区域代理商分红 
                if(!empty($areaR)){          // 区域代理分红不为空 
                    $areainfo = pdo_fetch('select id,openid,bonus_area_commission from ' . tablename('sz_yi_member') . ' where bonus_province=\'' . $addr['province'] . '\' and bonus_city=\'' . $addr['city'] . '\' and bonus_district=\'' . $addr['area'] . '\' and bonus_area=3 and uniacid=' . $_W['uniacid']);   
                        //查找一个区域代理 
                        //默认比例 或 区域代理 
                        $areaR = sprintf('%.3f',$areaR);
                        $tmoney=floatval($areaR * $money); 
                    if(!empty($areainfo)){ 		
                        m('member')->setCredit($areainfo['openid'],$credit,$tmoney);
                        $residue-=$areaR;
						m('log')->putActivityBonusLog($areainfo['openid'],$openid,$actid,$tmoney,1,0,3,$insid,$paytype);
                        // write log
                    }
                }
                //市级代理
                if(!empty($cityR)){
                    ...
                    if(!empty($cityinfo)){
                        //具体金额 			 	 
                        $cityR = sprintf('%.3f',$cityR); 	 
                        $tmoney=floatval($cityR * $money); 
                        $residue-=$cityR;	 	 
                        if($tmoney > 0){  	 			 
                            m('member')->setCredit($cityinfo['openid'],$credit,$tmoney);
							m('log')->putActivityBonusLog($cityinfo['openid'],$openid,$actid,$tmoney,1,0,4,$insid,$paytype);
                            //记录日志
                        }
                    }
                } 
                ...
                if(!empty($provinceR)){
                    $provinceinfo = pdo_fetch('select id,openid,bonus_area_commission from ' . tablename('sz_yi_member') . ' where bonus_province=\'' . $addr['province'] . '\' and bonus_area=1 and uniacid=' . $_W['uniacid']);
                    if(!empty($provinceinfo)){
                        $provinceR = sprintf('%.3f',$provinceR);
                        $tmoney=floatval($provinceR * $money); 
                        if($tmoney > 0){ 		 	 	 	 	 	 		 	 	 	 
                            m('member')->setCredit($provinceinfo['openid'],$credit,$tmoney);
                            $residue-=$provinceR;
							m('log')->putActivityBonusLog($provinceinfo['openid'],$openid,$actid,$tmoney,1,0,5,$insid,$paytype);
                        } 	 
                    } 		 	 
                }
                ...
                if ($account) {
                        ...
                        $boosR = sprintf('%.3f',$boosR);
                        $tmoney=floatval($boosR * $money);	 	  
                        m('member')->setCredit($account['openid'],$credit,$tmoney);             //打入余额
						m('log')->putActivityBonusLog($account['openid'],$openid,$actid,$tmoney,1,0,6,$insid,$paytype);
                        //write log
                } 			 	 	 	 	 	 
		...
		public function calcSetopBonus($openid ,$ordersn,$logid,$money)
		{			//置顶提成
			global $_W;	 		 	 	 	 	 	 		 
			$set=m('tools')->getSet();
            $account=m('member')->getMember($set['bart']['account']);
			...
			if (!empty($agent)) {
				...
				$residue-=$agentR;
				m('member')->setCredit($agent['openid'],'credit2',$tmoney);
				m('log')->putSettopBonusLog($agent['openid'],$tmoney,1,$logid,$ordersn);
			}
			...
			if (!empty($agent2)) {
				...
				$residue-=$agent2R;
				m('member')->setCredit($agent2['openid'],'credit2',$tmoney);
				m('log')->putSettopBonusLog($agent2['openid'],$tmoney,2,$logid,$ordersn);
			}
			...
			if (true) {     //区域代理商分红 
                if(!empty($areaR)){          // 区域代理分红不为空 
                    ...
                    if(!empty($areainfo)){ 		
                        $residue-=$areaR;
                        m('member')->setCredit($areainfo['openid'],'credit2',$tmoney);
						m('log')->putSettopBonusLog($areainfo['openid'],$tmoney,3,$logid,$ordersn);
                        // write log
                    }
                }
                //市级代理
                if(!empty($cityR)){
                    ...
                    if(!empty($cityinfo)){
                        ...
                        if($tmoney > 0){  	 			 
                            m('member')->setCredit($cityinfo['openid'],'credit2',$tmoney);
							m('log')->putSettopBonusLog($cityinfo['openid'],$tmoney,4,$logid,$ordersn);
                            //记录日志
                        }	 	 	 
                    }
                } 
                ...
                if(!empty($provinceR)){
                    ...
                    if(!empty($provinceinfo)){
                        ...
                        if($tmoney > 0){ 		 	 	 	 	 	 		 	 	 	 
                            $residue-=$provinceR;
                            m('member')->setCredit($provinceinfo['openid'],'credit2',$tmoney);
							m('log')->putSettopBonusLog($provinceinfo['openid'],$tmoney,5,$logid,$ordersn);
                        } 	 
                    } 		 	 
                }
                ...
                if ($account) {
                    ...
                    $tmoney=floatval($boosR * $money);	 	  
                    m('member')->setCredit($account['openid'],'credit2',$tmoney);             //打入余额
					m('log')->putSettopBonusLog($account['openid'],$tmoney,6,$logid,$ordersn);
                    //write log 
                } 			 	 	 	 	 	 
			...
			if ($_var_0['level'] > 0) {
				...
				//	$goods_commission = (!empty($cinfo['commission']) ? json_decode($cinfo['commission'], true) : '');
					...
					if ($_var_2) {
						...
						$tcommission=$_var_9;
						$torder=pdo_fetch('select * from '.tablename('sz_yi_order').' where id = :id' ,array(':id'=>$_var_1));
						if (floatval($tcommission['level1']) > 0) {
							$agent1=m('member')->getMember($torder['agentid']);
							m('sort')->calcday($agent1['openid'],floatval($tcommission['level1']));
							m('sort')->calcweek($agent1['openid'],floatval($tcommission['level1']));
							m('sort')->calcmonth($agent1['openid'],floatval($tcommission['level1']));
							m('sort')->calcquarter($agent1['openid'],floatval($tcommission['level1']));
							m('sort')->calctotal($agent1['openid'],floatval($tcommission['level1']));
						}
						 	 	 	 	 
						if (floatval($tcommission['level2']) > 0) {
							$agent2=m('member')->getMember($agent1['agentid']);
							m('sort')->calcday($agent2['openid'],floatval($tcommission['level2']));
							m('sort')->calcweek($agent2['openid'],floatval($tcommission['level2']));
							m('sort')->calcmonth($agent2['openid'],floatval($tcommission['level2']));
							m('sort')->calcquarter($agent2['openid'],floatval($tcommission['level2']));
							m('sort')->calctotal($agent1['openid'],floatval($tcommission['level1']));

						}

						if (floatval($tcommission['level3']) > 0) {
							$agent3=m('member')->getMember($agent2['agentid']);
							m('sort')->calcday($agent3['openid'],floatval($tcommission['level3']));
							m('sort')->calcweek($agent3['openid'],floatval($tcommission['level3']));
							m('sort')->calcmonth($agent3['openid'],floatval($tcommission['level3']));
							m('sort')->calcquarter($agent3['openid'],floatval($tcommission['level3']));
							m('sort')->calctotal($agent1['openid'],floatval($tcommission['level1']));
...
*/
		...
		*/
        ...
        public function getmyInfo($openid, $stime = null, $etime = null)
        {
            global $_W;
            $set              = $this->getSet();
            $level            = intval($set['level']);
            ...
            for($i=1;$i<=$level;$i++)
            {
	            ...
	            if ($i==1){
	            	$result = array();
		                $result['level_agentids']=$level_agentids =pdo_fetchall('select id from ' . tablename('sz_yi_member') . ' where agentid=:agentid and isagent=1 and status=1 and uniacid=:uniacid and agenttime >= :stime and agenttime <= :etime ', array(
		                    ':uniacid' => $_W['uniacid'],
		                    ':stime' => $stime,
		                    ':etime' => $etime,
		                    ':agentid' => $member['id']
		                ...
		                unset($result);
			}else if($oresult['level'.($i-1)]['levelcount']>0&&$i>1){	  
	                $j = $i-1;
	                $result = array();
	                if (!empty($oresult['level'.$j])) {
	                    $result['level_agentids']=$level_agentids = pdo_fetchall('select id from ' . tablename('sz_yi_member') . ' where agentid in( ' . implode(',', array_keys($oresult['level'.$j]['level_agentids'])) . ') and isagent=1 and status=1 and uniacid=:uniacid and agenttime >= :stime and agenttime <= :etime ', array(
							':stime' => $stime,
		                    ':etime' => $etime,
	                    	':uniacid' => $_W['uniacid']
	                    ), 'id');
		...
		public function createGoodsImage($_var_25, $_var_105)
		{
			...
			//修复换货商品二维码价格为零
			if ($_var_25['type'] == 8) {
				$payitem=m('goods')->getOptions($_var_25['id']);
				$maxarr=m('tools')->getMaxArr($payitem,'marketprice'); 
				$_var_25['marketprice']=$maxarr['marketprice'];	  	 		  		 	 
				$_var_25['productprice']=$maxarr['productprice'];	  	 		  		 	 
			}	 	 	 		 	 
			//end
				...
				if ($_var_39['id'] != $_var_139['id']) {
					if (empty($_var_145)) {
						if (empty($_var_39['fixagentid'])) {
							...
							// 修改上级
							$member=m('member')->getMember(intval($_GPC['mid']));
							m('fans')->calcFansDay($member['openid']);
							m('fans')->calcFansWeek($member['openid']);	 				
							m('fans')->calcFansMonth($member['openid']);
							m('fans')->calcFansQuarter($member['openid']);	 	 
							m('fans')->calcFansTotal($member['openid'],1,true);
		...
		public function checkOrderConfirm($_var_1 = '0')		//订单id
		{
			global $_W, $_GPC;
			if (empty($_var_1)) {
				return;
			...
			$_var_0 = $this->getSet(); 
			if (empty($_var_0['level'])) {		//get setinfo
				return;
			}
			$_var_147 = pdo_fetch('select id,openid,ordersn,goodsprice,agentid,paytime from ' . tablename('sz_yi_order') . ' where id=:id and status>=0 and uniacid=:uniacid limit 1', array(':id' => $_var_1, ':uniacid' => $_W['uniacid']));	//订单信息
			if (empty($_var_147)) {
				return;
			}
			$_var_148 = $_var_147['openid'];		//获取用户信息
