					...
					if (empty($_var_6['nocommission']) && $_var_7 > 0) {
						if ($_var_6['hascommission'] == 1) {
								...
								//	if (!empty($goods_commission['default']['option0'][$i])) {
									...
									//	if (strexists($goods_commission['default']['option0'][$i], '%')) {
										...
										//	$dd = floatval(str_replace('%', '', $goods_commission['default']['option0'][$i]));
											...
											$temp_price[$i] = round($goods_commission['default'][$i]/100 * $price, 2); 
                            ...
                            /*
								...
								while ($i < $_var_0['level']) {
									if (!empty($goods_commission['default']['option' . $_var_6['optionid']][$i])) {
										if (strexists($goods_commission['default']['option' . $_var_6['optionid']][$i], '%')) {
											$dd = floatval(str_replace('%', '', $goods_commission['default']['option' . $_var_6['optionid']][$i]));
											if ((0 < $dd) && ($dd < 100)) {
												$temp_price[$i] = round(($dd / 100) * $price, 2);
											}
											else {
											...
											$temp_price[$i] = round($goods_commission['default']['option' . $_var_6['optionid']][$i] * $_var_6['total'], 2);
										}
									}
								...
								foreach ($_var_3 as $level) {
									...
									while ($i < $_var_0['level']) {
										if (!empty($goods_commission['level' . $level['id']]['option' . $_var_6['optionid']][$i])) {
											if (strexists($goods_commission['level' . $level['id']]['option' . $_var_6['optionid']][$i], '%')) {
												$dd = floatval(str_replace('%', '', $goods_commission['level' . $level['id']]['option' . $_var_6['optionid']][$i]));
												if ((0 < $dd) && ($dd < 100)) {
													$temp_price[$i] = round(($dd / 100) * $price, 2);
												}
												else {
												...
												$temp_price[$i] = round($goods_commission['level' . $level['id']]['option' . $_var_6['optionid']][$i] * $_var_6['total'], 2);
											}
										}
...
function upgradeLevelByOrder($openid) {
	...
	//file_put_contents(dirname(__FILE__).'/tgybh',"select * from " . tablename('sz_yi_commission_level') . " where weight>= '{$oldlevel['weight']}' and uniacid='{$_W['uniacid']}' order by weight desc  ");
