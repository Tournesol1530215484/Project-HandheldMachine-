...
if (!class_exists('CommissionModel')) {
	class CommissionModel extends PluginModel
	{
		...
		*/
		...
		public function calcMemberCardBonus($openid ,$ordersn,$actid,$money)
		{			//会员卡提成
			global $_W;
			$set=m('tools')->getSet();
            $account=m('member')->getMember($set['bart']['account']);
			$residue=1;
			$agentR=floatval(30/100);
			$agent2R=floatval(10/100);
			$areaR=floatval(20/100);
			$cityR=floatval(10/100);
			$provinceR=floatval(5/100);
			$boosR=floatval(25/100);
			...
			if (!empty($agent)) {
				...
				m('log')->putActivityBonusLog($agent['openid'],$openid,$actid,$tmoney,1,0,1);
			}
			...
			if (!empty($agent2)) {
				...
				m('log')->putActivityBonusLog($agent2['openid'],$openid,$actid,$tmoney,1,0,2);
			}
			...
			if (true) {     //区域代理商分红 
                if(!empty($areaR)){          // 区域代理分红不为空 
                    $areainfo = pdo_fetch('select id,openid,bonus_area_commission from ' . tablename('sz_yi_member') . ' where bonus_province=\'' . $addr['province'] . '\' and bonus_city=\'' . $addr['city'] . '\' and bonus_district=\'' . $addr['area'] . '\' and bonus_area=3 and uniacid=' . $_W['uniacid']);   
                        //查找一个区域代理 
                        //默认比例 或 区域代理 
                        $tmoney = sprintf('%.3f',$areaR); 
                    if(!empty($areainfo)){
                        m('member')->setCredit($areainfo['openid'],'credit2',$tmoney);
                        $residue-=$areaR;
						m('log')->putActivityBonusLog($areainfo['openid'],$openid,$actid,$tmoney,1,0,3);
                        // write log
                    }
                }
                //市级代理
                if(!empty($cityRcityR)){
                    ...
                    if(!empty($cityinfo)){
                        //具体金额
                        $tmoney = sprintf('%.3f',$cityR);
                        $residue-=$cityR;
                        if($tmoney > 0){ 		 
                            m('member')->setCredit($cityinfo['openid'],'credit2',$tmoney);
							m('log')->putActivityBonusLog($areainfo['openid'],$openid,$actid,$tmoney,1,0,4);
                            //记录日志
                        }
                    }
                } 
                ...
                if(!empty($provinceR)){
                    ...
                    if(!empty($dephp_33)){
                        $tmoney = sprintf('%.3f',$provinceR);
                        if($tmoney > 0){
                            m('member')->setCredit($provinceinfo['openid'],'credit2',$tmoney);
                            $residue-=$provinceR;
							m('log')->putActivityBonusLog($provinceinfo['openid'],$openid,$actid,$tmoney,1,0,5);
                        } 	 
                    } 		
                }
                ...
                if ($account) {
                        ...
                        $tmoney = sprintf('%.3f',$boosR);
                        m('member')->setCredit($account['openid'],'credit2',$tmoney);             //打入余额
						m('log')->putActivityBonusLog($account['openid'],$openid,$actid,$tmoney,1,0,6);
                        //write log
                }
		...
		public function calcReward($openid ,$type,$actid,$money)
		{			//打赏
                    //type 1 活动 2 文章
            ...
            $account=m('member')->getMember($set['bart']['account']);
			$residue=1;
			$consumersR=floatval(60/100);        //自己
			$agentR=floatval(5/100);             //上级
			$agent2R=floatval(5/100);            //上上级
			$areaR=floatval(6/100);              //区域代理
			$cityR=floatval(3/100);              //市级代理
			$provinceR=floatval(1/100);          //省
			$boosR=floatval(20/100);             //平台

			$member=m('member')->getMember($openid);
			$tmoney=floatval($consumersR * $money);
			$residue-=$consumersR;
			m('member')->setCredit($member['openid'],'credit2',$tmoney);
			$logid=m('log')->putActivityBonusLog($member['openid'],$openid,$actid,$tmoney,2,$type,0);
			...
			if (!empty($agent)) {
				...
				$residue-=$agentR;
				m('member')->setCredit($agent['openid'],'credit2',$tmoney);
				m('log')->putActivityBonusLog($agent['openid'],$openid,$actid,$tmoney,2,$type,1,$logid);
			}
			...
			if (!empty($agent2)) {
				...
				$residue-=$agent2R;
				m('member')->setCredit($agent2['openid'],'credit2',$tmoney);
				m('log')->putActivityBonusLog($agent2['openid'],$openid,$actid,$tmoney,2,$type,2,$logid);
			}
			...
			if (true) {     //区域代理商分红 
                ...
                if(!empty($areaR)){          // 区域代理分红不为空 
                    ...
                    if(!empty($areainfo)){
                        ...
                        $residue-=$areaR;
                        m('member')->setCredit($areainfo['openid'],'credit2',$tmoney);
						m('log')->putActivityBonusLog($areainfo['openid'],$openid,$actid,$tmoney,2,$type,3,$logid);
                        // write log
                    }
                }
                //市级代理
                if(!empty($cityR)){
                    ...
                    if(!empty($cityinfo)){
                        ...
                        $residue-=$cityR;
                        m('member')->setCredit($cityinfo['openid'],'credit2',$tmoney);
						m('log')->putActivityBonusLog($areainfo['openid'],$openid,$actid,$tmoney,2,$type,4,$logid);
                        //记录日志
                    }
                } 
                ...
                if(!empty($provinceR)){
                    ...
                    if(!empty($provinceinfo)){
                        ...
                        $residue-=$provinceR;
                        m('member')->setCredit($provinceinfo['openid'],'credit2',$tmoney);
						m('log')->putActivityBonusLog($provinceinfo['openid'],$openid,$actid,$tmoney,2,$type,5,$logid);
                    } 		
                }
                ...
                if ($account) {
                        ...
                        $tmoney = sprintf('%.3f',$boosR);
                        m('member')->setCredit($account['openid'],'credit2',$tmoney);             //打入余额
						m('log')->putActivityBonusLog($account['openid'],$openid,$actid,$tmoney,2,$type,6,$logid);
                        //write log
                }
