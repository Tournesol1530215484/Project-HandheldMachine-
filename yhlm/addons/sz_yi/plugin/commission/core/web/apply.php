...
if ($operation == 'display') {
	...
	foreach ($list as &$row) {
		...
		foreach ($goods as &$g) {
			...
			for($i=1;$i<=$this->set['level'];$i++){

                $commission       = iunserializer($g['commission'.$i]);

				//$agentLevel = $this->model->getLevel($apply['mid'],$g['createtime']);
	...
	// 佣金提现手续费 2016年11月29日 15:46:51
	$poundage = $totalpay * $this->set['scale'] / 100;
	$totalpay -= $poundage;
	// 佣金提现手续费 2016年11月29日 15:46:51
	unset($row);
	...
	if (checksubmit('submit_check') && $apply['status'] == 1) {
		...
		foreach ($ogids as $ogid) {
			...
			for($i=1;$i<=$this->set['level'];$i++){
	            ...
	            if (isset($_GPC['status'.$i][$ogid])) {

	                if (intval($_GPC['status'.$i][$ogid]) == 2) {

	                    $paycommission += $g['commission'.$i];
	                ...
	                $update = array(
	                    ...
	                    'status'.$i => intval($_GPC['status'.$i][$ogid]),

	                    'content'.$i => $_GPC['content'.$i][$ogid]
		...
		if ($isAllUncheck) {
			...
			pdo_update('sz_yi_commission_apply', array(
...
if (checksubmit('submit_pay') && $apply['status'] == 2) {
    ...
    for ($i=1; $i <=15 ; $i++) { 
         ...
         $arr[] = " og.commission{$i} , og.status{$i} ";
    ...
    */
	...
	$order_goods_change = pdo_fetchall("select og.id,g.title,g.thumb,g.goodssn,og.goodssn as option_goodssn, g.productsn,og.productsn as option_productsn, og.total,og.price,og.optionname as optiontitle, og.realprice,og.oldprice,  {$arr} ,og.commissions from " . tablename('sz_yi_order_goods') . ' og ' . ' left join ' . tablename('sz_yi_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $id));
...
/*
	...
	if (!empty($cm1['agentid'])) {$cm2 = m('member')->getMember($cm1['agentid']);}

	if (!empty($cm2['agentid'])) {$cm3 = m('member')->getMember($cm2['agentid']);}
	...
	for ($i=4; $i <=15 ; $i++) { 
		...
		// $b = 'cm'.$i;
		 ...
		 if(!empty($GLOBALS['cm'.($i-1)]['agentid'])){



		 	 $GLOBALS['cm'.$i] = m('member')->getMember($GLOBALS['cm'.($i-1)]['agentid']);
...
*/
	...
	$cm[1]= m('member')->getMember($agentid);

	if (!empty($cm[1]['agentid'])) {$cm[2] = m('member')->getMember($cm[1]['agentid']);}

	if (!empty($cm[2]['agentid'])) {$cm[3] = m('member')->getMember($cm[2]['agentid']);}
	...
	for ($i=4; $i <=15 ; $i++) { 
		...
		// $b = 'cm'.$i;
		 ...
		 if(!empty($cm[$i-1]['agentid'])){



		 	 $cm[$i] = m('member')->getMember($cm[$i-1]['agentid']);
	...
	foreach ($order_goods_change as &$og) {
        ...
        for ($i=4; $i<=15 ; $i++) { 
		 		...
		 		if ($set['level'] >= $i) {
					...
					$commission = iunserializer($og['commission'.$i]);

					if (empty($commissions)) {

						$og['c'.$i] = isset($commission['level' . $agentLevel['id']]) ? $commission['level' . $agentLevel['id']] : $commission['default'];

					} else {

						$og['c'.$i] = isset($commissions['level'.$i]) ? floatval($commissions['level'.$i]) : 0;
