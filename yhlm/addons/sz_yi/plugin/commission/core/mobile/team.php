...
if($_GET['op'] == 'zixun'){
	$user = m('member')->getMember($openid);
	$user2 = m('member')->getMember($_GET['id']);
	$sender = $_GET['sender'];			//发送方
	if(empty($user) or empty($user2) or empty($sender) or $user['id'] == $_GET['id']){
		die('参数错误，请联系管理员');
	}
	if($sender == 'superior'){
		$superior_id  = $user['id'];		//当前访问的id
		$lower_id     = $_GET['id'];		//对方的id
		$superior_img = $user['avatar'];
		$lower_img    = $user2['avatar'];
		$msg_sender   = 'lower';
		$bzd          = '客户';
	}elseif($sender == 'lower'){
		$superior_id  = $_GET['id'];		//对方id 
		$lower_id     = $user['id'];		//当前访问的id
		$superior_img = $user2['avatar'];
		$lower_img    = $user['avatar'];
		$msg_sender   = 'superior';
		$bzd          = '店铺';
	...
	if($_POST['msg']){
		...
		if($sender == 'superior'){
			...
			$merchinfo=pdo_fetch('select * from '.tablename('sz_yi_perm_user').' where uniacid = :uniacid and openid = :openid',array(':uniacid'=>$_W['uniacid'],':openid'=>$openid));
			$template = array(	 
				'keyword1' => array(	 		 
					'title' => '您有新的咨询消息', 
					'value' => '您有来自'.$bzd.'['.mb_substr($merchinfo['username'],2,mb_strlen($merchinfo['username'],'utf8')-2,'utf8').']的未读消息'
					// 'value' => '您有来自'.$bzd.'['.$user['nickname'].']的未读消息'
				)
			);
		}
		$url = $this->createPluginMobileUrl('commission/team',array('op'=>'zixun','id'=>$user['id'],'sender'=>$msg_sender ));
		m('message')->sendCustomNotice($user2['openid'], $template, $url);
