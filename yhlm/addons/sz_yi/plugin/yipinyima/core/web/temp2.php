...
if ($operation == 'display') {
    ...
    $items = pdo_fetchall("SELECT id,yi_goodsid,shuliang,pihao,roleid,commjssionid,jifen,uniacid,start,end FROM " . tablename('yipinyima') .
 " where  yi_goodsid=".$_GPC['goodsid']." and  uniacid=".$_W['uniacid']."");
   /*$umss = pdo_fetchall("SELECT * FROM " . tablename('yipinyima') . " d " .
    ...
    if (empty($id)) {
        ...
        plog('yipinyima.temp.edit', "添加一品一码 ID: {$id}");
    } else {
