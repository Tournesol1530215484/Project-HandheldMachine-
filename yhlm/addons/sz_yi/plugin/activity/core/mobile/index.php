...
if ($op == 'display') {
	...
	$sg=pdo_fetchcolumn('select data from '.tablename('sz_yi_activity_signup').' where uniacid = :uniacid and openid = :openid order by id desc',array(':uniacid'=>$_W['uniacid'],':openid'=>$openid));
	$data=unserialize($sg);
	
	$accountinfo=m('activity')->getAccountInfo($openid);
	...
	if (!$accountinfo) {			//防止分表信息丢失
		...
		if ($actmerch) {
			...
			pdo_insert('sz_yi_activity_account_info',$data);
	...
	if ($ac == 'sub') {
		...
		if ($what == 'info') {
			...
			$re=pdo_update('sz_yi_activity_account_info',$data,array('uniacid'=>$_W['uniacid'],'openid'=>$openid));
	...
	if ($type == 1) {
		$money=floatval($actset['level1']);
		$str='初级会员';
	}else if($type == 2){
		$money=floatval($actset['level2']);
		$str='中级会员'; 	 	
	}else if($type ==3){	  
		$money=floatval($actset['level3']);
		$str='高级会员';
	}else{
		m('tools')->tip('没有这个等级!');
	}	 	 	 
	...
	if ($ac == 'sub' && $_W['isajax']) {
		...
		if ($re) {
			p('commission')->calcMemberCardBonus($openid,pdo_insertid(),$log['paytype'],$type,floatval($money)); 		 	 	 	 		 	 		 		
	...
	$ordersn=m('common')->createNO('activity', 'is_top', 'st');
	// $obj=$type==1 ? '活动':'文章';
	if ($type == 1) {
 		$obj='活动';
	...
	// $table=$type==1 ? 'sz_yi_activity':'sz_yi_activity_article';	 	 
	...
	if ($_W['isajax']) {
		...
		if ($ac == 'check') {
			...
			if ($re) {	 	 	 	 
				...
				pdo_insert('sz_yi_activity_settop_log',$log);		//记录日志
	...
	show_json(0,'评论失败');
}else if($op == 'sgcomment'){
    $id=intval($_GPC['id']);
    $sgid=intval($_GPC['sgid']);
    $time=m('time')->today();
    $num=pdo_fetchcolumn('select count(*) from '.tablename('sz_yi_match_comment').' where uniacid = :uniacid and atid = :id and openid = :openid and ctime < :stime and ctime > :etime ',array(':uniacid'=>$_W['uniacid'],':openid'=>$openid,':id'=>$id,':stime'=>$time[0],':etime'=>$time[1]));
    if ($num >= 5) {
        show_json(0,'你今天已经评论过五次了!');
    }
    empty($id) && show_json(0,'非法参数');
    ...
    $exists=pdo_fetch('select * from '.tablename('sz_yi_match_like').' where uniacid = :uniacid and openid = :openid and atid = :atid and sgid = :sgid and ctime > :stime and ctime < :etime ',$params);
    ...
    if ($lid) {
        $member=m('member')->getMember($openid);
        // $act=m('activity')->getact($id,5);
        $act=m('match')->getMatchPlayer($id,$sgid);
        pdo_update('sz_yi_match_signup',array('like'=>$act['like']+1),array('id'=>$act['id'],'uniacid'=>$_W['uniacid']));
        show_json(1,array('member'=>$member));
    }
