...
// }else if($op == 'test'){
	...
	if ($_W['isajax']) {
		...
		//获取用户头像
		// $tmuser=m('sz_yi_member_user')->getMuser($openid);
		//根据openid获取所属机构，然后调出机构ID:uniacid'=>$_W['uniacid']
		 // $uid='select uid from'.tablename('sz_yi_member').'where openid=:'.$openid.'and uniacid=:'.$_W['uniacid'].'';
		//$accountinfo=pdo_fetch('select * from '.tablename('sz_yi_activity_account_info').' where uniacid = :uniacid and openid = :openid ',array(':uniacid'=>$_W['uniacid'],':openid'=>$openid));
		 // $userid=pdo_query($uid,$params);
		 // $sqll='select logo,orgDesc from '.tablename('sz_yi_member_user').' uniacid=:'.$_W['uniacid'].'';
		 //  $tmuser=pdo_query($sqll,$params);
		if ($type == 'act') {
