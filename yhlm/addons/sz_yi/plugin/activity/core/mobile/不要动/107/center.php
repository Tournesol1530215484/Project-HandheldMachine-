...
if ($op == 'display') {
    ...
    $totals=pdo_fetchcolumn('select count(*) from '.tablename('sz_yi_activity_signin').' where uniacid = :uniacid ',array(':uniacid'=>$_W['uniacid']));
    ...
    $max=pdo_fetchcolumn('select continuous from '.tablename('sz_yi_activity_signin').' where uniacid = :uniacid and openid = :openid and date = :date',array(':uniacid'=>$_W['uniacid'],':openid'=>$heMember['openid'],':date'=>date('Ymd')));
    ...
    $minfo=m('member')->getMember($openid);
        ...
        $pt=m('tools')->createCardPoster2($minfo['openid'],$posterid,$this->createPluginMobileUrl('activity/article',array('op'=>'detail','id'=>$id,'mid'=>$minfo['id'])));
    ...
    // http://jhzh66.com/app/index.php?i=8&c=entry&method=activity&p=activity&op=apply&id=260&m=sz_yi&do=plugin

    !$act && m('tools')->tips('你还没有报名或该报名审核没有通过',$this->createPluginMobileUrl('activity/activity',array('op'=>'apply','id'=>$atid)));
    ...
    $activity=m('activity')->getact($act['actid']);
    ...
    $actitem=pdo_fetch('select * from '.tablename('sz_yi_activity_payitem').' where uniacid = :uniacid  and id = :id',array(':uniacid'=>$_W['uniacid'],':id'=>$act['item']));
    ...
    $url=$this->createPluginMobileUrl('activity/activity',array('op'=>'detail','id'=>$atid));
    ...
    $activity=m('activity')->getact($atid);
    ...
    $activity['stime'] > time() || $activity['etime'] < time() && m('tools')->tip('活动未开始或已结束,无法参加！');



    $returnurl=$this->createPluginMobileUrl('activity/center',array('op'=>'join','id'=>$atid));

    if ($activity['afterTheSignup']) {

        // http://jhzh66.com/app/index.php?i=8&c=entry&method=activity&p=activity&op=apply&id=260&m=sz_yi&do=plugin

        !$act && m('tools')->tips('你还没有报名，正在跳转到报名页面',$this->createPluginMobileUrl('activity/activity',array('op'=>'apply','id'=>$atid,'returnurl'=>$returnurl)));
    ...
    $exists=pdo_fetch('select * from '.tablename('sz_yi_activity_join_status').' where uniacid = :uniacid and openid = :openid',array(':uniacid'=>$_W['uniacid'],':openid'=>$openid));
    ...
    if ($exists) {
        ...
        $re=pdo_update('sz_yi_activity_join_status',$joinlog,array('openid'=>$openid,'uniacid'=>$_W['uniacid']));

    }else{

        pdo_insert('sz_yi_activity_join_status',$joinlog);      //账号首次参加
    ...
    $url=$this->createPluginMobileUrl('activity/activity',array('op'=>'detail','id'=>$_GPC['actid']));
    ...
    if (is_weixin()) {
        ...
        m('tools')->tips($sstr,$this->createPluginMobileUrl('activity/activity',array('op'=>'detail','id'=>$atid)));
    ...
    $buildtype = intval($_GPC['buildtype'])?:1; //支付后的后续操作类型标识
    ...
    $url2 = $this->createPluginMobileUrl('activity/activity',array('op'=>'org','openid'=>$activity['openid']));
    ...
    if ($_W['isajax']) {
        ...
        if ($ac == 'get') {
            ...
            if ($status == 1) {
                ...
                // $sql='select count(*) num,atid mid from '.tablename('sz_yi_activity_like').' where uniacid = :uniacid and type = 3 and ctime > :stime and ctime < :etime group by atid order by num desc ';
                ...
                /*if ($_GPC['time'] != 4) {
                    ...
                    $sql='select * from '.tablename('sz_yi_member').' where uniacid = :uniacid ';
                ...
                if (intval($_GPC['time']) != 4) {

                    $sql='select * from '.tablename('sz_yi_fans_sort_log').' where uniacid = :uniacid and utime >= :stime and utime <= :etime ';



                }else{

                    $sql='select * from '.tablename('sz_yi_fans_sort_log').' where uniacid = :uniacid ';
            ...
            else if($status == 6) {     //获取换货商家排行榜

                $sql='select sum(price) price,supplier_uid from '.tablename('sz_yi_goods').' where uniacid = :uniacid and status > 0 and isCheck = 1 and createtime > :stime and createtime < :etime group by supplier_uid order by price desc ';
                ...
                if ($list) {
                    ...
                    foreach ($list as $key => $value) {
                        //productprice
                        //$tminfo=m('member')->getMember($value['openid']);
                        $openid=pdo_fetchall("select openid from hs_sz_yi_af_supplier where id ='{$value['id']}'");
                        $tminfo=m('member')->getMember($openid[0]);
