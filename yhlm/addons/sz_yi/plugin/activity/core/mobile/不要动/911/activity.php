...
//        print_r($_GPC);exit;
		...
		if($list){
			...
			foreach ($list as $key => $value) {
				...
				if((3<=$value['is_top']) && $value['is_top']<=4){	//置顶
					$arrayTop[]=$list[$key];
				}else if($list [$key]['stime']>$todaytime){
					$arrayNow[] = $list[$key]; 
				}else{
			...
			$sort='SORT_ASC';
			$field='stime';
		    foreach($arrayNow as $uniqid => $row){
		        foreach($row as $key=>$value){
		            $arrSort[$key][$uniqid] = $value;
		        }
		    }
		    if($sort){
		        array_multisort($arrSort[$field], constant($sort), $arrayNow);
		    }

			//$arrayNow=array_multisort($arrayNow,'stime asc');
			unset($list);
