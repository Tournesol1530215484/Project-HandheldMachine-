...
//        print_r($_GPC);exit;
		...
		$sql.=$condition;
		//$sql.='order by is_top desc, toptime asc , stime desc ,etime asc';
		//$sql.=' order by is_top desc, toptime asc ,(case when stime >= $time than stime asc end),etime asc';
        ...
        print_r($list);
		// if ($list) {
		// 	$arrayTop=array();	//置顶部分的活动
		// 	$arrayNow=array();	//还可以报名的活动
		// 	$arrayEnd=array();	//已经结束了的活动

		// 	foreach ($list as $key => $value) {	//数组进行重组’
		// 		//活动置顶部分
		// 		if($list[$key][$value]['is_top']){	//置顶
		// 			$arrayTop[]=$list[$key][$value];
		// 		}else if($list[$key][$value]['stime']>time()){	//活动还未开始，根据开始时间正序排
		// 			$arrayNow[]=$list[$key][$value];
		// 		}else{								//活动已经开始，根据开始时间倒叙
		// 			$arrayEnd[]=$list[$key][$value];
		// 		}
		...
		// }
		// if(!empty($arrayNow)){	//排序
		// 	foreach ($arrayNow as $key => $value) {
		// 		$arrayNow[$key][$value['stime']]=$value;
		// 	}
		// }
		// if(!empty($arrayEnd)){	//倒叙
		...
		//var_dump(1111);
		 ...
		 //$list=$arrayTop+$arrayNow+$arrayEnd;
		 //print_r($list);
