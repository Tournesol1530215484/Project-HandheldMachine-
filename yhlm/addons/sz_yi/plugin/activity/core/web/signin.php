...
if ($op == 'display') {
    ...
    $sql='select * from '.tablename('sz_yi_activity_signin').' where 1 '.$condition;
    $sql.=' order by ctime desc ';
    $sql .= ' LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;       
    
    $list = pdo_fetchall($sql, $params);

    $totals = pdo_fetchcolumn("select count(*) from " . tablename('sz_yi_activity_signin'). " where 1 {$condition} ", $params);
    ...
    !$muser && message('你还没有注册!请先注册!',referer(),'error');
        ...
        $totals=pdo_fetchcolumn('select count(*) '.$field.' from '.tablename('sz_yi_activity_comment').' ac left join '.tablename('sz_yi_member').' m on m.openid = ac.openid '.$extsql.' where 1 '.$condition,$params);                     
        $pager = pagination($totals, $pindex, $psize);
