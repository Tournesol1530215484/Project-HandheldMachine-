...
if ($op == 'display') {
    ...
    $sql='select s.*,a.title from '.tablename('sz_yi_activity_signup').' s left join '.tablename('sz_yi_activity').' a on s.actid = a.id where a.uniacid = :uniacid and a.id = :id order by ctime desc';
    ...
    $lengths=count($list);
    $lists=array_slice($list, 5,$lengths);
    ...
    //报名信息导出
    $sqls='select s.*,a.title from '.tablename('sz_yi_activity_signup').' s left join '.tablename('sz_yi_activity').' a on s.actid = a.id where a.uniacid = :uniacid and a.id = :id order by ctime desc';         
    ...
    $listssss=pdo_fetchall($sqls,$params); 
    ...
    $openIdArray = array();
    foreach ($listssss as $key =>&$value) {

        $tinfo=unserialize($value['data']);

        if ($_GPC['export'] ==1) {

            $listssss[$key]['realname']=$tinfo['realname']['data'];

            $listssss[$key]['mobile']=$tinfo['mobile']['data'];

            $listssss[$key]['unit']=$tinfo['unit']['data'];
...
//            数据长度不是固定的
            ...
            $listssss[$key]=array_merge($value,$tinfo);
        }
    ...
    if ($_GPC['export'] == '1') {
        ...
        foreach ($listssss as $key => $value) {
            ...
            $listssss[$key]['ctime']=date('Y-m-d H:i:s',$value['ctime']);

            if ($value['status'] == 0){

                $listssss[$key]['status']='待审核';

            }else if ($value['status'] == 1){

                $listssss[$key]['status']='通过';

            }else if ($value['status'] == 2){

                $listssss[$key]['status']='驳回';
            ...
            if ($value['signin'] > 0) {
                ...
                $listssss[$key]['signin']='已签到';

            }else{

                $listssss[$key]['signin']='未签到';
    ...
    $condition='';  
     ...
     if ($_GPC['realname']) {
        ...
        $condition .= ' and data like :realname';

        $params[':data'] = "%{$_GPC['realname']}%";

    }     

    $sql='select s.*,a.title from '.tablename('sz_yi_activity_signup').' s left join '.tablename('sz_yi_activity').' a on s.actid = a.id where   a.uniacid = :uniacid and a.id = :id and s.signin > 0'.$condition.' order by ctime desc ';
   ...
   // $listssss=$list;    
    ...
    $sqls='select s.*,a.title from '.tablename('sz_yi_activity_signup').' s left join '.tablename('sz_yi_activity').' a on s.actid = a.id where   a.uniacid = :uniacid and a.id = :id and s.signin > 0'.$condition.' order by ctime desc ';        
...
//            数据长度不是固定的
            foreach ($tinfo as $index =>&$value2){
                if(is_numeric($index)){
                    $field = $value2['field'];
                    $listssss[$key][$field] = $value2['data'];
                    if($key==10){
                        $columns[] = array('title'=>$value2['title'],'field'=>$field,'width'=>15);
                    }
                }
