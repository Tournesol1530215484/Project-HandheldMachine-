...
if (empty($type)){
   ...
   //suppliermenu
  header('location: ' . $this -> createPluginWebUrl('suppliermenu/exhelper',array('ac'=>'express')));
    die;
}
if (p('supplier')){
...
if ($op == 'search'){
    ...
    if ($type == 1){
        ...
        include $this -> template('exhelper/print_tpl_detail');
    }
    die;
}elseif ($op == 'detail'){
...
load() -> func('tpl');
include $this -> template('exhelper/doprint');
