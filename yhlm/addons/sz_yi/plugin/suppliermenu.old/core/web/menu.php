...
if ($op == 'display') {
 //   ca('designer.menu.view');
    ...
    $menus    = pdo_fetchall("SELECT * FROM " . tablename('sz_yi_designer_menu') . " WHERE uniacid= :uniacid and supplier = '{$_W['uid']}' and menuname LIKE :name " . "ORDER BY createtime DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, array(
        ':uniacid' => $_W['uniacid'],
        ':name' => "%{$kw}%"
    ));
    $menusnum = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('sz_yi_designer_menu') . " WHERE uniacid= :uniacid  and supplier = '{$_W['uid']}' " . "ORDER BY createtime DESC ", array(
        ':uniacid' => $_W['uniacid']
    ));
    $total    = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('sz_yi_designer_menu') . " WHERE uniacid= :uniacid and supplier = '{$_W['uid']}' and menuname LIKE :name " . "ORDER BY createtime DESC ", array(
        ':uniacid' => $_W['uniacid'],
        ':name' => "%{$kw}%"
    ));
    ...
    if(!empty($_GPC['menuid'])){
        ...
        $menu   = pdo_fetch("SELECT * FROM " . tablename('sz_yi_designer_menu') . " WHERE uniacid= :uniacid and supplier = '{$_W['uid']}' and id = '{$_GPC['menuid']}' limit 1  ", array(
            ':uniacid' => $_W['uniacid'],
        ));
    }
    ...
    $menus  = array(
        array(
            ...
            "subMenus" => array(
                array(
                    'title' => '商城首页',
                    'url' => $this->createMobileUrl('shop',array('supplier'=>$_W['uid']))
                )
            )
        )
    );
