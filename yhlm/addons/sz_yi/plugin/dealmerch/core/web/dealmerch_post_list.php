...
if ($operation == 'display') {
	ca('dealmerch.dealmerch_post_list');
	// 7天自动收货
	...
	$condition = ' o.uniacid = :uniacid and o.deleted=0 and o.isexchange = 1 and o.storeid = 0 and o.addressid > 0 ';
	...
	if (p('supplier')) {
		$cond = " and  o.isexchange = 1 and o.storeid = 0 and o.addressid > 0 ";
		if ($perm_role == 1) {
...
//    $arr=array('SH201903281006482824',
//        'SH201903270848128842',
//        'SH201903270846048878',
//        'SH201903261047290876',
//        'SH201903241932003838',
//        'SH201903241842225682',
//        'SH201903230845044256',
//        'SH201903230835298769',
//        'SH201903230832576865',
//        'SH201903222224305858',
//        'SH201903210928272628',
//        'SH201903181531484242',
//        'SH201903162313122846',
//        'SH201903122219536868',
//        'SH201903120023145146',
//        'SH201903092232376260',
//        'SH201903092237356317',
//        'SH201903092237356368',
//        'SH201903092237354308',
//        'SH201903092237356424',
//        'SH201903090742376661',
//        'SH201903021926277226',
//
//    );
...
//                       print_r($v['supplier_uid']);
//                       print_r($seller);exit;
//                       $currency_credit3 = m('member')->getCredit($seller, 'currency_credit3');
//
////                       if (floatval($currency_credit3) >= floatval($v['price'])) {
////                           m('member')->setCredit($seller, 'currency_credit3', -floatval($v['price']));    //卖家获得易货码 冻结 如果卖家存在易货额度将自动使用易货额度激活
//                           m('member')->setCredit($seller, 'credit3', floatval($v['price']));    //卖家获得易货码 已使用易货额度自动激活
////                           m('log')->putBarterCurrencyLog($seller, $v['supplier_uid'], 11, -floatval($v['price']), $value['ordersn'], '销售易货码自动解冻');
////                       } else {
////                           m('member')->setCredit($seller, 'freeze_credit3', floatval($v['price']));    //卖家获得易货码 冻结 如果卖家存在易货额度将自动使用易货额度激活
////                       }
//                       m('log')->putBarterCodeLog($seller, $v['supplier_uid'], 2, 1, 1, floatval($v['price']), $value['ordersn'], '销售收入');
//                   }
////               }
//           }
//        }
		if($value['status']==2){
			$coum = (time()-$value['sendtime']);
			$day = $coum/86400;
			if($day>=$receive){
				pdo_update('sz_yi_order', array('status' => 3,'finishtime'=>time()),array('id' => $list[$key]['id'], 'uniacid' => $_W['uniacid']));
				p('descreturn')->insertDescReturnOrder($list[$key]['id']); // 全返
				p('descreturn')->promptly($list[$key]['id']); // 消费返
				p('descreturn')->moneyratio($list[$key]['id']); // 供应商佣金比例的发放
    //易货订单返回商家易货码  --- strat ---
                if ($value['isexchange'] == 1){

    //修正以前的订单 以防止现在的订单重复转账易货码给商家
                    $sure=pdo_fetch('select * from '.tablename('sz_yi_barter_code_log').' where type=2 and uniacid = :uniacid and dealsn=:dealsn',array(':uniacid'=>$_W['uniacid'],':dealsn'=>$value['ordersn']));
	...
	if ($_GPC['export'] == 1) {
		...
		foreach ($list as & $row) {
			...
			$row['finishtime'] = !empty($row['finishtime']) ?  $row['finishtime'] : '';
	...
	$totals = array();
	$totals['all'] = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('sz_yi_order') . ' o ' . ' left join ( select rr.id,rr.orderid,rr.status from ' . tablename('sz_yi_order_refund') . ' rr left join ' . tablename('sz_yi_order') . ' ro on rr.orderid =ro.id order by rr.id desc limit 1) r on r.orderid= o.id' . ' left join ' . tablename('sz_yi_member') . ' m on m.openid=o.openid  and m.uniacid =  o.uniacid' . ' left join ' . tablename('sz_yi_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename(' sz_yi_member') . ' sm on sm.openid = o.verifyopenid and sm.uniacid=o.uniacid' . ' ' . " WHERE $condition", $paras);
	...
	if ($to == 'confirmpay') {
		...
		// p('descreturn')->moneyratio($item['id']); // 供应商佣金比例的发放
		p('descreturn')->insertDescReturnOrder($item['id']); // 全返
		p('descreturn')->promptly($item['id']); // 消费返
		zymfunc_1($item);
		...
		if ($item['isexchange'] == 1) {
			if (doubleval($item['dispatchprice']) > 0) { 
				...
				m('member')->setCredit($supplierOpenid,'credit2',-doubleval($item['dispatchprice']));
			}    
			...
			message('退款申请处理成功!', referer(), 'success');
		}else{     
		...
		exit;
	} else if ($to == 'confirmsend2'){
		// 特殊订单 确认发货2 修改order标识
		confirmsend2($item);
	} else if ($to == 'confirmsend3'){
		// 特殊订单 确认发货3 修改order标识
		confirmsend3($item);
	} else if ($to == 'take1'){
		// 帮助用户第一次收货
		$result = pdo_update('sz_yi_order', array(
			'special_status' => 'm1'
			) , array(
		    'id' => $item['id'],
		    'uniacid' => $_W['uniacid']
		...
		if (!empty($result)) {
			message('第二次收货操作成功！', order_list_backurl(), 'success');
		}
	} else if ($to == 'take2'){
...
function confirmsend2($_var_21)
{
	global $_W;
	// print_r($_var_21);exit;
	$result = pdo_update('sz_yi_order', array(
		'special_status' => 'w2'
		) , array(
	    'id' => $_var_21['id'],
	...
	if (!empty($result)) {
		message('第二次发货操作成功！', order_list_backurl(), 'success');
	}
}
function confirmsend3($_var_21)
{
	global $_W;
	// print_r($_var_21);exit;
	$result = pdo_update('sz_yi_order', array(
	...
	if (!empty($result)) {
		message('第三次发货操作成功！', order_list_backurl(), 'success');
	}
}
// ======================================
...
function order_list_backurl()
{
	...
	//return $_GPC['op'] == 'display' ? $this->createWebUrl('dealmerch_post_list') : referer();
	//return $this -> createWebUrl('dealmerch/dealmerch_post_list');

	return $this -> createPluginWebUrl('dealmerch/dealmerch');
}
...
function zymfunc_3($_var_21)
{
	...
	if ($_var_21['special'] == 1) {
		// 特殊订单
		$data['special_status'] = 'w1';//特殊状态 w1表示后台已发货1次
	}

	pdo_update('sz_yi_order', $data , array(
	    'id' => $_var_21['id'],
	    'uniacid' => $_W['uniacid']
	));
...
function order_list_finish($_var_21)
{
	...
	pdo_update('sz_yi_order', $data, array('id' => $_var_21['id'], 'uniacid' => $_W['uniacid']));
	m('member')->upgradeLevel($_var_21['openid']);
	m('notice')->sendOrderMessage($_var_21['id']);
