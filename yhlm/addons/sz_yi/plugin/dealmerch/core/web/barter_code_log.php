...
if($op == 'detailorder'){
    ...
    if ($_W['isajax']) {                                      
           ...
           // $ordersn=pdo_fetch($sql,$params); 
            ...
            $ordersn=pdo_fetch('select * from '.tablename('sz_yi_barter_log').' where logno = :logno ',array(':logno'=>$logid));
...
$sql = 'select id,openid,type,currency,status,assoctype,dealtime,note,dealsn from '.tablename('sz_yi_barter_code_log').'  where 1 '.$condition;
...
if ($_GPC['export'] == 1){
    ...
    unset($row);
    m('excel') -> export($list, array('title' => '易货码收支查询-' . date('Y-m-d-H-i', time()), 'columns' => array(array('title' => 'ID', 'field' => 'id', 'width' => 12), array('title' => '交易编号', 'field' => 'dealsn', 'width' => 24), array('title' => '交易类型', 'field' => 'type', 'width' => 12), array('title' => '交易额度', 'field' => 'currency', 'width' => 12), array('title' => '交易状态', 'field' => 'status', 'width' => 12), array('title' => '关联类型', 'field' => 'assoctype', 'width' => 12), array('title' => '交易时间', 'field' => 'dealtime', 'width' => 24), array('title' => '交易说明', 'field' => 'note', 'width' => 24))));
}
