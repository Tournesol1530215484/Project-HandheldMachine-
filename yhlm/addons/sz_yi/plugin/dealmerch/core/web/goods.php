...
//            'isverify' => intval($_GPC['isverify']),
    ...
    if (p('supplier')) {
        ...
        if ((!empty($userroleid)) && ($userroleid == $suproleid)) {
            ...
            $sql = 'SELECT * FROM ' . tablename('sz_yi_goods') . $condition . ' and supplier_uid=' . $_W['uid'] . " ORDER BY `displayorder` DESC, uptime DESC, \r\n\t\t\t\t\t`id` DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
            ...
            $sql = 'SELECT * FROM ' . tablename('sz_yi_goods') . $condition . " ORDER BY `displayorder` DESC, `uptime` DESC, `id` DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
        ...
        $sql = 'SELECT * FROM ' . tablename('sz_yi_goods') . $condition . " ORDER BY `uptime` DESC, `displayorder` asc,\r\n\t\t\t\t\t`id` DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
...
//    print_r($params);exit;
    ...
    if($_GPC['export1'] == '1'){
         ...
         $sqls = 'SELECT * FROM ' . tablename('sz_yi_goods') . $condition . " and status =1 ORDER BY `uptime` DESC, `displayorder` asc,\r\n\t\t\t\t\t`id` DESC " ;

          $lists = pdo_fetchall($sqls, $params);
          plog('member.member.export', '在售换货商品');
    m('excel') -> export($lists, array('title' => '在售换货商品-' . date('Y-m-d-H-i', time()), 'columns' => array(
            array('title' => '商品ID', 'field' => 'id', 'width' => 12),
