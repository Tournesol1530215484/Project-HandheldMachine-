<?php
 global $_W;
$sql = '
CREATE TABLE IF NOT EXISTS `ims_sz_yi_af_dealmerch` (
...
CREATE TABLE IF NOT EXISTS `ims_sz_yi_dealmerch_apply` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
...
if(!pdo_fieldexists('sz_yi_goods', 'dealmerch_uid')){
    pdo_query('ALTER TABLE ' . tablename('sz_yi_goods') . ' ADD `dealmerch_uid` INT NOT NULL COMMENT \'商家ID\';');
}
if(!pdo_fieldexists('sz_yi_order', 'dealmerch_uid')){
    pdo_query('ALTER TABLE ' . tablename('sz_yi_order') . ' ADD `dealmerch_uid` INT NOT NULL COMMENT \'商家ID\';');
}
if(!pdo_fieldexists('sz_yi_order_goods', 'dealmerch_uid')){
    pdo_query('ALTER TABLE ' . tablename('sz_yi_order_goods') . ' ADD `dealmerch_uid` INT NOT NULL COMMENT \'商家ID\';');
}
if(!pdo_fieldexists('sz_yi_order_goods', 'dealmerch_apply_status')){
    pdo_query('ALTER TABLE ' . tablename('sz_yi_order_goods') . ' ADD `dealmerch_apply_status` tinyint(4) NOT NULL COMMENT \'1为商家已提现\';');
}
if(!pdo_fieldexists('sz_yi_af_dealmerch', 'id')){
    pdo_query('ALTER TABLE ' . tablename('sz_yi_af_dealmerch') . ' ADD PRIMARY KEY (`id`);');
}
if(!pdo_fieldexists('sz_yi_dealmerch_apply', 'id')){
    pdo_query('ALTER TABLE ' . tablename('sz_yi_dealmerch_apply') . ' ADD PRIMARY KEY (`id`);');
}
if(!pdo_fieldexists('sz_yi_af_dealmerch', 'id')){
    pdo_query('ALTER TABLE ' . tablename('sz_yi_af_dealmerch') . ' MODIFY `id` int(11) NOT NULL AUTO_INCREMENT,AUTO_INCREMENT=4;');
}
if(!pdo_fieldexists('sz_yi_dealmerch_apply', 'id')){
    pdo_query('ALTER TABLE ' . tablename('sz_yi_dealmerch_apply') . ' MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;');
}
...
$info = pdo_fetch('select * from ' . tablename('sz_yi_plugin') . ' where identity= "dealmerch"  order by id desc limit 1');
if(!$info){
    $sql = 'INSERT INTO ' . tablename('sz_yi_plugin') . ' (`displayorder`, `identity`, `name`, `version`, `author`, `status`, `category`) VALUES(0, \'dealmerch\', \'商家\', \'1.0\', \'官方\', 1, \'biz\');';
    pdo_query($sql);
}
if(!pdo_fieldexists('sz_yi_af_dealmerch', 'status')){
    pdo_query('ALTER TABLE ' . tablename('sz_yi_af_dealmerch') . ' ADD `status` TINYINT( 3 ) NOT NULL COMMENT \'0申请1驳回2通过\' AFTER `productname`;');
}
...
message('商家插件安装成功', $this -> createPluginWebUrl('dealmerch/dealmerch'), 'success');
