...
if ($operation == 'display') {
    ...
    if ($ac == 'recharge') {    //充值记录
        ...
        $list=array();
        $list=pdo_fetchall('select l.*,m.realname,m.nickname from '.tablename('sz_yi_barter_log').' l left join '.tablename('sz_yi_member').' m on l.openid = m.openid where l.uniacid = :uniacid and l.openid = :openid order by l.id desc limit '.($pindex-1)*$psize.','.$psize,array(':uniacid'=>$_W['uniacid'],':openid'=>$openid));
        if(!empty($list)){
            foreach ($list as $key => $value) {
