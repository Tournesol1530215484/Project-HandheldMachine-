...
$totals = pdo_fetchcolumn('select count(*) from ' . tablename('sz_yi_transfer_log').' where '.$condition,$params);
...
$list=pdo_fetchall('select id,sponsor_openid,currency,recipient_openid,note,transfertime from '.tablename('sz_yi_transfer_log').' where'.$condition,$params);
    ...
    //打印表单
    if ($_GPC['export1'] == '1'){
        $lists=pdo_fetchall('select id,sponsor_openid,currency,recipient_openid,note,transfertime from '.tablename('sz_yi_transfer_log').' where  uniacid=:uniacid order by id desc',$params);
        foreach($lists as $k =>&$v){
            ...
            $tempname=pdo_fetch('select realname,nickname from '.tablename('sz_yi_member').' where openid="'.$v['recipient_openid'].'"');
            $v['recipient_name']=!empty($tempname['realname'])?$tempname['realname']:$tempname['nickname'];
            $v['transfertime']=date("Y-m-d-H-i",$v['transfertime']);
        }
        plog('member.member.export', '导出换货码转账数据');
        m('excel') -> export($lists, array('title' => '导出换货码转账数据' . date('Y-m-d-H-i', time()), 'columns' => array(array('title' => '发起人', 'field' => 'sponsor_name', 'width' => 12),array('title' => '接收人', 'field' => 'recipient_name', 'width' => 12), array('title' => '换货码数量', 'field' => 'currency', 'width' => 12), array('title' => '时间', 'field' => 'transfertime', 'width' => 12), array('title' => '备注', 'field' => 'note', 'width' => 12))));
    }
