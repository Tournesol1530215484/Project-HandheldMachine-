...
if ($operation == 'display') {
    ca('dealmerch.dealmerch_adminset');
    $list = pdo_fetchall("SELECT s.*,m.nickname,m.avatar,m.mobile,m.realname,store.title storename FROM " . tablename('sz_yi_saler') . "  s " . " left join " . tablename('sz_yi_member') . " m on s.openid=m.openid and m.uniacid = s.uniacid " . " left join " . tablename('sz_yi_exchange_address') . " store on store.id=s.storeid " . " WHERE s.uniacid = '{$_W['uniacid']}' and isexchange = 1 ORDER BY id asc"); 
    ...
    if (checksubmit('submit')) {
        ...
        message('更新核销员成功！', $this->createPluginWebUrl('dealmerch/dealmerch_adminset', array(
            'op' => 'display'
        )), 'success');
    }
    ...
    if (empty($item)) {
        message('抱歉，核销员不存在或是已经被删除！', $this->createPluginWebUrl('dealmerch/dealmerch_adminset', array(
            'op' => 'display'
        )), 'error');
    }
    ...
    message('核销员删除成功！', $this->createPluginWebUrl('dealmerch/dealmerch_adminset', array(
        'op' => 'display' 
    )), 'success');
