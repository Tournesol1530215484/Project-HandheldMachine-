...
if($operation == 'post' ) {
	...
	$sql='select m.realname,m.nickname,l.type,l.remark,l.ctime,g.title,g.thumb from hs_sz_yi_goods_report_log l left join hs_sz_yi_member  m on m.openid=l.openid AND l.uniacid=:uniacid left join hs_sz_yi_goods g on l.goodsid=g.id';
	...
	foreach ($list as $key => $value) {
		...
		$list[$key]['type']=$temp;
		 array_splice($temp, 0, count($temp));
	}
	$temps=array();
	foreach ($list as $key => $value) {
	...
	// 	if($list[$i]['title']==$list[$i+1]['title'] && $list[$i]['openid']==$list[$i+1]['openid']){
	// 		$temp[]=$list[$i]['type'].','.$list[$i+1]['type'];
	// 		//$list[$i]['type']=$list[++$i]['type'];
	// 		//unset($list[++$i]);
	// 		//$list[$i]=array_merge($list[$i]=,$list[++$i]);
	// 	}
	// }
