...
if ($op == 'display') {
    ...
    $sql = "select log.id,m.id as mid, m.realname,m.avatar,m.weixin,log.logno,log.re_money,log.type,log.status,log.rechargetype,m.nickname,m.mobile,m.bank,m.bankname,g.groupname,log.money,log.createtime,l.levelname from " . tablename('sz_yi_barter_log') . " log " . " left join " . tablename('sz_yi_member') . " m on m.openid=log.openid" . " left join " . tablename('sz_yi_member_group') . " g on m.groupid=g.id" . " left join " . tablename('sz_yi_member_level') . " l on m.level =l.id" . " where 1 {$condition} ORDER BY log.createtime DESC ";
    ...
    $total = pdo_fetchcolumn("select count(*) from " . tablename('sz_yi_barter_log') . " log " . " left join " . tablename('sz_yi_member') . " m on m.openid=log.openid and m.uniacid= log.uniacid" . " left join " . tablename('sz_yi_member_group') . " g on m.groupid=g.id" . " left join " . tablename('sz_yi_member_level') . " l on m.level =l.id" . " where 1 {$condition} ", $params);
    $pager = pagination($total, $pindex, $psize);
} else if ($op == 'pay') {
    $id      = intval($_GPC['id']);
    $paytype = $_GPC['paytype'];
    $set     = m('common')->getSysset('shop');
    $log     = pdo_fetch('select * from ' . tablename('sz_yi_barter_log') . ' where id=:id and uniacid=:uniacid limit 1', array(
        ':id' => $id,
        ':uniacid' => $uniacid
    ));
    if (empty($log)) {
    ...
    if ($paytype == 'manual') {
        ...
        pdo_update('sz_yi_barter_log', array(
            'status' => 1
        ), array(
            'id' => $id,
            'uniacid' => $uniacid
