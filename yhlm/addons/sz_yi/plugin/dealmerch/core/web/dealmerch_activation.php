...
if($op == 'fenhong'){
    ...
    if ($_W['isajax']) {                                      
            ...
            $data=$_POST;
            //$logid=$data['logid'];  //获取用户激活订单id，在获取订单编号ac2019061810335d084d5eaabfa
            $logid=$data['logid'];
            $sql='select ordersn from'.tablename("sz_yi_currency_activation")."where id=".$logid;
            $ordersn=pdo_fetchall($sql,$params);
            $ordersnres=$ordersn[0]['ordersn'];
             $sqll='select sf.*,pu.nickname,pu.realname from '.tablename("sz_yi_barter_services_fee").'sf left join '.tablename('sz_yi_member')."pu on sf.mid=pu.id  where sf.type=1 and  sf.uniacid = 8 and sf.ordersn ='{$ordersnres}'"; 
             $list=pdo_fetchall($sqll,$params);     //获取所有的代理商信息  
            $rule=array(
                '商家',    //0            
                '开发',    //1
                '推荐',    //2   
...
// 导出易货订单
    ...
    if ($_GPC['export'] == 2){
        ...
        $sqls = 'select m.realname,m.nickname,ca.id,ca.type,ca.activacurrency,ca.activapay,ca.activatime,ca.paytype from '.tablename('sz_yi_currency_activation').' ca left join '.tablename('sz_yi_member').' m on ca.openid=m.openid where 1 ';
        $sqls.=$condition;
        $sqls .= ' ORDER BY ca.activatime DESC ';
        $lists = pdo_fetchall($sqls,$params);  
    ca('statistics.export.order');
    plog('statistics.export.order', '导出易货码激活记录');
    foreach ($lists as & $row){
        ...
        $row['activatime'] = date('Y-m-d H:i', $row['activatime']);
    }
    unset($row);
    m('excel') -> export($lists, 
        array('title' => '导出易货码激活记录-' . date('Y-m-d-H-i', time()), 'columns' => array(
            array('title' => '昵称', 'field' => 'nickname', 'width' => 24),
            array('title' => '真实名字', 'field' => 'realname', 'width' => 24), 
            array('title' => '操作类型', 'field' => 'type', 'width' => 12), 
            array('title' => '激活额度', 'field' => 'activacurrency', 'width' => 12), 
            array('title' => '激活支付', 'field' => 'activapay', 'width' => 12), 
            array('title' => ' 支付方式', 'field' => 'paytype', 'width' => 12), 
            array('title' => '激活时间', 'field' => 'activatime', 'width' => 12))));
}
