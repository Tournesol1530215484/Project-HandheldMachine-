...
define('TM_dealmerch_PAY', 'dealmerch_pay');
if (!class_exists('dealmerchModel')){
    class dealmerchModel extends PluginModel{
        public $parentAgents = "";
       ...
       function perms(){
            return array(
				'dealmerch' => array(
						...
						'child' => array(
							...
							'notice' => array('text' => '通知设置'), 
							'dealmerch' => array('text' => '商家管理'),
                            'dealmerch.add'=>array('text'=>'添加商家'),
                            'dealmerch.edit'=>array('text'=>'修改商家'),
                            'dealmerch.view'=>array('text'=>'查看商家'),
                            'dealmerch.delete'=>array('text'=>'删除商家'),
                            'barter_code_log' => array('text' => '易货码收支查询'),
                            'barter_currency' => array('text' => '易货额度收支查询'),
                            'consult' => array('text' => '客户咨询管理'),
                            'goods' => array('text' => '商品管理'),  
                            'goods.edit' => array('text' => '商品修改'),                 
                            'goods.edit.supplier_uid' => array('text' => '修改所属商家'), 
                            'goods.delete' => array('text' => '商品删除'),                                      
                            'dealmerch_for' => array('text' => '会员申请商家'),        
                            'dealmerch_for_resu' => array('text' => '申请结果'),
                            'dealmerch_apply' => array('text' => '商家提现申请'),
                            'dealmerch_finish' => array('text' => '商家提现完成'),
                            //注释后至分号前
							'dealmerch_local_list' => array('text' => '现场兑换订单'),
                            'dealmerch_post_list' => array('text' => '邮寄兑换订单'),
                            'dealmerch_work_number' => array('text' => '开拓工号管理'),
                            'dealmerch_sales_statis' => array('text' => '商品销售统计'),
                            'dealmerch_exchange' => array('text' => '兑换点管理'),
                            'dealmerch_adminset' => array('text' => '兑换管理员设置'),
							'dealmerch_stockchange' => array('text' => '库存变更记录'),
                        ), 
				) 
			); 
        } 
        ...
        public function senddealmerchInform($dephp_64 = '', $dephp_69 = ''){
        ...
        public function order_split($dephp_74){
            ...
            $dephp_75 = pdo_fetchall('select distinct dealmerch_uid from ' . tablename('sz_yi_order_goods') . ' where orderid=:orderid and uniacid=:uniacid', array(':orderid' => $dephp_74, ':uniacid' => $_W['uniacid']));
            if(count($dephp_75) == 1){
                pdo_update('sz_yi_order', array('dealmerch_uid' => $dephp_75[0]['dealmerch_uid']), array('id' => $dephp_74, 'uniacid' => $_W['uniacid']));
                return;
            }
            $dephp_76 = pdo_fetchall('select dealmerch_uid, id from ' . tablename('sz_yi_order_goods') . ' where orderid=:orderid and uniacid=:uniacid ', array(':orderid' => $dephp_74, ':uniacid' => $_W['uniacid']));
            ...
            foreach ($dephp_76 as $dephp_80 => $dephp_81){
                $dephp_79[$dephp_81['dealmerch_uid']][]['id'] = $dephp_81['id'];
            }
            $dephp_82 = false;
            unset($dephp_77['id']);
            ...
            if(!empty($dephp_79)){
                foreach ($dephp_79 as $dephp_80 => $dephp_81){
                    ...
                    foreach($dephp_81 as $dephp_96){
                        $dephp_70 = pdo_fetch('select price,realprice,oldprice,dealmerch_uid from ' . tablename('sz_yi_order_goods') . ' where id=:id and uniacid=:uniacid ', array(':id' => $dephp_96['id'], ':uniacid' => $_W['uniacid']));
				...
				if($_var_1['dispatchtype']==1){
                    ...
                    $dephp_86['dealmerch_uid'] = $dephp_97;
