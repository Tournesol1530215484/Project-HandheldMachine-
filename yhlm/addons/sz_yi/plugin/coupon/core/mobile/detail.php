...
$coupon_log = pdo_fetchcolumn('select count(id) from ' . tablename('sz_yi_coupon_log') . 'where couponid=:couponid and uniacid=:uniacid',array(':couponid' => $id, ':uniacid' => $_W['uniacid']));
...
if ($op == 'display') {
	...
	file_put_contents(dirname(__FILE__).'/bbbbbbb','aaaaaa');
	if (empty($coupon['gettype'])) {
		show_json(-1, '无法' . $coupon['gettypestr']);
	}
	if ($coupon['total'] != -1) {
		if ($coupon['total'] <= $coupon_log) {
			show_json(-1, '优惠券数量不足');
		}
	}
	if (!$coupon['canget']) {
