...
if ($_W['isajax'] && $_W['ispost']) {
	if ($apido == 'selectlike') {
		...
		if ($article_sys['article_temp'] == 0) {
			...
			$articles = pdo_fetchall("SELECT id,article_title,resp_img,article_rule_credit,article_rule_money FROM " . tablename('sz_yi_article') . " WHERE article_state=1 and uniacid=:uniacid order by id desc limit " . (($pindex - 1) * $psize . ',' . $psize), array(':uniacid' => $_W['uniacid']));
