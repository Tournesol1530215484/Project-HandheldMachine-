...
');
				...
				if (!empty($log) && empty($log['status']) && $log['fee'] == $total_fee) {
					...
					m('member')->setCredit($log['openid'], 'credit2', $log['money'], array(0, '换货商城会员充值:credit2:' . $log['money']));
				...
				if (!empty($log) && empty($log['status']) && $log['money'] == $total_fee) {
					pdo_update('sz_yi_member_log', array('type'=>3,'status' => 1, 'rechargetype' => 'wechat'), array('id' => $log['id']));
					m('member')->setCredit($log['openid'], 'credit2', $log['money'], array(0, '换货商城会员充值:credit2:' . $log['money']));
					m('member')->setRechargeCredit($log['openid'], $log['money']);
					if (p('sale')) {
						p('sale')->setRechargeActivity($log);
					...
					m('notice')->sendMemberLogMessage2($log['id']);
				}
			} else if ($type == 6) {
				$logno = trim($get['out_trade_no']);
				if (empty($logno)) {
				...
				//type = 4 激活换货码手续费	 		 		 
				if (!empty($log) && empty($log['status']) && $log['money'] == $total_fee) {
					pdo_update('sz_yi_member_log', array('type'=>4,'status' => 1, 'rechargetype' => 'wechat'), array('id' => $log['id']));
					m('member')->setCredit($log['openid'], 'credit2', $log['money'], array(0, '换货商城会员激活换货码:credit2:' . $log['money']));
					m('member')->setRechargeCredit($log['openid'], $log['money']);
					if (p('sale')) {
						p('sale')->setRechargeActivity($log);
					}	 				
					...
					m('notice')->sendMemberLogMessage3($log['id']);
				}
			}
		}	 			  
	}
