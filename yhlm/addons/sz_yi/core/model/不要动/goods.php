...

{
...

    {
        ...
        if (!$random) {
            ...
            $sql = "SELECT showgroups,PostFlag,LocalFlag,showlevels,id,title,thumb,marketprice,productprice,sales,total,description,unit,type FROM " . tablename('sz_yi_goods') . " where 1 {$condition} ORDER BY {$order} {$orderby} LIMIT " . ($page - 1) * $pagesize . ',' . $pagesize;

        } else {

            $sql = "SELECT showgroups,showlevels,PostFlag,LocalFlag,id,title,thumb,marketprice,productprice,sales,total,description,unit,type FROM " . tablename('sz_yi_goods') . " where 1 {$condition} ORDER BY rand() LIMIT " . $pagesize;
...
//            print_r($orderby);exit;
            $sql = "SELECT g.showgroups,g.showlevels,g.PostFlag,g.LocalFlag,g.id,g.title,g.thumb,g.sales,g.total,g.description,g.unit,g.type,op.marketprice,op.productprice,op.costprice FROM " . tablename('sz_yi_goods') . " g left join (select goodsid,productprice,max(marketprice) marketprice,costprice from ".tablename('sz_yi_goods_option')." where uniacid = 8 group by goodsid) op on g.id = op.goodsid where 1 {$condition} ORDER BY {$order} {$orderby} LIMIT " . ($page - 1) * $pagesize . ',' . $pagesize;         
//            print_r($sql);exit;
        } else {                                                                                                                                                                                                                                          
            $sql = "SELECT g.showgroups,g.showlevels,g.PostFlag,g.LocalFlag,g.id,g.title,g.thumb,g.marketprice,g.productprice,g.sales,g.total,g.description,g.unit,g.type,max(op.marketprice) FROM " . tablename('sz_yi_goods') ." g left join ".tablename('sz_yi_goods_option')." op on op.goodsid=g.id where 1 {$condition} ORDER BY rand() LIMIT " . $pagesize;
