   ...
   function tip($tips=''){
    ...
    function createPoster($poster, $member, $qr, $upload = true)
        ...
        function signin($openid){
            ...
            m('member')->setCredit($openid,'credit1',$score);

            pdo_insert('sz_yi_activity_signin',$data);
        ...
        public function receive(){
            ...
            foreach ($or as $key => $value) {
                ...
                if(floatval($currency_credit3) >= floatval($value['goodsprice'])){
                    ...
                    m('member') -> setCredit($seller, 'currency_credit3', -floatval($value['goodsprice']));     //卖家获得易货码 冻结 如果卖家存在易货额度将自动使用易货额度激活
                    m('member') -> setCredit($seller, 'credit3', floatval($value['goodsprice']));   //卖家获得易货码 已使用易货额度自动激活
