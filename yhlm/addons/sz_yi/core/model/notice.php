...
class Sz_DYi_Notice
{
    public function sendOrderMessage($orderid = '0', $delRefund = false)
    {
        ...
        $openid      = $order['openid'];
        $order_goods = pdo_fetchall('select g.id,g.title,og.realprice,og.total,og.price,og.optionname as optiontitle,g.extnotice,g.noticeopenid,g.noticetype from ' . tablename('sz_yi_order_goods') . ' og ' . ' left join ' . tablename('sz_yi_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(
            ':uniacid' => $_W['uniacid'],
            ':orderid' => $orderid
        ));
        $goods       = '';
            ...
            foreach ($order_goods as $og) {
                if (!empty($og['noticeopenid'])) {
                    ...
                    if (empty($og['noticetype']) || (is_array($noticetype) && in_array(0, $noticetype))) {
                        ...
                        // start
                        if ($og['extnotice']) {
                            $ntclist=pdo_fetchall('select openid from '.tablename('sz_yi_member').' where uniacid = :uniacid and id in ('.$og['extnotice'].')',array(':uniacid'=>$_W['uniacid']));//333
                            foreach ($ntclist as $nk => $nv) {      //多用户提醒
                                if (!empty($tm['new'])) {
                                    m('message')->sendTplNotice($nv['openid'], $tm['new'], $msg, '', $account);
                                } else {
                                    m('message')->sendCustomNotice($nv['openid'], $msg, '', $account);
                                }
                            }
                        }
                        //end
    ...
    public function sendMemberLogMessage2($log_id = '')
    {
        global $_W, $_GPC;
        $log_info   = pdo_fetch('select * from ' . tablename('sz_yi_member_log') . ' where id=:id and uniacid=:uniacid limit 1', array(
            ':id' => $log_id,
    ...
    public function sendMemberLogMessage3($log_id = '')
    {
        global $_W, $_GPC;
        $log_info   = pdo_fetch('select * from ' . tablename('sz_yi_member_log') . ' where id=:id and uniacid=:uniacid limit 1', array(
            ':id' => $log_id,
