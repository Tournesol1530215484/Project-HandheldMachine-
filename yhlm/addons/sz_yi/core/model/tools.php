...
class Sz_DYi_Tools{
    ...
    function batch_filter_Emoji($data){

        foreach ($data as $key => $value) {
            $data[$key]=$this->filter_Emoji($value);
        }
        return $data;
    }
   ...
   function tip($tips=''){
    ...
    function createCardPoster2($openid,$id,$url=0) //创建文章海报
    {
        global $_W,$_GPC;            
         $poster = pdo_fetch('select * from ' . tablename('sz_yi_poster') . ' where isdefault = 1 and uniacid=:uniacid and type = :id limit 1', array(
            ':uniacid' => $_W['uniacid'],                        
    ...
    function createPoster($poster, $member, $qr, $upload = true)
        ...
        {           
            ...
            if ($_GPC['what'] == 2) {
                $tempinfo=m('member')->getMember($openid);
               // $activity['ContactOrg']=$tempinfo['nickname'];
                $activity['ContactOrg']=$tempinfo['realname'];
                $activity['mobileOrg']=$tempinfo['mobile'];             
            }
            ...
            if (!is_file($path . $file) || $qr['qrimg'] != $qr['current_qrimg']) {
                ...
                foreach ($data as $d) {
                    ...
                    if ($d['type'] == 'head') {
                        ...
                        //修改  
                        if($activity['ContactOrg'] !=''){
                            $activity['ContactOrg']="邀请人:".$activity['ContactOrg'];
                        }    
                        // $activity['ContactOrg']="邀请人:".$activity['ContactOrg'];                                                 
                        ...
                        //修改      
                        if($activity['mobileOrg'] !=''){
                            $activity['mobileOrg']="电话:".$activity['mobileOrg'];
                        }
                        //$activity['mobileOrg']="电话:".$activity['mobileOrg'];                                       
                    ...
                    } else if ($d['type'] == 'iv_address') {  //修改地址

                        $activity['address']="地址:".$activity['city'].$activity['area'].$activity['address'];  //添加
        ...
        //     if (!is_file($path . $file) || $qr['qrimg'] != $qr['current_qrimg']) {
                ...
                imagettftext($target, 22, 0, 120, 550, $_var_125, $_var_110, $_var_130);
            ...
            // }
            $img = $_W['siteroot'] . "addons/sz_yi/data/paper/" . $_W['uniacid'] . "/" . $file;
            if (!$upload) {
                return $img;
            }
            if ($qr['qrimg'] != $qr['current_qrimg'] || empty($qr['mediaid']) || empty($qr['createtime']) || $qr['createtime'] + 3600 * 24 * 3 - 7200 < time()) {
        ...
        function imagecropper($source_path, $target_width, $target_height,$type)
        {   
            global $_W;
            $poster = pdo_fetch('select * from ' . tablename('sz_yi_poster') . ' where uniacid=:uniacid and type=:type and isdefault=1 limit 1', array(
                ':uniacid' => $_W['uniacid'],
            ...
            $_var_100 = IA_ROOT . '/addons/sz_yi/data/rebg/' . $_W['uniacid'] . '/';
            if (!is_dir($_var_100)) {       
                load()->func('file');
                mkdirs($_var_100);
            }

            $tfn=md5(json_encode($poster)).'.jpg';       
            $path=$_var_100.$tfn;
            if (is_file($path)) {
                return $path;
            }                 
            $re=imagejpeg($target_image,$path);                   
        ...
        function signin($openid){
            ...
            $lastsignin=pdo_fetch('select  * from '.tablename('sz_yi_activity_signin').' where uniacid = :uniacid and openid = :openid order by id desc ',array(':uniacid'=>$_W['uniacid'],':openid'=>$openid));

            
                if ( $lastsignin && date('Ymd',$lastsignin['ctime']) == date('Ymd')){        //在今天
                    show_json(0,'打卡失败,你今天在'.date('Y-m-d H:i:s',$lastsignin['ctime']).'已经打卡了!');
                }else{
                    if ( $lastsignin && $lastsignin['date'] == date('Ymd',strtotime('-1 day'))){                    //上次签到时间在24h之内
            ...
            $score=rand($set['minscore'],$set['maxscore']);
            ...
            m('member')->setCredit($openid,'credit1',$score);//把获取的随机积分保存到数据库

            pdo_insert('sz_yi_activity_signin',$data);  //添加打卡记录，继续打卡
        ...
        public function receive(){
            global $_W;
            $or=pdo_fetchall('select * from '.tablename('sz_yi_order').' where uniacid = :uniacid and isexchange = 1 and status = 3 ',array(':uniacid'=>$_W['uniacid']));
            
            foreach ($or as $key => $value) {
                $exists=pdo_fetch('select * from '.tablename('sz_yi_barter_code_log').' where uniacid = :uniacid and dealsn = :ordersn and type = 2 ',array(':uniacid'=>$_W['uniacid'],':ordersn'=>$value['ordersn']));
                
                if ($exists || $value['supplier_uid'] == 641) {
                    unset($or[$key]);
                }
            }

            foreach ($or as $key => $value) {
                $exists=pdo_fetch('select * from '.tablename('sz_yi_barter_code_log').' where uniacid = :uniacid and dealsn = :ordersn and type = 1',array(':uniacid'=>$_W['uniacid'],':ordersn'=>$value['ordersn']));
                if (!$exists || $value['openid'] == 'oSI4Lj6FuF7nRHHuo8tGMR8S_P14') {
                    unset($or[$key]);                                
                }                    
            }
             
            foreach ($or as $key => $value) {
                $seller=pdo_fetchcolumn('select openid from '.tablename('sz_yi_perm_user').' where uniacid = :uniacid and uid = :uid and dealmerchid > 0 ',array(':uniacid'=>$_W['uniacid'],':uid'=>$value['supplier_uid']));

                $currency_credit3=m('member')->getCredit($seller,'currency_credit3');
                
                if(floatval($currency_credit3) >= floatval($value['goodsprice'])){
                    
                    m('member') -> setCredit($seller, 'currency_credit3', -floatval($value['goodsprice']));     
                    //卖家获得易货码 冻结 如果卖家存在易货额度将自动使用易货额度激活
                    m('member') -> setCredit($seller, 'credit3', floatval($value['goodsprice']));   
                    //卖家获得易货码 已使用易货额度自动激活
                    m('log')->putBarterCurrencyLog($seller,$value['supplier_uid'],11,-floatval($value['goodsprice']),$value['ordersn'],'销售易货码自动解冻');

                }else{

                    m('member')->setCredit($seller, 'freeze_credit3', floatval($value['goodsprice']));    //卖家获得易货码 冻结 
                } 
                    m('log')->putBarterCodeLog($seller,$value['supplier_uid'],2,1,1,floatval($value['goodsprice']),$value['ordersn'],'销售收入',date('Ymd'));
        ...
        function calcCash($openid){
            global $_W;

            $cash['total']['recharge']=pdo_fetchcolumn('select sum(money) from '.tablename('sz_yi_member_log').' where type = 0 and uniacid = :uniacid and openid = :openid ',array(':uniacid'=>$_W['uniacid'],':openid'=>$openid));
    
            $cash['total']['withdraw']=pdo_fetchcolumn('select sum(money) from '.tablename('sz_yi_member_log').' where type = 1 and uniacid = :uniacid and openid = :openid ',array(':uniacid'=>$_W['uniacid'],':openid'=>$openid));   
            //现金消费，获取了所有的不是订单为易货的数据
            //$cash['total']['shop']=pdo_fetchcolumn('select sum(price) from '.tablename('sz_yi_order').' where isexchange = 0 and uniacid = :uniacid and openid = :openid ',array(':uniacid'=>$_W['uniacid'],':openid'=>$openid)); 

            $cash['total']['shop']=pdo_fetchcolumn('select sum(price) from '.tablename('sz_yi_order').' where isexchange = 0 and status = 3 and uniacid = :uniacid and openid = :openid  ',array(':uniacid'=>$_W['uniacid'],':openid'=>$openid));   
                    
            $cash['total']['goods']=0;      //货款 
            $cash['total']['other']=0;      //其他 
            $cash['openid']=$openid;
            ...
            $cash['total']['fans']=floatval($fans)+floatval($me);   //粉丝购物奖励
            
            return $cash;   
        }


        function calcCode($openid){
            global $_W;


            $code['total']['friend']['put']=0;
            $code['total']['friend']['put']=pdo_fetchcolumn('select sum(currency) from '.tablename('sz_yi_barter_code_log').' where uniacid = :uniacid and type = 14 and openid = :openid',array(':uniacid'=>$_W['uniacid'],':openid'=>$openid)); 

            $code['total']['friend']['get']=0;
            ...
            if (!empty($tempget)) { 
                foreach ($tempget as $key => $value) {
                    ...
                    $code['total']['friend']['get']+=intval($tempget[$key]['currency']);     
                }
            }      
            $code['total']['saler']=0;      
            $code['total']['saler']=pdo_fetchcolumn('select sum(currency) from '.tablename('sz_yi_barter_code_log').' where uniacid = :uniacid and type = 2 and openid = :openid',array(':uniacid'=>$_W['uniacid'],':openid'=>$openid));     
            ...
            if (!$dm) {
                ...
                //$code['total']['refund']=pdo_fetchcolumn('select count(*) from '.tablename('sz_yi_order').' where uniacid = :uniacid and status = -1 and supplier_uid = :supplier_uid',array(':uniacid'=>$_W['uniacid'],':supplier_uid'=>$dm['uid'])); 
                $code['total']['refund']=pdo_fetchcolumn('select sum(price) from '.tablename('sz_yi_order').' where uniacid = :uniacid and status = -1 and supplier_uid = :supplier_uid',array(':uniacid'=>$_W['uniacid'],':supplier_uid'=>$dm['uid']));                   
            }

            $code['total']['refunduse']=pdo_fetchcolumn('select sum(goodsprice) from '.tablename('sz_yi_order').' where uniacid = :uniacid and openid = :openid and refundtime != 0 ',array(':uniacid'=>$_W['uniacid'],':openid'=>$openid));
            

            //所有的换货消耗已付款     [这里被修改了，修改时间2019:8:27,注释部分是被修改之前的代码，可以看到是从易货记录表里面获取的数据，很明显这里和后面改动部分完全是两种逻辑]
            // $code['total']['use']=0;    
            ...
            //         $code['total']['use']+=intval($value['currency']);      
            //     }
            // }
            $code['total']['use']=0; 
            $code['total']['use']=pdo_fetchcolumn('select sum(goodsprice ) from '.tablename('sz_yi_order').' where status not in (-1,0) and uniacid =:uniacid and openid = :openid ',array(':uniacid'=>$_W['uniacid'],':openid'=>$openid));

            $fans=pdo_fetchcolumn('select sum(money) from '.tablename('sz_yi_ad_bonus_log').' where uniacid = :uniacid and openid = :openid and bonusType = 2 and ( level = 1 or level = 2 )',array(':uniacid'=>$_W['uniacid'],':openid'=>$openid));
            ...
            $code['total']['fans']=floatval($fans)+floatval($me);   //粉丝购物奖励

            //$code['total']['recharge']=0;
            //获取后台充值记录,获取充值
           $total = pdo_fetchcolumn("select sum(money) from " . tablename('sz_yi_barter_log') . " log  where    uniacid = :uniacid and openid = :openid ",array(':uniacid'=>$_W['uniacid'],':openid'=>$openid));

           $system1=pdo_fetchcolumn("select sum(money) from " . tablename('sz_yi_barter_log') . " log  where rechargetype='system1' and  uniacid = :uniacid and openid = :openid ",array(':uniacid'=>$_W['uniacid'],':openid'=>$openid));

            $code['total']['recharge'] =$total-$system1;//总金额减去系统扣除部分
