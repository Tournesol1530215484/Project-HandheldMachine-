...
if ($operation == 'display') {
	...
	//if ( empty($level) ) {
	$level = pdo_fetch("SELECT id,levelname,commission_level_id FROM " . tablename('sz_yi_member_level') . " WHERE id = '$id'");
    if ( empty($level)) {
        message('抱歉，等级不存在或是已经被删除！', $this->createWebUrl('member/level', array( 'op' => 'display' )), 'error');
    }
	if ( $level['commission_level_id'] ) {
		message('抱歉，请移驾分销等级，删除相对应的分销等级！', $this->createWebUrl('member/level', array( 'op' => 'display' )), 'error');
	}
	//end
