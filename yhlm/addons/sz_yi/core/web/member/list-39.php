...
if ($op == 'display') {
    ...
    foreach ($list as &$row) {
        ...
        $row['ordermoney'] = pdo_fetchcolumn('select sum(price) from ' . tablename('sz_yi_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(
            ':uniacid' => $_W['uniacid'],
            ':openid' => $row['openid']
        ));
        ...
        //待返金额
        $surplus_price = pdo_fetch('select surplus_price from'.tablename('sz_yi_descreturn_log').'where uniacid=:uniacid and openid=:openid order by id desc',array(':uniacid'=>$_W['uniacid'],':openid'=>$row['openid']));
        $row['surplus_price'] = $surplus_price['surplus_price'];
        //下级分销员总数
        $info              = p('commission')->getInfo($row['openid'], array(
            'total',
            'pay'
        ));
        $row['num'] = $info['agentcount'];
        //上级分销员
        $a = pdo_fetch('select nickname from'.tablename('sz_yi_member').'where uniacid=:uniacid and id=:id',array(':uniacid'=>$_W['uniacid'],':id'=>$row['agentid']));
        $row['agent'] = $a['nickname'];
        //已提现金额
        $row['widmoney'] = pdo_fetchcolumn('select sum(money) from ' . tablename('sz_yi_member_log') . ' where uniacid=:uniacid and openid=:openid and status=1 and type=1', array(
            ':uniacid' => $_W['uniacid'],
            ':openid' => $row['openid']
        ));
        //微信充值金额
        $row['wemoney'] = pdo_fetchcolumn('select sum(money) from ' . tablename('sz_yi_member_log') . ' where uniacid=:uniacid and openid=:openid and status=1 and rechargetype=:rechargetype', array(
            ':uniacid' => $_W['uniacid'],
            ':openid' => $row['openid'],
            ':rechargetype' => 'wechat'
        ));
        ...
        $row['stmoney'] = pdo_fetchcolumn('select sum(money) from ' . tablename('sz_yi_member_log') . ' where uniacid=:uniacid and openid=:openid and status=1 and rechargetype=:rechargetype', array(
            ':uniacid' => $_W['uniacid'],
            ':openid' => $row['openid'],
            ':rechargetype' => 'system'
        ));
    ...
    if (checksubmit('submit')) {
		...
		if( !empty($_GPC['password']) && !empty($id) ){
			//修改密码
			$data['pwd'] = md5( $_GPC['password'] );
		}
        if( !empty($_GPC['withdraw_pwd']) && !empty($id) ){
            //修改密码
            $data['repel'] = 0;
            $data['withdraw_pwd'] = md5( $_GPC['withdraw_pwd'] );
        }
