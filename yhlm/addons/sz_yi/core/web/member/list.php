...
if ($op == 'display') {
    ...
    $sql = "select dm.*,l.levelname,g.groupname,a.realname as agentrealname,a.nickname as agentnickname,a.avatar as agentavatar from " . tablename('sz_yi_member') . " dm " . " left join " . tablename('sz_yi_member_group') . " g on dm.groupid=g.id" . " left join " . tablename('sz_yi_member') . " a on a.id=dm.agentid" . " left join " . tablename('sz_yi_member_level') . " l on dm.level =l.id" . " left join " . tablename('mc_mapping_fans') . "f on f.openid=dm.openid  and f.uniacid={$_W['uniacid']}" . " where 1 {$condition}  ORDER BY dm.id DESC";
    ...
    $member && dataupdate($id);
    //udpate end
    //去重start
    $oldmember = pdo_getall('mc_mapping_fans', array('openid' => $member['openid']));
    if (count($oldmember) > 1) {
        pdo_delete('mc_mapping_fans', array('fanid' => $oldmember[1]['fanid'], 'uniacid' => $_W['uniacid']));
    }
    //去重end
    ca('member.member.view');
    $hascommission = false;
