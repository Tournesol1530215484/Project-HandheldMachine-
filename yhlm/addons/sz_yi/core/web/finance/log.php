...
if ($op == 'display') {
    ...
    $sql = "select log.id,m.id as mid, m.realname,m.avatar,m.weixin,log.logno,log.re_money,log.type,log.status,log.rechargetype,m.nickname,m.mobile,m.bank,m.bankname,g.groupname,log.money,log.createtime,l.levelname from " . tablename('sz_yi_member_log') . " log " . " left join " . tablename('sz_yi_member') . " m on m.openid=log.openid" . " left join " . tablename('sz_yi_member_group') . " g on m.groupid=g.id" . " left join " . tablename('sz_yi_member_level') . " l on m.level =l.id" . " where 1 {$condition} ORDER BY log.createtime DESC ";
