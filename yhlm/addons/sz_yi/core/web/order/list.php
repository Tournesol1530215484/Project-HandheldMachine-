...
if ($operation == 'display') {
	...
	$condition = ' o.uniacid = :uniacid and o.deleted=0 and o.isexchange = 0 ';
	...
	foreach($list as $key=>$value){
		...
		if($value['status']==2){
			...
			if($day>=7){
				...
				pdo_update('sz_yi_order', array('status' => 3),array('id' => $list[$key]['id'], 'uniacid' => $_W['uniacid']));
	...
	$paytype = array('0' => array('css' => 'default', 'name' => '未支付'), '1' => array('css' => 'danger', 'name' => '余额支付'), '11' => array('css' => 'default', 'name' => '后台付款'), '2' => array('css' => 'danger', 'name' => '在线支付'), '21' => array('css' => 'success', 'name' => '微信支付'),'30' => array('css' => 'success', 'name' => 'app微信支付'), '22' => array('css' => 'warning', 'name' => '支付宝支付'), '23' => array('css' => 'warning', 'name' => '银联支付'), '3' => array('css' => 'primary', 'name' => '货到付款'),);
			...
			if(empty($sure)){
		        ...
		        foreach($order_goods as $k =>$v){
					...
					if(floatval($currency_credit3) >= floatval($v['price'])){     
						...
						m('log')->putBarterCurrencyLog($seller,$v['supplier_uid'],11,-floatval($v['price']),$item['ordersn'],'销售易货码自动解冻');
		            ...
		            m('log')->putBarterCodeLog($seller,$v['supplier_uid'],2,1,1,floatval($v['price']),$item['ordersn'],'销售收入');
