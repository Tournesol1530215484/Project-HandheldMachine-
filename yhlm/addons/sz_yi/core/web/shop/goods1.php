...
if ($operation == 'change') {
	...
	if (checksubmit('submit')) {
		...
		if($_GPC['type']==4){
			if(empty($_GPC['app_key'])) message('缺少app_key');
			if(empty($_GPC['app_secret'])) message('缺少app_secret');
		}
