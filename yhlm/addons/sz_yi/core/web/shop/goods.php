...
if ($operation == 'change') {
    ...
    // if ($_GPC['debug']) {			//给没有提醒人的商品补上提醒人以及提醒类型

    // 	$total=pdo_fetchcolumn('select count(*) from '.tablename('sz_yi_goods').' where uniacid = :uniacid and noticeopenid = "" and supplier_uid !="" ',array(':uniacid'=>$_W['uniacid']));

    // 	$list=pdo_fetchall('select g.*,p.openid from '.tablename('sz_yi_goods').' g left join '.tablename('sz_yi_perm_user').' p on p.uid = g.supplier_uid where g.uniacid = :uniacid and g.noticeopenid = "" and g.supplier_uid !="" ',array(':uniacid'=>$_W['uniacid']));
    ...
    // 			if (empty($value['noticetype'])) {

    // 				pdo_update('sz_yi_goods',array('noticeopenid'=>$value['openid'],'noticetype'=>'0,1,2'),array('id'=>$value['id']));

    // 			}else{

    // 				pdo_update('sz_yi_goods',array('noticeopenid'=>$value['openid']),array('id'=>$value['id']));
    ...
    if (checksubmit('submit')) {
        ...
        if (empty($data['noticeopenid']) && !empty($data['supplier_uid'])) {	//如果没有选择提醒人并且供应商不为空

            $pu=p('bonus')->getMerch($data['supplier_uid']);

            $data['noticeopenid']=$pu['openid'];

            $data['noticetype']='0,1,2';
        ...
        if (!empty($_GPC['area'])) {
            ...
            $data['province'] = $_GPC['area']['provance'];
        ...
        $data['isdescreturn'] = $_GPC['isdescreturn'];
    ...
    if (p('supplier')) {
        ...
        if ((!empty($userroleid)) && ($userroleid == $suproleid)) {
            ...
            $sql = 'SELECT * FROM ' . tablename('sz_yi_goods') . $condition . " ORDER BY `displayorder` DESC,`id` DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
        ...
        $sql = 'SELECT * FROM ' . tablename('sz_yi_goods') . $condition . " ORDER BY  `displayorder` DESC,`id` DESC,`status` DESC, LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
