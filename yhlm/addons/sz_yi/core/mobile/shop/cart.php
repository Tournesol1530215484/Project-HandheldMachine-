...
$tmplateType = (isMobile()) ? 'mobile' : 'pc';
 if ($_W['isajax']) {
    if ($operation == 'display') {
        ...
        $total      = 0;
        $totalprice = 0;
        // $sql        = 'SELECT f.id,f.total,f.goodsid,g.total as stock, o.stock as optionstock, g.maxbuy,g.title,g.thumb,ifnull(o.marketprice, f.marketprice) as marketprice,g.productprice,o.title as optiontitle,f.optionid,o.specs FROM ' . tablename('sz_yi_member_cart') . ' f ' . ' left join ' . tablename('sz_yi_goods') . ' g on f.goodsid = g.id ' . ' left join ' . tablename('sz_yi_goods_option') . ' o on f.optionid = o.id ' . ' where 1 ' . $condition . ' ORDER BY `id` DESC ';
        $sql        = 'SELECT f.id,f.total,f.goodsid,g.total as stock, o.stock as optionstock, g.maxbuy,g.title,g.thumb,f.marketprice as marketprice,g.productprice,o.title as optiontitle,f.optionid,o.specs FROM ' . tablename('sz_yi_member_cart') . ' f ' . ' left join ' . tablename('sz_yi_goods') . ' g on f.goodsid = g.id ' . ' left join ' . tablename('sz_yi_goods_option') . ' o on f.optionid = o.id ' . ' where 1 ' . $condition . ' ORDER BY `id` DESC ';
    ...
    } else if ($operation == 'add' && $_W['ispost']) {          //加入购物车
        $a = 1;
        $id    = intval($_GPC['id']);
        $total = intval($_GPC['total']);
        empty($total) && $total = 1;
        $optionid = intval($_GPC['optionid']);      //规格的id
        $level     = m('member')->getLevel($openid);        //获取
        $member_level1=$level['level'];              //
        $goods    = pdo_fetch('select id,marketprice,discounts,type from ' . tablename('sz_yi_goods') . ' where uniacid=:uniacid and id=:id limit 1', array(
            ':uniacid' => $uniacid,
            ':id' => $id
        ));             //查找商品参数
        ...
        $discounts = json_decode($goods['discounts'], true);
         if(!empty($level['level'])){
               $level['marketprice']     =$goods['marketprice'];        //判断是否有打折
                    //
        }else{
            $level['marketprice'] = $discounts['level'.$level['id']];
            //file_put_contents(dirname(__FILE__).'/002',$val['marketprice']);
        }
        ...
        if (empty($data)) {
            $data = array(
                ...
                'goodsid' => $id,
                'optionid' => $optionid,
                'marketprice' => $level['marketprice'],
                'total' => intval($total)+intval(),
                'diyformid' => $diyformid,
