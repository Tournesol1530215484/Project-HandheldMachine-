...
if ($_W['isajax']) {
    ...
    $member_level     = m('member')->getLevel($openid);
    $member_level1   =pdo_fetch("SELECT max(level) as level2 FROM " . tablename('sz_yi_member_level') . " WHERE uniacid=:uniacid", array(
        ':uniacid' => $uniacid,
       
    ));

    
    $member_level2=$member_level['level'];
    $member_level1=$member_level1['level2'];
    $level1   =pdo_fetchall("SELECT * FROM " . tablename('sz_yi_member_level') . " WHERE uniacid=:uniacid", array(
        ':uniacid' => $uniacid,
       
    ));
   
file_put_contents(dirname(__FILE__).'/dasdsadsa010',$member_level);
