...
$tmplateType = (isMobile()) ? 'mobile' : 'pc';
 if ($_W['isajax']) {
    if ($operation == 'display') {
        ...
        $goods    = pdo_fetch('select id,marketprice,discounts from ' . tablename('sz_yi_goods') . ' where uniacid=:uniacid and id=:id limit 1', array(
            ':uniacid' => $uniacid,
            ':id' => $id
        ));
        ...
        $discounts = json_decode($goods['discounts'], true);
         if(empty($level['level'])){
               $level['marketprice']     =$goods['marketprice'];
