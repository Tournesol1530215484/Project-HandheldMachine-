...
// $authstatement=m('common')->getSysset('bart');  

$authstatement=pdo_fetch('select sets from '.tablename('sz_yi_sysset').' where uniacid ='.$_W['uniacid']);

$authstatement=unserialize($authstatement['sets']);

$authstatement['bart']['protocol'][0]['content']=html_entity_decode($authstatement['bart']['protocol'][0]['content']);

// echo($authstatement['bart']['protocol'][0]['content']);

// exit;

$authstatement=$authstatement['bart'];
...
if ($_W['isajax']) {
    ...
    if ($goods['type'] == 8 ) {
        ...
        if ($goods['PostFlag']==0) {       //不是快递发货的易货产品不能加入购物车
    ...
    if (!empty($goods['hasoption'])) {
        ...
        foreach ($options as $o) {
            ...
            if ($goods['type'] == 8 ){
                ...
                if ($tempprice == 0){
                    ...
                    $tempprice=$o['marketprice'];

                }

                    $tempprice=$tempprice>$o['marketprice']?$o['marketprice']:$tempprice;
        ...
        //如果市场价为空 并且 规格只有一个 goods市场价等于规格marketprice

        if(doubleval($goods['marketprice']) <= 0 && count($options) == 1){

            $goods['marketprice'] = $options[0]['marketprice'];
...
if(!empty($level['level'])){
    ...
    $goods['costprice']=floatval($goods['costprice']);        //用于计算分享利润
  ...
  //file_put_contents(dirname(__FILE__).'/dasdsadsa000',$discounts);   
    ...
    if ($goods['isverify'] == 2) {
        ...
        if (empty($storeids)) {
            ...
            $stores = pdo_fetchall('select * from ' . tablename('sz_yi_exchange_address') . ' where  uniacid=:uniacid and status=1', array(
            ...
            $stores = pdo_fetchall('select * from ' . tablename('sz_yi_exchange_address') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and status=1', array(
        ...
        if (count($options) == 1) {
            ...
            $goods['productprice']=$options[0]['productprice'];
        ...
        if(count($options)>1){
         ...
         for($i=1;$i<$len;$i++){ //该层循环用来控制每轮 冒出一个数 需要比较的次数
            ...
            for($k=0;$k<$len-$i;$k++){
               ...
               if(doubleval($options[$k]['productprice'])>doubleval($options[$k+1]['productprice'])){ 
        ...
        $goods['minproductprice']=$options[0]['productprice'];

        $goods['maxproductprice']=$options[$len-1]['productprice']; 
    ...
    if (!empty($authstatement['protocol'])) {

	    foreach ($authstatement['protocol'] as $key => $value) {

	        $authstatement['protocol'][$key]['content']=html_entity_decode($authstatement['protocol'][$key]['content']);
    ...
    $_W['goods_type']=$goods['type']; 

	         

    !empty($goods['afterSalesServices']) && $goods['afterSalesServices']=html_entity_decode($goods['afterSalesServices']);      
...
if ($com) {
    ...
    //是否全国商家    如果是全国商家type ==2  本地 merchid == $merchid

$supplierInfo=pdo_fetch('select merchid,type,dealmerchid from '.tablename('sz_yi_perm_user').' where uniacid =:uniacid and uid=:uid',array(':uniacid'=>$_W['uniacid'],':uid'=>$goods['supplier_uid']));

if ($supplierInfo['merchid']!=0 && $supplierInfo['type']==2 ){  //本地+全国

    $storeid=pdo_fetchcolumn('select id from '.tablename('sz_yi_merch_user').' where uid=:uid and uniacid=:uniacid',array(':uid'=>$goods['supplier_uid'],':uniacid'=>$_W['uniacid']));

    if ($storeid){

        $storeurl=$this->createMobileUrl('member/merch',array('op'=>'detail','id'=>$storeid));

    }

}else if($supplierInfo['merchid']==0 && $supplierInfo['type']==2){      //全国商家

        $storeurl=$this->createPluginMobileUrl('supplier/store',array('op'=>'skip','storeid'=>$goods['supplier_uid']));

}else if($supplierInfo['merchid']!=0 && $supplierInfo['type'] < 2 ){ //本地
...
}else if($supplierInfo['dealmerchid'] > 0 ){            //易货商家

    $storeurl=$this->createPluginMobileUrl('supplier/store',array('op'=>'skip','merch'=>5,'storeid'=>$goods['supplier_uid']));

}                                                 



            $spoid=pdo_fetchcolumn('select openid from'.tablename('sz_yi_perm_user').' where uniacid = :uniacid and uid=:uid',array(':uniacid'=>$_W['uniacid'],':uid'=>$goods['supplier_uid']));

            $spinfo=m('member')->getMember($spoid);

            $liuyan=$this->createPluginMobileUrl('commission/team',array('op'=>'zixun','sender'=>'superior','id'=>$spinfo['id']));

$this->setHeader();
if($_GPC['debug']==1){
    include $this->template('shop/detail_debug');
}else{
