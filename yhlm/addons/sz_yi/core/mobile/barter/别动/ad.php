...
if ($ac == 'getcate') {		//展示面板
	...
	switch ($cate) {
		...
		case '2':
			$ad=pdo_fetchall('select id,thumb,version,usermax,daymax,national,province,city,area from '.tablename('sz_yi_ad_model').' where uniacid = :uniacid and now = 1 and status = 1  and stime < :time and etime > :time order by id desc ',array(':uniacid'=>$_W['uniacid'],':time'=>time()));
			break;	 

		case '3':
			$ad=pdo_fetchall('select id,thumb,version,usermax,daymax,national,province,city,area from '.tablename('sz_yi_ad_model').' where uniacid = :uniacid and now = 1 and status = 1  and stime < :time and etime > :time ',array(':uniacid'=>$_W['uniacid'],':time'=>time()));
			break;
		default:
