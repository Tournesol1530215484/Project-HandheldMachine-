...
if($op=='display'){
    ...
    foreach ($list as $k=>$v){
        ...
        $temp=pdo_fetchall('select id,type,title,thumb_url from '.tablename('sz_yi_goods').' where uniacid = :uniacid and supplier_uid = :uid and type = 8 and status = 1 and ischeck = 1 and status =1 order by id desc limit 0,2',array(':uniacid'=>$_W['uniacid'],':uid'=>$list[$k]['uid']));    
