...
if ($op == 'detail') {          //广告页详情
	...
	if ($_W['isajax']) {
	    ...
	    //广告主图
	     ...
	     $ad['mainmap']=unserialize($ad['mainmap']);

	    foreach ($ad['mainmap'] as $key => $value) {

	    	$ad['mainmap'][$key]=tomedia($value);
	    ...
	    $ad['goods']=pdo_fetchall('select * from '.tablename('sz_yi_goods').'where uniacid = :uniacid and id in ('.$ad['goodsid'].') ',array(':uniacid'=>$_W['uniacid'])  );//修改

	    foreach ($ad['goods'] as $key => $value) {

	        $ad['goods'][$key]['thumb']=tomedia($value['thumb']);

	        $ad['goods'][$key]['mainmap']=tomedia($value['mainmap']);	//广告主图
...
if ($ac == 'getcate') {		//展示面板
	...
	switch ($cate) {
		...
		default:
			...
			$ad=pdo_fetchall('select id,thumb,mainmap,version,usermax,daymax,national,province,city,area from '.tablename('sz_yi_ad_model').' where uniacid = :uniacid and cate  = :cate and now = 1 and status = 1  and stime < :time and etime > :time ',array(':uniacid'=>$_W['uniacid'],':cate'=>$cate,':time'=>time()));
	...
	if ($thisad) {
		...
		$thisad['mainmap']=unserialize($thisad['mainmap']);	//广告主图
