...
if ($_W['isajax']) {
	if ($op == 'get') {
        ...
        //摒弃所有时间是2019年9月1日之前的数据，所有的数据都是从2019年9月1日之后的数据
        $time = strtotime("2019-09-1 0:00:00"); 
        $mbonus=pdo_fetchcolumn('select sum(money) from '.tablename('sz_yi_year_vip_log').' where uniacid = :uniacid and openid = :openid and ctime > '.$time.' '.$mcon,array(':uniacid'=>$_W['uniacid'],':openid'=>$openid));
        $svs=pdo_fetchcolumn('select sum(money) from '.tablename('sz_yi_barter_services_fee').' where uniacid = :uniacid and mid = :mid and ctime > '.$time.''.$mcon,array(':uniacid'=>$_W['uniacid'],':mid'=>$member['id']));  
        //2019年8月1日的所有的换货服务费
        //print_r($svs);
        $title="换货服务费提现";
        $already=pdo_fetchcolumn('select sum(money) from '.tablename('sz_yi_member_log').' where uniacid = :uniacid and openid = :openid and type=1 and title=:title and status<=1 and createtime > '.$time.'',array(':title'=>$title,':uniacid'=>$_W['uniacid'],':openid'=>$member['openid']));
        // print_r($already);exit;
        $info['already']= $already;

        $total=$mbonus+$svs; 
        $info['total']['service']=$total-$already;  //所有的换货服务费减去已经提现了的
        $info['credit2']=$info['total']['service']; //计算所有的换货服务费
        $temp=$info['credit2'];
