...
if($op=='display'){
  ...
  //获取同城商品
    ...
    }else if ($_GPC['debug'] == 3) {

      include $this -> template('barter/samecity_debug1');
    ...
    if(!empty($_GPC['cate'])){  //获取栏目
        //查询出原id
        $datas=array();
        $ids=array();
        $dataThree=array();
        $datasss=pdo_fetch("select * from hs_sz_yi_category where uniacid={$uniacid} and name='{$_GPC['cate']}'");
        $datas=pdo_fetchall("select * from hs_sz_yi_category where uniacid={$uniacid} and parentid ={$datasss['id']}"); //二级分类
        foreach ($datas as $key => $value) {
            //$dataThree=pdo_fetchall("select * from hs_sz_yi_category where uniacid={$uniacid} and parentid={$value['id']}");   //三级分类，只做这么多了
            $ids[]=$value['id'];
        }

        $tempid=array_values($ids);
        $tempid=implode(',', $tempid);
       // $tempid=array_map(null, $tempid)
        $condition .= " AND `ccate` in ({$tempid})";

       // $params[':ccate'] = 'in' . trim($tempid);

        //$ids=self::Parenttree($datasss['id'],$allcate);
    }
...
if (!empty($_GPC['city'])) {
 ...
 function  Parenttree($id,$date){
        static $tree=array();
        foreach($date as $key=>$value){
            if($value['parentid']==$id){
                $tree[] = $value['id'];
                $value['parentid']=$value['id'];
                $this->Parenttree($value['parentid'], $date);
            }
        }
        return $tree;
    }
