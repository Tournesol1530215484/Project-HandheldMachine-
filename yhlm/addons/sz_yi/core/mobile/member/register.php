...
if($id){
	...
	$member1=pdo_fetch('select * from ' . tablename('sz_yi_member') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'],':id'=>$id));
	if(!empty($member1)){
	   $_SESSION['sid'] = $_GPC['id'];
	}
    ...
    if ($_W['ispost']){
      ...
      $openid = pdo_fetchcolumn('select openid from ' . tablename('sz_yi_member') . ' where mobile=:mobile and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':mobile' => $mobile));
        ...
        if(empty($openid)){
            ...
            $member_data = array('uniacid' => $_W['uniacid'], 'uid' => 0, 'openid' => 'u' . md5($mobile), 'mobile' => $mobile, 'agentid'=>$sid,'pwd' => md5($password), 'createtime' => time(), 'status' => 0, 'regtype' => 2,);
