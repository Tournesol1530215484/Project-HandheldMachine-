...
if ($operation == 'display' && $_W['isajax']){
    ...
    foreach ($order_goods as $data){
        ...
        if($data['deduct2'] == -1){
            show_json(-1,$value['title'].'<br/> 不能使用余额支付！');
        }
    ...
    if ($type == 'credit'){
        ...
        foreach ($order_goods as $key => $value) {
            ...
            if($value['deduct2'] == -1){
                show_json(0,$value['title'].'<br/> 不能使用余额支付！');
            }
        }
