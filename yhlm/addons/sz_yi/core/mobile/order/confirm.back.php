...
// $df  = pdo_fetchall('select * from ' . tablename('sz_yi_member_df') . ' where openid=:openid AND uniacid=:uniacid limit 0,10', array(':openid' => $openid,':uniacid' => $_W['uniacid']));
...
if ($_W['isajax']) {
    ...
    if ($operation == 'display') {
        ...
        show_json(1, array(
           ...
           /* 'realprice' => $discounts['level'.$level['id']],*/
        ...
        foreach ($goodsarr as $g) {
            ...
            if (empty($data['isnodiscount']) && $level['discount'] > 0 && $level['discount'] < 10 && 1==2) {
           ...
           // $totalprice += $discounts['level'.$level['id']];
        ...
        $daje  = $_GPC['daje'];
        //        $abc   =$_GPC['newsend'];
        //        show_json(0, $uname.' '.$daje.' '.$abc);
        if(!empty($uname)&&!empty($daje)){ 
            if($_GPC['newsend'] == 0){ 
                //$userxr = explode(',',$uname);
                $udata = array(
                    'openid' => $openid,
                    'name'   => $uname,
                    'ctime'  => time(),
                ...
                $resilt = pdo_insert('sz_yi_member_df',$udata);
                if(empty($resilt)){ 
                    show_json(0, '代发人错误');
                }
                $order['sfdf'] = $uname;
                ...
                $munid = $_GPC['uname'];
                $ures = pdo_fetch('select * from ' . tablename('sz_yi_member_df') . ' where openid=:openid AND id=:id AND uniacid=:uniacid limit 0,10', array(':id'=>$munid,':openid' => $openid,':uniacid' => $_W['uniacid']));
                $order['sfdf'] = $ures['name'].' '.$ures['phone'];
            }
            $order['dfje'] = $daje;
            $order['price']= $order['price'] + $daje;
        }
