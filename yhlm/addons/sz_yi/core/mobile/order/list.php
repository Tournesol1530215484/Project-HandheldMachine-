...
//$openid="oSI4Lj3W9xYC3ZTjDQndMW6_4YmY";

//$openid="oSI4LjyV5YmKl63hDJNhC9tYKnrI";
	...
	if ($operation == 'display') {
		...
		if ($status != '') {
			...
			if ($status != 4) {
				...
				$condition .= ' and (refundid <> 0 or refundtime != 0) ';
		...
		$list = pdo_fetchall('select id,createtime,ordersn,price,status,iscomment,isverify,verified,verifycode,iscomment,refundid,expresscom,express,expresssn,finishtime,virtual,paytype,expresssn,special,special_status from ' . tablename('sz_yi_order') . " where 1 {$condition} order by createtime desc LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
...
if(!empty($_GPC['style'])){
	...
	if($_GPC[style] == 1){
		...
		include $this->template('order/list1');

	}else if($_GPC[style] == 2){

		include $this->template('order/list2');
