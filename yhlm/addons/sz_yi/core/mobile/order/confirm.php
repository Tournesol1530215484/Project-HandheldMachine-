...
$Memberlevel = m('member')->getLevel($openid);
...
if ($operation == "display" || $operation == "create") {
    ...
    if ($diyform_plugin) {
        ...
        if (!empty($id)) {
            ...
            $sql         = "SELECT supplier_uid, id as goodsid,type,diyformtype,diyformid,diymode FROM " . tablename("sz_yi_goods") . " where id=:id and uniacid=:uniacid  limit 1";
...
if ($_GPC['id']) {
    $goods_type = pdo_fetchcolumn(' select type from '.tablename('sz_yi_goods')." where uniacid=:uniacid and id = '{$_GPC['id']}' limit 1",array(':uniacid'=>$_W['uniacid']));
}else if($_GPC['cartids']){
    $condition = ' and c.id in (' . $_GPC['cartids'] . ')';
    $sql   = 'SELECT g.type FROM ' . tablename('sz_yi_member_cart') . ' c ' . ' left join ' . tablename('sz_yi_goods') . " g on c.goodsid = g.id where c.openid=:openid and c.deleted=0 and c.uniacid=:uniacid {$condition}";
    $temptype = pdo_fetchall($sql, array(
                ':uniacid' => $uniacid,
                ':openid' => $openid
            ));
    ...
    count($temptype)==$yihuo && $goods_type=8;
                        
    if ($yihuo!=0 && count($temptype)!=$yihuo) {    //存在易货商品 且全是易货
        // show_json(0,'易货商品与普通商品需分开下单');     
        die("<script>alert('易货商品与普通商品需分开下单');history.back();</script>");
...
if($operation == 'del'){ 
        $id = $_GPC['id'];
        $res = pdo_delete('sz_yi_member_df', array('id' => $id));
        //$res =1;
        if(!empty($res)){
            show_json(1);
        }else{ 
...
if($operation == 'cx'){ 
        $id = $_GPC['id'];
        $res = pdo_fetchcolumn('select name from'.tablename('sz_yi_member_df') .'where id=:id', array('id' => $id));
        show_json(1,$res);
}
        ...
        if (empty($id)) { // 购物车区间
            ...
            if (empty($goods)) {
                ...
                foreach ($goods as $k => $v) {
                    ...
                    if(!empty($v['discounts'])){

                        $dis = json_decode($v['discounts'], true);

                        if(!empty($dis['level'.$level['id']])){

                            $goods[$k]['marketprice'] = $dis['level'.$level['id']];
            ...
            $dis = json_decode($data['discounts'], true);

            

            if(!empty($dis['level'.$Memberlevel['id']])){

                $data['marketprice'] = $dis['level'.$Memberlevel['id']];

            }else{

                $data['marketprice']=$data['marketprice'];//价钱
...
//        print_r($goods);exit;
        ...
        if($goods_type == 8){
            if ($goods[0]['storeids']){
                $carrier_list = pdo_fetchall('select * from '.tablename('sz_yi_exchange_address').' where uniacid = :uniacid and id IN ('.$goods[0]['storeids'].')',array(':uniacid'=>$_W['uniacid']));
            }
        }else{
            $carrier_list = pdo_fetchall("select * from " . tablename("sz_yi_store") . " where  uniacid=:uniacid and status=1", array(":uniacid" => $_W["uniacid"]));
        }
...
//        print_r($goods);exit;
        foreach ($goods as &$g) {
            ...
            if (empty($g["isnodiscount"]) && floatval($level["discount"]) > 0 && floatval($level["discount"]) < 10 && 1==2) {
...
//            print_r(111);exit;
        ...
        foreach ($goodsarr as $g) {
            ...
            if(p('supplier')){
                ...
                $sql  = 'SELECT id as goodsid,supplier_uid,title,type, weight,total,issendfree,isnodiscount, isreturn,thumb,marketprice,cash,isverify,goodssn,productsn,sales,istime,timestart,timeend,usermaxbuy,maxbuy,unit,buylevels,buygroups,deleted,status,deduct,manydeduct,virtual,discounts,deduct2,ednum,edmoney,edareas,diyformtype,diyformid,diymode,dispatchtype,dispatchid,dispatchprice FROM ' . tablename('sz_yi_goods') . ' where id=:id and uniacid=:uniacid  limit 1';

			}else{

                $sql  = 'SELECT  id as goodsid,title,type, weight,total,issendfree,isnodiscount, isreturn,thumb,marketprice,cash,isverify,goodssn,productsn,sales,istime,timestart,timeend,usermaxbuy,maxbuy,unit,buylevels,buygroups,deleted,status,deduct,manydeduct,virtual,discounts,deduct2,ednum,edmoney,edareas,diyformtype,diyformid,diymode,dispatchtype,dispatchid,dispatchprice FROM ' . tablename('sz_yi_goods') . ' where id=:id and uniacid=:uniacid  limit 1';
            ...
            if (empty($data['isnodiscount']) && $level['discount'] > 0 && $level['discount'] < 10) {
                ...
                $ggprice = $dprice;
                    ...
                    // 报单商品
                if ($_GPC['taxation']['taxationPrice']) {
                    $ggprice = floatval($_GPC['taxation']['taxationPrice']) * $_GPC['taxation']['taxationNum'];
                    // show_json(-1, 'XX商品'.$ggprice);
                }else{ // 普通商品
                    $ggprice = $gprice;
                    // show_json(-1, '普通商品'.$ggprice);
                }
            ...
            if($data['type'] == 8 && empty($address)){  //产品类型为8 地址为空 购买方式:兑换点
                $isverify = true;
            }
        ...
        // show_json(-1, $allgoods);
        $order = array();
        if (!$isvirtual && !$isverify && $dispatchtype == 0) {
            // show_json(-1, $allgoods);
            foreach ($allgoods as $key => $g) {

                // 特殊商品
                if ($g['type'] == 5) {
                    $order['special'] = 1;// 特殊订单的入库标识
                    // 特殊商品不可以混合买
                    foreach ($allgoods as $value) {
                        if ($value['type'] != 5) {
                            show_json(-1, '特殊商品不可以混合买');
                        }
                    }
                }

                // 报单商品 2016年11月16日 11:39:03
                if ($g['type'] == 6) {
                    // 报单商品不可以混合买
                    foreach ($allgoods as $value) {
                        if ($value['type'] != 6) {
                            show_json(-1, '报单商品不可以混合买');
                        }
                    }
                }

                // 接收报单商品的价格
                if ($g['type'] == 6 && $_GPC['taxation']) { 
                    if (count($allgoods) != 1) {
                        show_json(-1, '报单商品一次只能购买一件');
                    }
                }
...
//                    print_r($g);
                    if ($g["dispatchtype"] == 1) { 
                        ...
                        if ($g["dispatchprice"] > 0) {
                            ...
                            $dispatch_price += $g["dispatchprice"] ;    //只收一次邮费
                            // $dispatch_price += $g["dispatchprice"] * $g["total"]; 
...
//            $dispatch_array=$dispatch_array[8];
//            print_r($dispatch_array);
//exit;
            if (!empty($dispatch_array)) {
...
//        print_r($dispatch_price);exit;
        ...
        $order['isexchange']=$goods_data['type'] == 8 ? 1 :0;           //如果为易货商品
        $order['uniacid']          = $uniacid;
        $order['openid']           = $openid;
        $order['ordersn']          = $ordersn;
        $order['price']            = $totalprice;
        // if ($_GPC['id'] == 2326) {
        //     $order['price']            = floatval(8totalprice - $goodsprice);
        // }            
        $order['cash']             = $cash;
        $order['supplier_uid']     = $goods_data['supplier_uid'];
        ...
        if ($_GPC['cartids']) {     //易货商品购物车标识isexchange 1
            $condition = ' and c.id in (' . $_GPC['cartids'] . ')';
            $sql   = 'SELECT g.type FROM ' . tablename('sz_yi_member_cart') . ' c ' . ' left join ' . tablename('sz_yi_goods') . " g on c.goodsid = g.id where c.openid=:openid and c.deleted=0 and c.uniacid=:uniacid {$condition}";
            $temptype = pdo_fetchall($sql, array(
                        ':uniacid' => $uniacid,
            ...
            count($temptype)==$yihuo && $order['isexchange']=1;
//          show_json(0,$allgoods);     		 		 	 	         
        ...
        foreach ($allgoods as $key => $goods) {
            ...
            $order_goods = array(
				...
				'dispatchprice'=>$goods['dispatchprice'],
                'total' => $goods['total'],
                ...
                'discount' => empty($goods['discount'])?0:$goods['discount']

            );

            // 报单商品
            if ($_GPC['taxation']['taxationPrice']) {
                $order_goods['price'] = floatval($_GPC['taxation']['taxationPrice']) * $_GPC['taxation']['taxationNum'];
                // file_put_contents(dirname(__FILE__).'/bd'.$key.'.txt', '测试');
            }else{ // 普通商品
                $order_goods['price'] = $goods['marketprice'] * $goods['total'];
                // file_put_contents(dirname(__FILE__).'/pt'.$key.'.txt', '测试');
            }
        ...
        if($isreturn ==1){
             ...
             // $re =   p('return') -> setGoodsQueue($orderid); // 不需要的功能
...
$allow=true;
// if($goods_type == 8){
//     $sure=pdo_fetch('select id from '.tablename('sz_yi_perm_user').' where openid = :openid and uniacid = :uniacid and dealmerchid > 0 and status = 1 ' ,array(':openid'=>$openid,':uniacid'=>$_W['uniacid']));
//     if (!$sure){
//         $allow=false;
//     }
// }

if(!empty($goods_type)&&$goods_type==4){



include $this->template('order/confirm_level');
