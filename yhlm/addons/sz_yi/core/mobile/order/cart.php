...
//file_put_contents(dirname(__FILE__).'/dasdsadsa000',$optionid);
        if(!empty($level['level'])){
            $level1   =pdo_fetchall("SELECT * FROM " . tablename('sz_yi_member_level') . " WHERE uniacid=:uniacid and level=:level", array(
                ':uniacid' => $uniacid,
                ':level'=>$member_level1
        ...
        foreach ($level1 as &$val) {
           $val['marketprice']     =$discounts['default'];
        }
     
     }else{
        foreach ($level1 as &$val) {
           $val['marketprice']     =$discounts['level'.$val['id']];
        }
}
        ...
        if (empty($data)) {
            $data = array(
                ...
                'goodsid' => $id,
                'optionid' => $optionid,
                'marketprice' => $level1['marketprice'],
                'total' => $total,
                'diyformid' => $diyformid,
