...
//ALTER TABLE `ims_sz_yi_goods` ADD `commission_type` TINYINT(1) NOT NULL DEFAULT '0' COMMENT '商品佣金类别 0统一分销佣金 1详细设置分销佣金' AFTER `commission`;
if(!pdo_fieldexists('sz_yi_goods', 'commission_type')) {
  pdo_query("ALTER TABLE ".tablename('sz_yi_goods')." ADD   `commission_type` TINYINT(1) NOT NULL DEFAULT '0' COMMENT '商品佣金类别 0统一分销佣金 1详细设置分销佣金' ");
}


//ALTER TABLE `ims_mc_members` ADD `credit7` DECIMAL( 10, 2 ) UNSIGNED NOT NULL DEFAULT '0'
if(!pdo_fieldexists('mc_members', 'credit7')) {
  pdo_query("ALTER TABLE ".tablename('mc_members')." ADD   `credit7` DECIMAL( 10, 2 ) UNSIGNED NOT NULL DEFAULT '0' ");
}


 if(!pdo_fieldexists('sz_yi_commission_level', 'autoupdate')) {
  pdo_query("ALTER TABLE ".tablename('sz_yi_commission_level')." ADD   `autoupdate` TINYINT(1)  DEFAULT '1';");
}



if(!pdo_fieldexists('sz_yi_bonus_level', 'conditions')) {
  pdo_query("ALTER TABLE ".tablename('sz_yi_bonus_level')." ADD   `conditions` TINYINT(1)  DEFAULT '1' COMMENT '分红满足条件'  ;");
}


//ALTER TABLE  `ims_sz_yi_bonus_level` ADD  `update_con` VARCHAR( 225 ) NOT NULL DEFAULT  '' COMMENT  '升级条件' AFTER  `conditions`

if(!pdo_fieldexists('sz_yi_bonus_level', 'update_con')) {
  pdo_query("ALTER TABLE ".tablename('sz_yi_bonus_level')." ADD   `update_con` VARCHAR( 225 )   DEFAULT '' COMMENT  '升级条件'  ;");
}


if(!pdo_fieldexists('sz_yi_bonus_level', 'bonuslevel')) {
  pdo_query("ALTER TABLE ".tablename('sz_yi_bonus_level')." ADD   `bonuslevel` int(11)   DEFAULT '0' ");
}

//ALTER TABLE  `ims_sz_yi_goods` ADD  `culate_method` TINYINT( 1 ) UNSIGNED NOT NULL DEFAULT  '0' COMMENT  '计算佣金方式'
if(!pdo_fieldexists('sz_yi_goods', 'culate_method')) {
  pdo_query("ALTER TABLE ".tablename('sz_yi_goods')." ADD   `culate_method` TINYINT( 1 )   UNSIGNED NOT NULL DEFAULT  '6' COMMENT  '计算佣金方式' ;");
}

//ALTER TABLE  `ims_sz_yi_order_goods` ADD  `discount` TINYINT( 1 ) UNSIGNED NOT NULL DEFAULT  '0' COMMENT  '购买时使用的折扣'
if(!pdo_fieldexists('sz_yi_order_goods', 'discount')) {
  pdo_query("ALTER TABLE ".tablename('sz_yi_order_goods')." ADD   `discount` TINYINT( 1 )   UNSIGNED NOT NULL DEFAULT  '0' COMMENT  '购买时使用的折扣'  ;");
}

//ALTER TABLE  `ims_sz_yi_goods` ADD  `bonus_level_id` INT( 11 ) NOT NULL COMMENT  '分红代理等级id'
if(!pdo_fieldexists('sz_yi_goods', 'bonus_level_id')) {
  pdo_query("ALTER TABLE ".tablename('sz_yi_goods')." ADD   `bonus_level_id` INT( 11 )   NOT NULL DEFAULT  '0'   COMMENT  '分红代理等级id' ;");
}
...
if(!pdo_fieldexists('sz_yi_commission_level', 'tiaojian')) {
  pdo_query("ALTER TABLE ".tablename('sz_yi_commission_level')." ADD   `tiaojian`  TINYINT( 1 )   NOT NULL DEFAULT  '0'   ");
}


if(!pdo_fieldexists('sz_yi_commission_level', 'updatelevel')) {
  pdo_query("ALTER TABLE ".tablename('sz_yi_commission_level')." ADD   `updatelevel`  text   NOT NULL   ");
}

if(!pdo_fieldexists('sz_yi_commission_level', 'autoupdate')) {
  pdo_query("ALTER TABLE ".tablename('sz_yi_commission_level')." ADD   `autoupdate` tinyint(1)  NOT NULL DEFAULT  '1'   ");
}

if(!pdo_fieldexists('sz_yi_commission_level', 'weight')) {
  pdo_query("ALTER TABLE ".tablename('sz_yi_commission_level')." ADD   `weight` smallint(6)  NOT NULL DEFAULT  '1'   ");
}

//ALTER TABLE  `ims_sz_yi_designer` ADD  `supplier` INT( 11 ) NOT NULL DEFAULT  '0' COMMENT  '供应商uid'

if(!pdo_fieldexists('sz_yi_designer', 'supplier')) {
  pdo_query("ALTER TABLE ".tablename('sz_yi_designer')." ADD   `supplier` INT( 11 )   NOT NULL DEFAULT  '0' COMMENT  '供应商uid'  ");
}

//sz_yi_designer_menu

if(!pdo_fieldexists('sz_yi_designer_menu', 'supplier')) {
  pdo_query("ALTER TABLE ".tablename('sz_yi_designer_menu')." ADD   `supplier` INT( 11 )   NOT NULL DEFAULT  '0' COMMENT  '供应商uid'  ");
}

/*

if(!pdo_fieldexists('sz_yi_category', 'supplier')) {
  pdo_query("ALTER TABLE ".tablename('sz_yi_category')." ADD   `supplier` INT( 11 )   NOT NULL DEFAULT  '0' COMMENT  '供应商uid'  ");
}

*/

//ALTER TABLE  `ims_sz_yi_goods` CHANGE  `supplier_uid`  `supplier_uid` INT( 11 ) NOT NULL DEFAULT  '0' COMMENT  '供应商ID'


if(!pdo_fieldexists('sz_yi_goods', 'supplier')) {
  pdo_query("ALTER TABLE ".tablename('sz_yi_goods')." CHANGE  `supplier_uid`  `supplier_uid` INT( 11 ) NOT NULL DEFAULT  '0' COMMENT  '供应商ID'  ");
}


//ALTER TABLE  `ims_sz_yi_perm_user` ADD  `setting` VARCHAR( 255 ) NOT NULL DEFAULT  ''
if(!pdo_fieldexists('sz_yi_perm_user', 'setting')) {
  pdo_query("ALTER TABLE ".tablename('sz_yi_perm_user')." ADD  `setting` VARCHAR( 255 ) NOT NULL DEFAULT  '' ");
}
...
//ALTER TABLE  `ims_sz_yi_commission_level` ADD  `member_level` INT( 11 ) NOT NULL DEFAULT  '0' COMMENT  '会员等级'
if(!pdo_fieldexists('sz_yi_commission_level', 'member_level')) {
  pdo_query("ALTER TABLE ".tablename('sz_yi_commission_level')."  ADD  `member_level` INT( 11 ) NOT NULL DEFAULT  '0' COMMENT  '会员等级' ");
}

for ($i=1; $i <=15 ; $i++) { 
  
  if(!pdo_fieldexists('sz_yi_order_goods', 'status'.$i)) {
    pdo_query("ALTER TABLE ".tablename('sz_yi_order_goods')."  ADD  `status{$i}` tinyint( 1 ) NOT NULL DEFAULT  '0'  ");
  }

  if(!pdo_fieldexists('sz_yi_order_goods', 'content'.$i)) {
    pdo_query("ALTER TABLE ".tablename('sz_yi_order_goods')."  ADD  `content{$i}` text   DEFAULT null  ");
  }

  if(!pdo_fieldexists('sz_yi_order_goods', 'commission'.$i)) {
    pdo_query("ALTER TABLE ".tablename('sz_yi_order_goods')."  ADD  `commission{$i}`  text   DEFAULT  null   ");
  }

}

if(!pdo_fieldexists('sz_yi_member', 'bonuslevel')) {
  pdo_query("ALTER TABLE ".tablename('sz_yi_member')." ADD    `bonuslevel`  int(11) DEFAULT '0';");
}

if(!pdo_fieldexists('sz_yi_perm_role', 'status1')) {
  pdo_query("ALTER TABLE ".tablename('sz_yi_perm_role')." ADD    `status1`  tinyint(1) DEFAULT '0';");
}


if(!pdo_fieldexists('sz_yi_commission_level', 'member_level')) {
  pdo_query("ALTER TABLE ".tablename('sz_yi_commission_level')." ADD    `member_level`  int(11) DEFAULT '0';");
}

if(!pdo_fieldexists('sz_yi_goods', 'isgoodsreturn')) {
  pdo_query("ALTER TABLE ".tablename('sz_yi_goods')." ADD    `isgoodsreturn`  tinyint(1)  DEFAULT '0'; ");
}



 
//include dirname(__FILE__).'/plugin/suppliermenu/init.php';
...
$sql = "
 ...
 CREATE TABLE IF NOT EXISTS ".tablename('sz_yi_goods_return_log'). " (
 ...
 CREATE TABLE IF NOT EXISTS ".tablename('sz_yi_member_style'). " (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `style` tinyint(1)  NOT NULL,
  `stylename` varchar(255)  DEFAULT NULL,
  `styletime` int(11) DEFAULT '0',
  PRIMARY KEY (`id`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8;



CREATE TABLE IF NOT EXISTS ".tablename('sz_yi_chat_log')." (
  `id` int(11) NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS  ".tablename('sz_yi_chat_room')."  (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `mid` int(11) NOT NULL DEFAULT '0',
  `uniacid` int(11) NOT NULL DEFAULT '0',
...
CREATE TABLE IF NOT EXISTS ".tablename('sz_yi_adpc')." (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `uniacid` int(11) DEFAULT '0',
...
pdo_query("UPDATE ".tablename('sz_yi_plugin')."   SET `name` = '海生分销' WHERE `identity` = 'commission'");
...
if(!pdo_fieldexists('sz_yi_adv', 'thumb_pc')) {
  pdo_query("ALTER TABLE ".tablename('sz_yi_adv')." ADD     `thumb_pc`  varchar(255)  DEFAULT '' ;");
}


for($i=1;$i<=15;$i++){

  if(!pdo_fieldexists('sz_yi_commission_level', 'commission'.$i)) {
    pdo_query("ALTER TABLE ".tablename('sz_yi_commission_level')." ADD     `commission{$i}`  decimal(10,2)  DEFAULT '0.00' ;");
  }
}


//authority
  if(!pdo_fieldexists('sz_yi_commission_level', 'authority')) {
    pdo_query("ALTER TABLE ".tablename('sz_yi_commission_level')." ADD     `authority`  tinyint(1)  DEFAULT '0' ;");
  }
...
if(!pdo_fieldexists('sz_yi_order_goods', 'openid')) {
    pdo_query("ALTER TABLE ".tablename('sz_yi_order_goods')." ADD     `openid`  varchar(100)  DEFAULT '' ;");
}
//diyformdata

if(!pdo_fieldexists('sz_yi_order_goods', 'diyformdata')) {
    pdo_query("ALTER TABLE ".tablename('sz_yi_order_goods')." ADD     `diyformdata` text  DEFAULT '' ;");
}

//diyformfields
if(!pdo_fieldexists('sz_yi_order_goods', 'diyformfields')) {
    pdo_query("ALTER TABLE ".tablename('sz_yi_order_goods')." ADD     `diyformfields` text  DEFAULT '' ;");
}


/*
CREATE TABLE IF NOT EXISTS `ims_sz_yi_liuyan` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `weid` int(11) DEFAULT '0',
  `superior_id` int(11) DEFAULT '0',
  `lower_id` int(11) DEFAULT '0',
...
CREATE TABLE IF NOT EXISTS `ims_sz_yi_chat` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `uniacid` int(11) NOT NULL DEFAULT '0',
  `content` text NOT NULL,
  `sender` int(11) NOT NULL DEFAULT '0',
  `geter` int(11) NOT NULL DEFAULT '0',
  `ctime` int(11) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`)
) ENGINE=MyISAM DEFAULT CHARSET=gbk AUTO_INCREMENT=1 ;
...
$sql = "
CREATE TABLE IF NOT EXISTS ".tablename('sz_yi_liuyan')." (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `weid` int(11) DEFAULT '0',
  `superior_id` int(11) DEFAULT '0',
  `lower_id` int(11) DEFAULT '0',
...
CREATE TABLE IF NOT EXISTS ".tablename('sz_yi_chat')."(
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `uniacid` int(11) NOT NULL DEFAULT '0',
  `content` text NOT NULL,
  `sender` int(11) NOT NULL DEFAULT '0',
