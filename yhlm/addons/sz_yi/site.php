...
class Sz_yiModuleSite extends Core { 
    ...
    public function doMobileApi(){ 
            ...
            $res = pdo_fetchall('select uniacid from  '.tablename('account')." where isdeleted = 0 ");

 

            foreach ($res as $key => $value) {

                  $url = $_W['siteroot']."app/index.php?i={$value['uniacid']}&c=entry&method=api&p=bonus&m=sz_yi&do=plugin";
                  


                file_get_contents($url);

                $url1 =  $_W['siteroot']."app/index.php?i={$value['uniacid']}&c=entry&method=api&p=return&m=sz_yi&do=plugin";  
    ...
    //易货商城
    public function doMobileBarter(){ $this->_exec(__FUNCTION__,'index',false); }
